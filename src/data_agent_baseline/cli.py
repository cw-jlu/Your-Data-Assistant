"""
此模块提供命令行界面 (CLI) 支持，用于管理项目状态、检查任务以及运行基准评估。
"""
import os
...
# Windows 上强制使用 UTF-8 编码，避免 GBK 编码问题
if os.name == 'nt':
...
@app.command()
def status(
    config: Path = typer.Option(..., exists=True, dir_okay=False, help="YAML 配置文件路径。"),
) -> None:
    """显示本地项目布局以及数据文件的存在情况。"""
...
def inspect_task(
    ...
    """显示指定任务的元数据以及 context/ 目录下的可用文件列表。"""
    app_config = load_app_config(config)
...
def run_benchmark_command(
    config: Path = typer.Option(..., exists=True, dir_okay=False, help="YAML 配置文件路径。"),
    limit: int | None = typer.Option(None, min=1, help="最大运行任务数量。"),
) -> None:
    """根据配置从数据集中批量运行多个任务并评估。"""
    app_config = load_app_config(config)
    dataset = DABenchPublicDataset(app_config.dataset.root_path)
