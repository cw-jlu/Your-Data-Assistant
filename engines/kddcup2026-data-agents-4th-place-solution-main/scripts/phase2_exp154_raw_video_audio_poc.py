"""Phase 2 PoC: exp152-style raw video + keyframe note + audio ASR.

This script intentionally does not modify exp152. It reuses exp152 components,
keeps raw video attached to the main agent, injects a CPU-ASR transcript into
the preamble, and optionally exposes the same combined video note to
source_router.
...
# Match exp152 production defaults, plus ASR. Keyframe note extraction still
# samples video at 1 fps, but image keyframes are not attached to the main agent;
# with EXP152_VIDEO=0/EXP152_WATCH_VIDEO=0, exp152 attaches raw video_url.
os.environ.setdefault("EXP152_KEYFRAMES", "0")
os.environ.setdefault("EXP152_VIDEO_KEYFRAME_NOTE", "1")
os.environ.setdefault("EXP152_KEYFRAME_SAMPLE_FPS", "1.0")
...
def main() -> None:
    ...
    i = 1
    while (runs / f"exp_154_raw_video_audio_poc_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_154_raw_video_audio_poc_{i:03d}"
    out_dir.mkdir(parents=True)

    os.environ.setdefault(
        "EXP152_KEYFRAME_ROOT",
        str(Path("/tmp") / "kobushi_exp154_keyframe_cache" / out_dir.name),
    )
    os.environ.setdefault(
        "EXP152_PDF_TEXT_CACHE_ROOT",
        str(Path("/tmp") / "kobushi_exp154_pdf_text_cache" / out_dir.name),
    )
    ...
    print(f"=== exp154 raw-video+audio POC -> {out_dir.name} ===", flush=True)
    print(
