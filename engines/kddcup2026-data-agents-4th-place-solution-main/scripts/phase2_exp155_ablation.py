"""Phase 2 exp_155 v4 ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_155_phase_tool_visibility.flags):
  EXP155_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP155_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP155_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP155_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP155_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP155_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool
  EXP155_FINAL_SQL_GUARD=1 → classify and rewrite unnecessary DISTINCT / NULL filters
  EXP155_AUDIO_ASR=1     → transcribe task video audio as a cautious preamble/router hint

exp155 is the v4 candidate: v1/exp149-style column auditor + the exp153 ASR
pipeline. The v2 projection pruner and final SQL guard are deliberately off by
default.

Run dir is named by the active levers, e.g.:
  EXP155_ANSWER_SHAPE=1 python scripts/phase2_exp155_ablation.py   → exp_155_answer_shape_001
  EXP155_PROSE=1        python scripts/phase2_exp155_ablation.py   → exp_155_prose_001
  EXP155_KEYFRAMES=1    python scripts/phase2_exp155_ablation.py   → exp_155_keyframes_001
  EXP155_WATCH_VIDEO=1  python scripts/phase2_exp155_ablation.py   → exp_155_watch_video_001
  (no flags)            python scripts/phase2_exp155_ablation.py   → exp_155_baseline_001
...
from experiments.exp_155_phase_tool_visibility.phased_agent import PhasedReActAgent, PhasedAgentConfig
...
from experiments.exp_155_phase_tool_visibility.anti_aggregation import no_agg_note, review_aggregation
...
def main():
    ...
    i = 1
    while (runs / f"exp_155_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_155_{tag}_{i:03d}"
    ...
    print(f"=== exp_155 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP155_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp155"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP155_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP155_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP155_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP155_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP155_KEYFRAME_MAX_IMAGES", "18"),
                   "audio_asr": flags.audio_asr_on(),
                   "audio_asr_cache_root": os.environ.get("EXP155_AUDIO_ASR_CACHE_ROOT"),
