"""Phase 2 exp_153 final-SQL-guard ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_153_audio_asr.flags):
  EXP153_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP153_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP153_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP153_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP153_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP153_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool
  EXP153_FINAL_SQL_GUARD=1 → classify and rewrite unnecessary DISTINCT / NULL filters

exp153 forks exp150 and adds Final SQL Guard on top of the projection-pruner
pipeline.

Run dir is named by the active levers, e.g.:
  EXP153_ANSWER_SHAPE=1 python scripts/phase2_exp153_ablation.py   → exp_153_answer_shape_001
  EXP153_PROSE=1        python scripts/phase2_exp153_ablation.py   → exp_153_prose_001
  EXP153_KEYFRAMES=1    python scripts/phase2_exp153_ablation.py   → exp_153_keyframes_001
  EXP153_WATCH_VIDEO=1  python scripts/phase2_exp153_ablation.py   → exp_153_watch_video_001
  (no flags)            python scripts/phase2_exp153_ablation.py   → exp_153_baseline_001
...
from experiments.exp_153_audio_asr.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_153_audio_asr.preamble import build_preamble
from experiments.exp_153_audio_asr.tools.registry import create_default_tool_registry
from experiments.exp_153_audio_asr.adaptive_vote import adaptive_vote
from experiments.exp_153_audio_asr.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_153_audio_asr.math_advisor import generate_formula
from experiments.exp_153_audio_asr.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_153_audio_asr.video_summarizer import summarize_video
from experiments.exp_153_audio_asr.prefix_cache import (
    prefix_cache_enabled,
    with_prefix_cache_header,
)
from experiments.exp_153_audio_asr.audio_asr import (
...
def run_one(
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_153_audio_asr.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_153_audio_asr.keyframe_cache import ensure_task_keyframes
    ...
    for i in range(N_ATTEMPTS):
        try:
            ...
            tools = create_default_tool_registry(
                auditor_model=m,
                guard_model=make_model(temp=0.0, task_id=tid) if flags.final_sql_guard_on() else None,
...
def main():
    ...
    i = 1
    while (runs / f"exp_153_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_153_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP153_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP153_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp153_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP153_KEYFRAME_ROOT"):
        os.environ["EXP153_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp153_keyframe_cache" / out_dir.name
        )
    if flags.audio_asr_on() and not os.environ.get("EXP153_AUDIO_ASR_CACHE_ROOT"):
        os.environ["EXP153_AUDIO_ASR_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp153_audio_asr_cache" / out_dir.name
        )
    ...
    print(f"=== exp_153 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP153_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP153_KEYFRAME_ROOT')}", flush=True)
    if flags.audio_asr_on():
        print(f"  audio_asr_cache_root={os.environ.get('EXP153_AUDIO_ASR_CACHE_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP153_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp153"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP153_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP153_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP153_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP153_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP153_KEYFRAME_MAX_IMAGES", "18"),
                   "audio_asr": flags.audio_asr_on(),
                   "audio_asr_cache_root": os.environ.get("EXP153_AUDIO_ASR_CACHE_ROOT"),
