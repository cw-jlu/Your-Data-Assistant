"""Full 50-task bench against local llama.cpp, single-attempt per task.
- Sequential tasks (= llama.cpp parallel=1, so serial)
...
def main():
    ...
    # Auto-pick next free out dir
    _runs = ROOT / "artifacts" / "runs"
    _i = 1
    while (_runs / f"llamacpp_single_{_i:03d}").exists():
        _i += 1
    OUT = _runs / f"llamacpp_single_{_i:03d}"
    ...
    print(f"[{time.strftime('%H:%M:%S')}] tasks={len(tids)}, single-attempt on local llama.cpp", flush=True)
