"""Phase 2 exp_144 per-lever ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_144_modality.flags):
  EXP144_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP144_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP144_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP144_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP144_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool

ALL flags off ⇒ identical pipeline to exp_143_dynamic_doc (sanity baseline).

Run dir is named by the active levers, e.g.:
  EXP144_ANSWER_SHAPE=1 python scripts/phase2_exp144_ablation.py   → exp_144_answer_shape_001
  EXP144_PROSE=1        python scripts/phase2_exp144_ablation.py   → exp_144_prose_001
  EXP144_KEYFRAMES=1    python scripts/phase2_exp144_ablation.py   → exp_144_keyframes_001
  EXP144_WATCH_VIDEO=1  python scripts/phase2_exp144_ablation.py   → exp_144_watch_video_001
  (no flags)            python scripts/phase2_exp144_ablation.py   → exp_144_baseline_001
...
from experiments.exp_144_modality.phased_agent import PhasedReActAgent, PhasedAgentConfig
...
from experiments.exp_144_modality.video_summarizer import summarize_video
...
def run_one(tid: str, out_dir: Path) -> dict:
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_144_modality.keyframe_cache import ensure_task_keyframes
...
def main():
    ...
    i = 1
    while (runs / f"exp_144_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_144_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP144_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP144_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp144_pdf_text_cache" / out_dir.name
        )
    if flags.keyframes_on() and not os.environ.get("EXP144_KEYFRAME_ROOT"):
        os.environ["EXP144_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp144_keyframe_cache" / out_dir.name
        )
    ...
    print(f"=== exp_144 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP144_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on():
        print(f"  keyframe_root={os.environ.get('EXP144_KEYFRAME_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP144_PREFIX_CACHE_NAMESPACE", "kobushi-phase2"),
                   "source_router": flags.source_router_on(),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP144_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP144_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP144_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP144_KEYFRAME_MAX_IMAGES", "18")},
        "results": results,
