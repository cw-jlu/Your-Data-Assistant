"""Phase 2 exp_163 v4 ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_163_plateau_keyframes.flags):
  EXP163_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP163_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP163_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP163_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP163_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP163_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool
  EXP163_FINAL_SQL_GUARD=1 → classify and rewrite unnecessary DISTINCT / NULL filters
  EXP163_AUDIO_ASR=1     → transcribe task video audio as a cautious preamble/router hint

exp163 is the v4 candidate: v1/exp149-style column auditor + the exp153 ASR
pipeline. The v2 projection pruner and final SQL guard are deliberately off by
default.

Run dir is named by the active levers, e.g.:
  EXP163_ANSWER_SHAPE=1 python scripts/phase2_exp163_ablation.py   → exp_163_answer_shape_001
  EXP163_PROSE=1        python scripts/phase2_exp163_ablation.py   → exp_163_prose_001
  EXP163_KEYFRAMES=1    python scripts/phase2_exp163_ablation.py   → exp_163_keyframes_001
  EXP163_WATCH_VIDEO=1  python scripts/phase2_exp163_ablation.py   → exp_163_watch_video_001
  (no flags)            python scripts/phase2_exp163_ablation.py   → exp_163_baseline_001
...
from experiments.exp_163_plateau_keyframes import flags
from experiments.exp_163_plateau_keyframes.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_163_plateau_keyframes.preamble import build_preamble
from experiments.exp_163_plateau_keyframes.tools.registry import create_default_tool_registry
from experiments.exp_163_plateau_keyframes.adaptive_vote import adaptive_vote
from experiments.exp_163_plateau_keyframes.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_163_plateau_keyframes.math_advisor import generate_formula
from experiments.exp_163_plateau_keyframes.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_163_plateau_keyframes.video_summarizer import summarize_video
from experiments.exp_163_plateau_keyframes.prefix_cache import (
    prefix_cache_enabled,
    with_prefix_cache_header,
)
from experiments.exp_163_plateau_keyframes.audio_asr import (
...
_NUM_COMMA_RE = __import__("re").compile(r"^(-?\d{1,3}(?:,\d{3})+)(\.\d+)?$")


def _normalize_numeric_cell(value):
    """Strip thousands-commas from digits+commas-only cells so a correct-but-
    comma-formatted number matches gold's bare-number form. Touches nothing
    else (text, %, units, 亿/万). Mirrors submission/main.py."""
    t = str(value).strip()
...
def run_one(
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_163_plateau_keyframes.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_163_plateau_keyframes.keyframe_cache import ensure_task_keyframes
                ...
                "steps": [
                    {
                        ...
                        "thought": (s.thought or "")[:20000],
                        # exp163 PoC: capture full model output incl. <think> so we
                        # can verify the agent actually reads the video slides.
                        "raw_response": (s.raw_response or "")[:30000],
                        "observation_ok": s.observation.get("ok") if isinstance(s.observation, dict) else None,
                        "observation_content_preview": str(s.observation.get("content") if isinstance(s.observation, dict) else s.observation)[:4000],
                    }
    ...
    with open(pred, "w", newline="") as f:
        w = csv.writer(f); w.writerow(voted.columns)
        [w.writerow([_normalize_numeric_cell(c) for c in r]) for r in voted.rows]
...
def main():
    ...
    i = 1
    while (runs / f"exp_163_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_163_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP163_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP163_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp163_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP163_KEYFRAME_ROOT"):
        os.environ["EXP163_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp163_keyframe_cache" / out_dir.name
        )
    if flags.audio_asr_on() and not os.environ.get("EXP163_AUDIO_ASR_CACHE_ROOT"):
        os.environ["EXP163_AUDIO_ASR_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp163_audio_asr_cache" / out_dir.name
        )
    ...
    print(f"=== exp_163 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP163_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP163_KEYFRAME_ROOT')}", flush=True)
    if flags.audio_asr_on():
        print(f"  audio_asr_cache_root={os.environ.get('EXP163_AUDIO_ASR_CACHE_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP163_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp163"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP163_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP163_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP163_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP163_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP163_KEYFRAME_MAX_IMAGES", "50"),
                   "audio_asr": flags.audio_asr_on(),
                   "audio_asr_cache_root": os.environ.get("EXP163_AUDIO_ASR_CACHE_ROOT"),
