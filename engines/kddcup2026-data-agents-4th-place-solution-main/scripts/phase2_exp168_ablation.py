"""Phase 2 exp_168 per-lever ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_168_column_selector.flags):
  EXP168_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP168_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP168_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP168_KEYFRAMES=1     → attach plateau-selected keyframes as images (no raw video)
  EXP168_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP168_VISUAL_ATTACH=0 → keep keyframe text note but attach no images/video
  EXP168_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool

ALL flags off ⇒ identical pipeline to exp_143_dynamic_doc (sanity baseline).

Run dir is named by the active levers, e.g.:
  EXP168_ANSWER_SHAPE=1 python scripts/phase2_exp166_ablation.py   → exp_167_answer_shape_001
  EXP168_PROSE=1        python scripts/phase2_exp166_ablation.py   → exp_167_prose_001
  EXP168_KEYFRAMES=1    python scripts/phase2_exp166_ablation.py   → exp_167_keyframes_001
  EXP168_WATCH_VIDEO=1  python scripts/phase2_exp166_ablation.py   → exp_167_watch_video_001
...
from experiments.exp_168_column_selector.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_168_column_selector.preamble import build_preamble
from experiments.exp_168_column_selector.tools.registry import create_default_tool_registry
from experiments.exp_168_column_selector.domain_router import classify_domain
from experiments.exp_168_column_selector.domain_prompt import domain_note, modality_note
from experiments.exp_168_column_selector.adaptive_vote import adaptive_vote
from experiments.exp_168_column_selector.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_168_column_selector.math_advisor import generate_formula
from experiments.exp_168_column_selector.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_168_column_selector.video_summarizer import summarize_video
from experiments.exp_168_column_selector.prefix_cache import (
...
def _run_advisors(question: str, tid: str, run_formula: bool = True) -> tuple[str, object]:
    ...
    runner: with EXP168_MATH_OTHER_ONLY the formula fires only when domain='other'.
    """
...
def run_one(tid: str, out_dir: Path) -> dict:
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_168_column_selector.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_168_column_selector.keyframe_cache import ensure_task_keyframes
    ...
    # Final-step output-column selector (post-vote = after VERIFY + adaptive_vote).
    # Additive to the in-loop column auditor; finance/EHR only; keep-all on error.
    final_cols = list(voted.columns)
    final_rows = [list(r) for r in voted.rows]
    if flags.column_selector_on() and str(domain_for_audit).strip().lower() != "other":
        try:
            from experiments.exp_168_column_selector.column_selector import select_output_columns
            sel = select_output_columns(
                question=task.question, columns=final_cols, rows=final_rows,
                model=make_model(0.0, task_id=tid),
            )
            meta = sel.to_dict(); meta["domain"] = domain_for_audit
            if sel.applied:
                final_cols, final_rows = sel.columns, sel.rows
            (task_dir / "column_selector.json").write_text(
                json.dumps(meta, ensure_ascii=False, indent=2))
        except Exception as e:
            (task_dir / "column_selector.json").write_text(
                json.dumps({"applied": False, "error": str(e)[:300]}, ensure_ascii=False, indent=2))
    pred = task_dir / "prediction.csv"
    with open(pred, "w", newline="") as f:
        w = csv.writer(f); w.writerow(final_cols); [w.writerow(r) for r in final_rows]
    score = _official_score(pred, GOLD_ROOT / tid / "gold.csv")
    return {"tid": tid, "formula": formula, "anti_aggregation": anti_agg.raw,
            "anti_agg_label": anti_agg.label, "advisor_used": advisor_used,
            "score": score, "n_ok": len(answers),
            "cols": list(final_cols), "elapsed": time.time() - t0}
...
def main():
    ...
    i = 1
    while (runs / f"exp_168_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_168_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP168_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP168_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp166_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP168_KEYFRAME_ROOT"):
        os.environ["EXP168_KEYFRAME_ROOT"] = str(
    ...
    print(f"=== exp_168 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP168_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP168_KEYFRAME_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP168_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp166"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP168_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP168_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP168_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP168_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP168_KEYFRAME_MAX_IMAGES", "50")},
        "results": results,
