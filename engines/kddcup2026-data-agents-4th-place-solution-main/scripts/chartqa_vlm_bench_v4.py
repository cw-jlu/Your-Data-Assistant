"""ChartQA bench v4 — v2's verbatim answer-type prompt + OPTIONAL python tool.

v3 lesson:
  - long tool-centric system prompt caused 2 verbatim drift regressions
  - tool was only invoked 1/50 times when self-selected
  - clean acc dropped from 0.933 (v2) to 0.875

v4 strategy:
  - Keep v2's strict verbatim rules and ANSWER: format as the primary contract
  - Add tool as a SHORT optional clause at the end
  - Conservative on multi-turn (max 3 turns)
"""
...
PROMPT_V4 = """Read the chart and answer the question.
...
def main():
    ...
    ap.add_argument("--out", default="artifacts/chartqa_bench_v4_mix.json")
    args = ap.parse_args()
    ...
    out_path.write_text(json.dumps({
        ...
        "prompt_version": "v4_mix_verbatim_plus_optional_tool",
        "results": results,
