"""ChartQA test bench v2 — fixes answer-type prompt bias.

v1 prompt: "Answer with the value ONLY ... For numbers, return just the digits"
  → biases label questions to return numbers (= idx 1126/1139/2233 fail).

v2 prompt: explicit two-stage answer:
  - First, identify whether the answer is a CATEGORY/LABEL or a NUMBER
  - Then output the answer in the appropriate form
"""
...
PROMPT_V2 = """Read the chart and answer the question.
...
After your reasoning, output ONLY the final answer on the last line.
Format: ANSWER: <value>"""
...
def ask(question: str, b64: str, max_tokens: int = 4096) -> dict:
    body = {
        "model": MODEL,
        "messages": [{
            "role": "user",
            "content": [
                {"type": "text", "text": f"{PROMPT_V2}\n\nQuestion: {question}"},
...
def run_one(idx: int, ex: dict, max_tokens: int) -> dict:
    ...
    try:
        ...
        ok = relaxed_match(final, gold)
        return {
            "idx": idx, "q": q[:120], "gold": gold,
            "pred": final[:120], "raw": content[:280],
...
def main():
    ...
    ap.add_argument("--out", default="artifacts/chartqa_bench_v2.json")
    args = ap.parse_args()
