"""Phase 2 exp_151 column-auditor-baseline ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_151_column_auditor_baseline.flags):
  EXP151_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP151_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP151_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP151_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP151_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP151_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool

exp151 is a fresh fork of exp149. It intentionally keeps the old
post-execution column auditor and does not include exp150's terminal
projection-pruner integration.

Run dir is named by the active levers, e.g.:
  EXP151_ANSWER_SHAPE=1 python scripts/phase2_exp151_ablation.py   → exp_151_answer_shape_001
  EXP151_PROSE=1        python scripts/phase2_exp151_ablation.py   → exp_151_prose_001
  EXP151_KEYFRAMES=1    python scripts/phase2_exp151_ablation.py   → exp_151_keyframes_001
  EXP151_WATCH_VIDEO=1  python scripts/phase2_exp151_ablation.py   → exp_151_watch_video_001
  (no flags)            python scripts/phase2_exp151_ablation.py   → exp_151_baseline_001
...
from experiments.exp_151_column_auditor_baseline.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_151_column_auditor_baseline.preamble import build_preamble
from experiments.exp_151_column_auditor_baseline.tools.registry import create_default_tool_registry
from experiments.exp_151_column_auditor_baseline.adaptive_vote import adaptive_vote
from experiments.exp_151_column_auditor_baseline.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_151_column_auditor_baseline.math_advisor import generate_formula
from experiments.exp_151_column_auditor_baseline.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_151_column_auditor_baseline.video_summarizer import summarize_video
from experiments.exp_151_column_auditor_baseline.prefix_cache import (
...
def run_one(tid: str, out_dir: Path) -> dict:
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_151_column_auditor_baseline.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_151_column_auditor_baseline.keyframe_cache import ensure_task_keyframes
...
def main():
    ...
    i = 1
    while (runs / f"exp_151_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_151_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP151_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP151_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp151_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP151_KEYFRAME_ROOT"):
        os.environ["EXP151_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp151_keyframe_cache" / out_dir.name
        )
    ...
    print(f"=== exp_151 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP151_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP151_KEYFRAME_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP151_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp151"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP151_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP151_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP151_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP151_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP151_KEYFRAME_MAX_IMAGES", "18")},
        "results": results,
