"""exp_138 math advisor bench using SUBMISSION-IDENTICAL pipeline.

Differences from math_advisor_full50.py:
- Delegates to `experiments.exp_138_thin_preamble.runner._run_single_task_with_timeout`
...
- Math advisor formula injection happens inside each subprocess via
  exp_138_thin_preamble/preamble.py (= ad-hoc bench-level injection removed).
...
# Use exp_138 module's runner and config (= same as submission/main.py)
from experiments.exp_138_thin_preamble.runner import _run_single_task_with_timeout
from experiments.exp_138_thin_preamble.config import load_app_config, AppConfig

OPTS = EvaluationOptions()
N_ATTEMPTS_NOTE = "(= configured in exp_138.runner._ATTEMPT_TEMPS = (0.6, 0.6, 0.6))"
MAX_WORKERS = 4
...
def _build_app_config() -> AppConfig:
    ...
    config_yaml = REPO / "src" / "experiments" / "exp_138_thin_preamble" / "config.yaml"
...
def main():
    ...
    _i = 1
    while (_runs / f"exp_138_thin_preamble_{_i:03d}").exists():
        _i += 1
    OUT = _runs / f"exp_138_thin_preamble_{_i:03d}"
    OUT.mkdir(parents=True, exist_ok=True)
    print(f"[init] output dir: {OUT}", flush=True)

    config = _build_app_config()
    _log(f"=== exp_138 math advisor bench (submission-style: 4 tasks parallel × 3 attempts spawn, max_steps={config.agent.max_steps}) ===")
