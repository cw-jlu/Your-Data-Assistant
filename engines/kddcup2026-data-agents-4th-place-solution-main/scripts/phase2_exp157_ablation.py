"""Phase 2 exp_157 v4 ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_157_domain_prompt.flags):
  EXP157_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP157_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP157_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP157_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP157_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP157_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool
  EXP157_FINAL_SQL_GUARD=1 → classify and rewrite unnecessary DISTINCT / NULL filters
  EXP157_AUDIO_ASR=1     → transcribe task video audio as a cautious preamble/router hint

exp157 is the v4 candidate: v1/exp149-style column auditor + the exp153 ASR
pipeline. The v2 projection pruner and final SQL guard are deliberately off by
default.

Run dir is named by the active levers, e.g.:
  EXP157_ANSWER_SHAPE=1 python scripts/phase2_exp157_ablation.py   → exp_157_answer_shape_001
  EXP157_PROSE=1        python scripts/phase2_exp157_ablation.py   → exp_157_prose_001
  EXP157_KEYFRAMES=1    python scripts/phase2_exp157_ablation.py   → exp_157_keyframes_001
  EXP157_WATCH_VIDEO=1  python scripts/phase2_exp157_ablation.py   → exp_157_watch_video_001
  (no flags)            python scripts/phase2_exp157_ablation.py   → exp_157_baseline_001
...
from experiments.exp_157_domain_prompt import flags
from experiments.exp_157_domain_prompt.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_157_domain_prompt.preamble import build_preamble
from experiments.exp_157_domain_prompt.tools.registry import create_default_tool_registry
from experiments.exp_157_domain_prompt.adaptive_vote import adaptive_vote
from experiments.exp_157_domain_prompt.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_157_domain_prompt.math_advisor import generate_formula
from experiments.exp_157_domain_prompt.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_157_domain_prompt.video_summarizer import summarize_video
from experiments.exp_157_domain_prompt.prefix_cache import (
    prefix_cache_enabled,
    with_prefix_cache_header,
)
from experiments.exp_157_domain_prompt.audio_asr import (
    audio_preamble_block,
    find_video,
    load_existing_asr,
    prepare_task_audio,
)
from experiments.exp_157_domain_prompt.domain_router import classify_domain
...
def run_one(
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_157_domain_prompt.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_157_domain_prompt.keyframe_cache import ensure_task_keyframes
...
def main():
    ...
    i = 1
    while (runs / f"exp_157_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_157_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP157_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP157_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp157_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP157_KEYFRAME_ROOT"):
        os.environ["EXP157_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp157_keyframe_cache" / out_dir.name
        )
    if flags.audio_asr_on() and not os.environ.get("EXP157_AUDIO_ASR_CACHE_ROOT"):
        os.environ["EXP157_AUDIO_ASR_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp157_audio_asr_cache" / out_dir.name
        )
    ...
    print(f"=== exp_157 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP157_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP157_KEYFRAME_ROOT')}", flush=True)
    if flags.audio_asr_on():
        print(f"  audio_asr_cache_root={os.environ.get('EXP157_AUDIO_ASR_CACHE_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP157_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp157"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP157_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP157_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP157_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP157_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP157_KEYFRAME_MAX_IMAGES", "18"),
                   "audio_asr": flags.audio_asr_on(),
                   "audio_asr_cache_root": os.environ.get("EXP157_AUDIO_ASR_CACHE_ROOT"),
