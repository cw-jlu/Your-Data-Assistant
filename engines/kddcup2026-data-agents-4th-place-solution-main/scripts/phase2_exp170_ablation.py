...
Each lever is toggled by an env var (see experiments.exp_170_prose_table.flags):
  EXP170_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP170_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP170_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP170_KEYFRAMES=1     → attach plateau-selected keyframes as images (no raw video)
  EXP170_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP170_VISUAL_ATTACH=0 → keep keyframe text note but attach no images/video
  EXP170_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool

ALL flags off ⇒ identical pipeline to exp_143_dynamic_doc (sanity baseline).

Run dir is named by the active levers, e.g.:
  EXP170_ANSWER_SHAPE=1 python scripts/phase2_exp166_ablation.py   → exp_167_answer_shape_001
  EXP170_PROSE=1        python scripts/phase2_exp166_ablation.py   → exp_167_prose_001
  EXP170_KEYFRAMES=1    python scripts/phase2_exp166_ablation.py   → exp_167_keyframes_001
  EXP170_WATCH_VIDEO=1  python scripts/phase2_exp166_ablation.py   → exp_167_watch_video_001
...
from experiments.exp_170_prose_table.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_170_prose_table.preamble import build_preamble
from experiments.exp_170_prose_table.tools.registry import create_default_tool_registry
from experiments.exp_170_prose_table.domain_router import classify_domain
from experiments.exp_170_prose_table.domain_prompt import domain_note, modality_note
from experiments.exp_170_prose_table.adaptive_vote import adaptive_vote
from experiments.exp_170_prose_table.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_170_prose_table.math_advisor import generate_formula
from experiments.exp_170_prose_table.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_170_prose_table.video_summarizer import summarize_video
from experiments.exp_170_prose_table.prefix_cache import (
...
def _run_advisors(question: str, tid: str, run_formula: bool = True) -> tuple[str, object]:
    ...
    runner: with EXP170_MATH_OTHER_ONLY the formula fires only when domain='other'.
    """
...
def run_one(tid: str, out_dir: Path) -> dict:
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_170_prose_table.keyframe_cache import ensure_task_keyframes
    ...
    for i in range(N_ATTEMPTS):
        try:
            ...
            if flags.prose_extract_on():
                import os as _os2
                from experiments.exp_170_prose_table.prose_extractor import extract_prose_tables, select_prose_docs
                _root = _os2.environ.setdefault("EXP170_EXTRACTED_ROOT", str(out_dir / "_prose_extracted"))
                if not (Path(_root) / tid / "_manifest.json").is_file():
                    _sm = make_model(temp=0.0, task_id=tid)
                    _df = _os2.environ.get("EXP170_PROSE_DOC_FILTER")  # optional manual override
...
def main():
    ...
    i = 1
    while (runs / f"exp_170_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_170_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP170_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP170_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp166_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP170_KEYFRAME_ROOT"):
        os.environ["EXP170_KEYFRAME_ROOT"] = str(
    ...
    print(f"=== exp_170 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP170_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP170_KEYFRAME_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP170_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp166"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP170_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP170_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP170_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP170_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP170_KEYFRAME_MAX_IMAGES", "50")},
        "results": results,
