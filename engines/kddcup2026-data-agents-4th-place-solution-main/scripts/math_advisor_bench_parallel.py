"""exp_137 math advisor bench using SUBMISSION-IDENTICAL pipeline.

Differences from math_advisor_full50.py:
- Delegates to `experiments.exp_137_math_advisor.runner._run_single_task_with_timeout`
...
- Math advisor formula injection happens inside each subprocess via
  exp_137_math_advisor/preamble.py (= ad-hoc bench-level injection removed).
...
# Use exp_137 module's runner and config (= same as submission/main.py)
from experiments.exp_137_math_advisor.runner import _run_single_task_with_timeout
from experiments.exp_137_math_advisor.config import load_app_config, AppConfig

OPTS = EvaluationOptions()
N_ATTEMPTS_NOTE = "(= configured in exp_137.runner._ATTEMPT_TEMPS = (0.6, 0.6, 0.6))"
MAX_WORKERS = 4
...
def _build_app_config() -> AppConfig:
    ...
    config_yaml = REPO / "src" / "experiments" / "exp_137_math_advisor" / "config.yaml"
...
def main():
    ...
    _log(f"=== exp_137 math advisor bench (submission-style: 4 tasks parallel × 3 attempts spawn, max_steps={config.agent.max_steps}) ===")
