...
Each lever is toggled by an env var (see experiments.exp_173_groupby_dim.flags):
  EXP173_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP173_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP173_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP173_KEYFRAMES=1     → attach plateau-selected keyframes as images (no raw video)
  EXP173_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP173_VISUAL_ATTACH=0 → keep keyframe text note but attach no images/video
  EXP173_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool

ALL flags off ⇒ identical pipeline to exp_143_dynamic_doc (sanity baseline).

Run dir is named by the active levers, e.g.:
  EXP173_ANSWER_SHAPE=1 python scripts/phase2_exp166_ablation.py   → exp_167_answer_shape_001
  EXP173_PROSE=1        python scripts/phase2_exp166_ablation.py   → exp_167_prose_001
  EXP173_KEYFRAMES=1    python scripts/phase2_exp166_ablation.py   → exp_167_keyframes_001
  EXP173_WATCH_VIDEO=1  python scripts/phase2_exp166_ablation.py   → exp_167_watch_video_001
...
from experiments.exp_173_groupby_dim.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_173_groupby_dim.preamble import build_preamble
from experiments.exp_173_groupby_dim.tools.registry import create_default_tool_registry
from experiments.exp_173_groupby_dim.domain_router import classify_domain
from experiments.exp_173_groupby_dim.domain_prompt import domain_note, modality_note
from experiments.exp_173_groupby_dim.adaptive_vote import adaptive_vote
from experiments.exp_173_groupby_dim.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_173_groupby_dim.math_advisor import generate_formula
from experiments.exp_173_groupby_dim.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_173_groupby_dim.video_summarizer import summarize_video
from experiments.exp_173_groupby_dim.prefix_cache import (
    prefix_cache_enabled,
    with_prefix_cache_header,
)

# DATA_ROOT defaults to the Phase 2 demo set; override via EXP173_DATA_ROOT
# (e.g. data/public for a Phase 1 regression check). Expects <root>/input + <root>/output.
DATA_ROOT = Path(os.environ.get("EXP173_DATA_ROOT") or (REPO / "data" / "phase2_demo" / "demo_samples_phase2"))
...
def _run_advisors(question: str, tid: str, run_formula: bool = True) -> tuple[str, object]:
    ...
    runner: with EXP173_MATH_OTHER_ONLY the formula fires only when domain='other'.
    """
...
def run_one(tid: str, out_dir: Path) -> dict:
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_173_groupby_dim.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_173_groupby_dim.keyframe_cache import ensure_task_keyframes
    ...
    if flags.domain_router_on():
        try:
            ...
            if flags.icl_fewshot_on():
                from experiments.exp_173_groupby_dim.icl_fewshot import fewshot_note
                icl_note_text = fewshot_note(task.question, dr.domain)
        except Exception:
            domain_for_audit = "other"
    ...
    for i in range(N_ATTEMPTS):
        try:
            ...
            if flags.prose_extract_on():
                import os as _os2
                from experiments.exp_173_groupby_dim.prose_extractor import extract_prose_tables, select_prose_docs, prose_gate
                _root = _os2.environ.setdefault("EXP173_EXTRACTED_ROOT", str(out_dir / "_prose_extracted"))
                if not (Path(_root) / tid / "_manifest.json").is_file():
                    _sm = make_model(temp=0.0, task_id=tid)
                    _df = _os2.environ.get("EXP173_PROSE_DOC_FILTER")  # optional manual override
            ...
            if modality_note_text or domain_note_text or icl_note_text:
                preamble_text = modality_note_text + domain_note_text + icl_note_text + preamble_text
            if anti_agg.no_agg:
...
def main():
    ...
    i = 1
    while (runs / f"exp_171_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_171_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    # self-describe the task set so trace_viewer_v3 resolves this run's gold/questions
    # against the RIGHT root (e.g. data/external/eval_set_v1), not the default Phase-2 set.
    if os.environ.get("EXP173_DATA_ROOT"):
        try:
            (out_dir / "_task_root").write_text(os.environ["EXP173_DATA_ROOT"].strip(), encoding="utf-8")
        except Exception:
            pass

    if flags.pdf_preprocess_on() and not os.environ.get("EXP173_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP173_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp166_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP173_KEYFRAME_ROOT"):
        os.environ["EXP173_KEYFRAME_ROOT"] = str(
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP173_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP173_KEYFRAME_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP173_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp166"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP173_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP173_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP173_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP173_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP173_KEYFRAME_MAX_IMAGES", "50")},
        "results": results,
