"""Phase 2 exp_162 full tool descriptions ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_162_full_tool_descriptions.flags):
  EXP162_ANSWER_SHAPE=1  → ① DISTINCT explicit-only + NULL/BLANK preserve
  EXP162_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP162_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP162_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP162_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP162_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool
  EXP162_SOFT_FINAL_SQL_GUARD=1 → reject risky DISTINCT / NULL filters;
                                  never auto-rewrite SQL
  EXP162_AUDIO_ASR=1     → transcribe task video audio as a cautious preamble/router hint

exp162 = exp161 + full tool descriptions in every phase. Runtime phase gating
is still enforced, but the system prompt sees the full tool list as in exp153.
The failed exp152-style auto-rewrite final SQL guard remains off; this
experiment only rejects risky final SQL and asks the model to rewrite.

Run dir is named by the active levers, e.g.:
  EXP162_ANSWER_SHAPE=1 python scripts/phase2_exp162_ablation.py   -> exp_162_answer_shape_001
  EXP162_PROSE=1        python scripts/phase2_exp162_ablation.py   -> exp_162_prose_001
  EXP162_KEYFRAMES=1    python scripts/phase2_exp162_ablation.py   -> exp_162_keyframes_001
  EXP162_WATCH_VIDEO=1  python scripts/phase2_exp162_ablation.py   -> exp_162_watch_video_001
  (no flags)            python scripts/phase2_exp162_ablation.py   -> exp_162_...soft_final_sql_guard_001
...
from experiments.exp_162_full_tool_descriptions.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_162_full_tool_descriptions.preamble import build_preamble
from experiments.exp_162_full_tool_descriptions.tools.registry import create_default_tool_registry
from experiments.exp_162_full_tool_descriptions.adaptive_vote import adaptive_vote
from experiments.exp_162_full_tool_descriptions.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_162_full_tool_descriptions.math_advisor import generate_formula
from experiments.exp_162_full_tool_descriptions.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_162_full_tool_descriptions.video_summarizer import summarize_video
from experiments.exp_162_full_tool_descriptions.prefix_cache import (
    prefix_cache_enabled,
    with_prefix_cache_header,
)
from experiments.exp_162_full_tool_descriptions.audio_asr import (
...
def run_one(
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_162_full_tool_descriptions.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_162_full_tool_descriptions.keyframe_cache import ensure_task_keyframes
    ...
    for i in range(N_ATTEMPTS):
        try:
            ...
            for j, s in enumerate(r.steps):
                observation = s.observation if isinstance(s.observation, dict) else {"raw": s.observation}
                full_steps.append({
                    "i": j,
                    ...
                    "observation_ok": observation.get("ok"),
                    "ok": s.ok,
                })
    ...
    compact_traces = [
        {
            **{k: v for k, v in attempt.items() if k not in {"steps", "compact_steps"}},
            "steps": attempt.get("compact_steps", []),
        }
        for attempt in attempt_traces
    ]
    (task_dir / "trace.compact.json").write_text(
        json.dumps(compact_traces, ensure_ascii=False, indent=2, default=str)
    )
...
def main():
    ...
    i = 1
    while (runs / f"exp_162_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_162_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP162_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP162_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp162_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP162_KEYFRAME_ROOT"):
        os.environ["EXP162_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp162_keyframe_cache" / out_dir.name
        )
    if flags.audio_asr_on() and not os.environ.get("EXP162_AUDIO_ASR_CACHE_ROOT"):
        os.environ["EXP162_AUDIO_ASR_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp162_audio_asr_cache" / out_dir.name
        )
    ...
    print(f"=== exp_162 ABLATION levers={levers or ['baseline']} -> {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP162_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP162_KEYFRAME_ROOT')}", flush=True)
    if flags.audio_asr_on():
        print(f"  audio_asr_cache_root={os.environ.get('EXP162_AUDIO_ASR_CACHE_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP162_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp162"),
                   "source_router": flags.source_router_on(),
                   "anti_agg": flags.anti_agg_on(),
                   "anti_agg_sql_guard": flags.anti_agg_sql_guard_on(),
                   "soft_final_sql_guard": flags.soft_final_sql_guard_on(),
                   "final_sql_guard": flags.final_sql_guard_on(),
                   "video_keyframe_note": flags.video_keyframe_note_on(),
                   "video_keyframe_note_max_chars": os.environ.get("EXP162_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP162_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP162_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP162_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP162_KEYFRAME_MAX_IMAGES", "18"),
                   "audio_asr": flags.audio_asr_on(),
                   "audio_asr_cache_root": os.environ.get("EXP162_AUDIO_ASR_CACHE_ROOT"),
