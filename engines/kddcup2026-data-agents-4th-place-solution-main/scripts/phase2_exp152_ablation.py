"""Phase 2 exp_152 final-SQL-guard ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_152_final_sql_guard.flags):
  EXP152_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP152_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP152_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP152_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP152_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP152_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool
  EXP152_FINAL_SQL_GUARD=1 → classify and rewrite unnecessary DISTINCT / NULL filters

exp152 forks exp150 and adds Final SQL Guard on top of the projection-pruner
pipeline.

Run dir is named by the active levers, e.g.:
  EXP152_ANSWER_SHAPE=1 python scripts/phase2_exp152_ablation.py   → exp_152_answer_shape_001
  EXP152_PROSE=1        python scripts/phase2_exp152_ablation.py   → exp_152_prose_001
  EXP152_KEYFRAMES=1    python scripts/phase2_exp152_ablation.py   → exp_152_keyframes_001
  EXP152_WATCH_VIDEO=1  python scripts/phase2_exp152_ablation.py   → exp_152_watch_video_001
  (no flags)            python scripts/phase2_exp152_ablation.py   → exp_152_baseline_001
...
from experiments.exp_152_final_sql_guard.phased_agent import PhasedReActAgent, PhasedAgentConfig
...
from experiments.exp_152_final_sql_guard.anti_aggregation import no_agg_note, review_aggregation
...
from experiments.exp_152_final_sql_guard.video_summarizer import summarize_video
...
def main():
    ...
    i = 1
    while (runs / f"exp_152_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_152_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP152_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP152_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp152_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP152_KEYFRAME_ROOT"):
        os.environ["EXP152_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp152_keyframe_cache" / out_dir.name
        )
    ...
    print(f"=== exp_152 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP152_PDF_TEXT_CACHE_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP152_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp152"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP152_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP152_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP152_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP152_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP152_KEYFRAME_MAX_IMAGES", "18")},
        "results": results,
