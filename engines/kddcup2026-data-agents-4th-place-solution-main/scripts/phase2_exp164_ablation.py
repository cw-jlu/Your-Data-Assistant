"""Phase 2 exp_164 per-lever ABLATION bench.

Each lever is toggled by an env var (see experiments.exp_164_exp149_replay.flags):
  EXP164_ANSWER_SHAPE=1  → ① DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP164_PROSE=1         → ② pypdf read + prose_docs inventory + 404→doc redirect + protocol
  EXP164_VIDEO=1         → ③ pre-extract video summary into preamble (no raw video in loop)
  EXP164_KEYFRAMES=1     → attach extracted unique video keyframes as images (no raw video)
  EXP164_VIDEO_KEYFRAME_NOTE=1 → extract visible keyframe information into preamble/router
  EXP164_WATCH_VIDEO=1   → expose raw-video inspection as an EXPLORE tool

ALL flags off ⇒ identical pipeline to exp_143_dynamic_doc (sanity baseline).

Run dir is named by the active levers, e.g.:
  EXP164_ANSWER_SHAPE=1 python scripts/phase2_exp164_ablation.py   → exp_164_answer_shape_001
  EXP164_PROSE=1        python scripts/phase2_exp164_ablation.py   → exp_164_prose_001
  EXP164_KEYFRAMES=1    python scripts/phase2_exp164_ablation.py   → exp_164_keyframes_001
  EXP164_WATCH_VIDEO=1  python scripts/phase2_exp164_ablation.py   → exp_164_watch_video_001
  (no flags)            python scripts/phase2_exp164_ablation.py   → exp_164_baseline_001
...
from experiments.exp_164_exp149_replay.phased_agent import PhasedReActAgent, PhasedAgentConfig
from experiments.exp_164_exp149_replay.preamble import build_preamble
from experiments.exp_164_exp149_replay.tools.registry import create_default_tool_registry
from experiments.exp_164_exp149_replay.adaptive_vote import adaptive_vote
from experiments.exp_164_exp149_replay.anti_aggregation import no_agg_note, review_aggregation
from experiments.exp_164_exp149_replay.math_advisor import generate_formula
from experiments.exp_164_exp149_replay.video_keyframe_note import (
    extract_keyframe_note,
    preamble_block as video_keyframe_preamble_block,
)
from experiments.exp_164_exp149_replay.video_summarizer import summarize_video
from experiments.exp_164_exp149_replay.prefix_cache import (
...
def run_one(tid: str, out_dir: Path) -> dict:
    ...
    if flags.pdf_preprocess_on():
        try:
            from experiments.exp_164_exp149_replay.pdf_text_cache import ensure_task_pdf_cache
    ...
    if flags.keyframes_on():
        try:
            from experiments.exp_164_exp149_replay.keyframe_cache import ensure_task_keyframes
...
def main():
    ...
    i = 1
    while (runs / f"exp_164_{tag}_{i:03d}").exists():
        i += 1
    out_dir = runs / f"exp_164_{tag}_{i:03d}"
    out_dir.mkdir(parents=True, exist_ok=True)

    if flags.pdf_preprocess_on() and not os.environ.get("EXP164_PDF_TEXT_CACHE_ROOT"):
        os.environ["EXP164_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp164_pdf_text_cache" / out_dir.name
        )
    if (flags.keyframes_on() or flags.video_keyframe_note_on()) and not os.environ.get("EXP164_KEYFRAME_ROOT"):
        os.environ["EXP164_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp164_keyframe_cache" / out_dir.name
        )
    ...
    print(f"=== exp_164 ABLATION levers={levers or ['baseline']} → {out_dir.name} ===")
    ...
    if flags.pdf_preprocess_on():
        print(f"  pdf_text_cache_root={os.environ.get('EXP164_PDF_TEXT_CACHE_ROOT')}", flush=True)
    if flags.keyframes_on() or flags.video_keyframe_note_on():
        print(f"  keyframe_root={os.environ.get('EXP164_KEYFRAME_ROOT')}", flush=True)
    ...
    (out_dir / "summary.json").write_text(json.dumps({
        ...
        "config": {"n_attempts": N_ATTEMPTS, "max_workers": a.workers,
                   ...
                   "prefix_cache_namespace": os.environ.get("EXP164_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp164"),
                   ...
                   "video_keyframe_note_max_chars": os.environ.get("EXP164_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"),
                   "watch_video": flags.watch_video_on(),
                   "source_router_model": os.environ.get("EXP164_SOURCE_ROUTER_MODEL", "1"),
                   "pdf_text_cache_root": os.environ.get("EXP164_PDF_TEXT_CACHE_ROOT"),
                   "keyframe_root": os.environ.get("EXP164_KEYFRAME_ROOT"),
                   "keyframe_max_images": os.environ.get("EXP164_KEYFRAME_MAX_IMAGES", "18")},
        "results": results,
