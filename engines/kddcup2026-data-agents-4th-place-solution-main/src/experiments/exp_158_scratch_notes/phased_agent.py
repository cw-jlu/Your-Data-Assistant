...
from experiments.exp_158_scratch_notes.agent import (
    parse_model_step,
)
from experiments.exp_158_scratch_notes.prompt import (
    allowed_tools_for_phase,
    build_observation_prompt,
    build_phased_system_prompt,
    build_task_prompt,
)
from experiments.exp_158_scratch_notes.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_158_scratch_notes.tools.registry import ToolRegistry
from experiments.exp_158_scratch_notes import flags
from experiments.exp_158_scratch_notes.source_router import route_answer_source
...
# exp_158 lever ② (prose_on): detect a DuckDB catalog-404 so we can redirect
        ...
        if current == "explore" and next_phase == "answer":
            ...
            if watch_video_required and not watched_video_ok:
                return False, current, (
                    ...
                    "EXP158_WATCH_VIDEO=1, but no successful watch_video call has "
...
class PhasedReActAgent:
    ...
    def _find_keyframe_paths(task: PublicTask) -> list:
        ...
        root = _P(os.environ.get("EXP158_KEYFRAME_ROOT", "/tmp/kobushi_exp158_keyframe_cache"))
        ...
        try:
            max_images = max(1, int(os.environ.get("EXP158_KEYFRAME_MAX_IMAGES", "18")))
        except ValueError:
            max_images = 18
    ...
    def _build_messages(
        ...
        if flags.keyframes_on():
            ...
            # exp_158 lever ③ (video_on): the raw video is NOT attached — a
    ...
    def run(self, task: PublicTask) -> AgentRunResult:
        ...
        for step_index in range(1, self.config.max_steps + 1):
            ...
            try:
                ...
                # exp_158 lever ② (prose_on): a catalog-404 on a knowledge-named
