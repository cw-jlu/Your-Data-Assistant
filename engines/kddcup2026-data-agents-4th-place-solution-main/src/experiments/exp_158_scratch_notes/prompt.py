...
Available tool descriptions for this phase:
{tool_descriptions}
""".strip()


_DOMAIN_GUIDANCE = {
    "fund": """DOMAIN GUIDANCE (fund)
Use this only to resolve ambiguous schema, source, unit, and row-grain choices.
Observed task data and the question always take priority.

Distinguish fund products, fund companies/advisors, fund managers, and trustees.
Match metric words exactly: return, annualized return, benchmark growth, scale,
risk, tenure, and horizon/window. Preserve ranking/source order, duplicate base
rows, blanks, and raw precision unless the question asks otherwise.""",
    "stock": """DOMAIN GUIDANCE (stock)
Use this only to resolve ambiguous schema, source, unit, and row-grain choices.
Observed task data and the question always take priority.

Distinguish company names, stock abbreviations, stock codes, shareholders, and
corporate-action records. Preserve display names, whitespace, ratio scale, and
Chinese units. Do not deduplicate shareholder/company/action rows unless
uniqueness is explicitly requested.""",
    "macro": """DOMAIN GUIDANCE (macro)
Use this only to resolve ambiguous schema, source, unit, and row-grain choices.
Observed task data and the question always take priority.

Use the source family named by the question, such as monetary authority, other
depository corporations, GDP, RMB credit, retail, or social financing. Do not
switch to a similarly named table only because a column exists. For retrieve/list
questions, preserve source order, blanks/nulls, raw units, and precision.""",
    "ehr": """DOMAIN GUIDANCE (ehr)
Use this only to resolve ambiguous schema, source, unit, and row-grain choices.
Observed task data and the question always take priority.

Resolve patient, admission, visit, or stay identifiers before querying events.
Keep diagnoses, labs, procedures, and treatments within the requested encounter.
For first/last/current/min/max questions, use the relevant time field and a
deterministic tie-break. Match clinical item names exactly.""",
    "other": """DOMAIN GUIDANCE (other)
No domain-specific assumptions. Follow the question, schema, and observed rows.""",
}


def domain_guidance(domain: str | None) -> str:
    from experiments.exp_158_scratch_notes import flags

    if not flags.domain_prompt_on():
        return ""
    key = (domain or "other").strip().lower()
    return _DOMAIN_GUIDANCE.get(key, _DOMAIN_GUIDANCE["other"])
...
def allowed_tools_for_phase(phase: str) -> tuple[str, ...]:
    ...
    if flags.scratch_notes_on():
        if phase == "explore":
            for name in ("read_notes", "append_note"):
                if name not in allowed:
                    allowed.insert(max(len(allowed) - 1, 0), name)
        elif phase in {"answer", "verify"} and "read_notes" not in allowed:
            allowed.insert(0, "read_notes")
    return tuple(allowed)


# ====================================================================
# exp_158 LEVER ① — answer_shape_guard (DISTINCT → JOIN-fanout-only +
...
=== exp_158 ANSWER-SHAPE OVERRIDE (supersedes the DISTINCT rule above) ===
...
=== exp_158 ANSWER-SHAPE OVERRIDE (supersedes the DUPLICATE SANITY step above) ===
...
# ====================================================================
# exp_158 LEVER ② — prose_doc_forced_read, H4a parse protocol. Appended to
...
- Before `answer_from_sql`, check each output column's unit from the question,
  column name, and source value text.
- Do not silently drop a visible unit suffix (%, currency, share/unit words)
  while keeping the same numeric magnitude in a bare numeric cell.
- If the source says `X%` but the final cell has no percent unit, use the
  corresponding fraction unless the question or output column explicitly asks
  for percent-points.
- Preserve raw numeric precision unless the question explicitly asks to round."""


_SCRATCH_NOTE_EXPLORE = """
SCRATCH NOTES:
When prose/PDF/doc evidence contains multiple candidate records, write observed
facts with `append_note` as soon as you read them. Include the source line/range
in `source_ref`. Notes are memory only: record observed fields, not guesses."""


_SCRATCH_NOTE_ANSWER = """
SCRATCH NOTES:
Before hand-building SQL `VALUES` from prose/PDF/doc evidence, call `read_notes`
and use only notes with source_ref. If notes are incomplete, return to EXPLORE."""


_SCRATCH_NOTE_VERIFY = """
SCRATCH NOTES:
If the final SQL uses prose/PDF/doc-derived literals, call `read_notes` and
check the row candidates/order against the notes before confirming."""
...
def build_phased_system_prompt(
    ...
    exp_158 ablation: when EXP158_ANSWER_SHAPE / EXP158_PROSE are on, the
    ...
    if flags.watch_video_on() and phase == "explore":
        instr = instr + "\n" + _EXPLORE_WATCH_VIDEO_PROTOCOL
    if flags.unit_prompt_on() and phase == "answer":
        instr = instr + "\n" + _ANSWER_UNIT_PROMPT
    if flags.scratch_notes_on():
        if phase == "explore":
            instr = instr + "\n" + _SCRATCH_NOTE_EXPLORE
