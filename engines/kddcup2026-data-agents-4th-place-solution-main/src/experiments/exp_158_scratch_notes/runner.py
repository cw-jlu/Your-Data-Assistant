...
from experiments.exp_158_scratch_notes.agent import ReActAgent, ReActAgentConfig
from experiments.exp_158_scratch_notes.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_158_scratch_notes.adaptive_vote import adaptive_vote
from experiments.exp_158_scratch_notes.config import AppConfig
from experiments.exp_158_scratch_notes import flags
from experiments.exp_158_scratch_notes.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_158_scratch_notes.prefix_cache import with_prefix_cache_header
from experiments.exp_158_scratch_notes.tools.registry import (
    ToolRegistry,
    create_default_tool_registry,
)
from experiments.exp_158_scratch_notes.audio_asr import audio_preamble_block
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP158_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP158_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp158_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP158_KEYFRAME_ROOT"):
        _os.environ["EXP158_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp158_keyframe_cache" / run_name
        )


def _use_precomputed_preprocess() -> bool:
    raw = _os.environ.get("EXP158_USE_PRECOMPUTED_PREPROCESS", "")
...
def _precomputed_task_dir(task_id: str) -> Path | None:
    if not _use_precomputed_preprocess():
        return None
    root = _os.environ.get("EXP158_PREPROCESS_ROOT")
    if not root:
        return None
...
def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp158 bench script."""
...
def _run_single_task_core(
    *,
    ...
    if flags.domain_prompt_on():
        domain_model = build_model_adapter(
            _config_with_temperature(config, 0.0),
            task_id=f"{task_id}:domain",
        )
        domain_route = classify_domain(task, domain_model)
    ...
    if flags.audio_asr_on():
        ...
        if asr_text:
            preamble_text = audio_preamble_block(
                task.question,
                asr_text,
                max_chars=_env_int("EXP158_ASR_MAX_CHARS", 2600),
            ) + preamble_text
    ...
    if tools is None:
        ...
        # v4/exp158 = v1 (exp149) base + audio_asr: no projection pruner and no
    ...
    if asr_text and _env_flag("EXP158_AUDIO_TO_ROUTER", True):
        router_video_note = (
            (router_video_note or "")
            + "\n\n# VIDEO AUDIO TRANSCRIPT (ASR; may contain errors)\n"
            + asr_text[: _env_int("EXP158_ROUTER_ASR_MAX_CHARS", 1600)]
        ).strip()
    ...
    trace_root = _os.environ.get("EXP158_TRACE_ROOT")
    if trace_root:
    ...
    payload["exp158_runtime_metadata"] = {
        ...
        "domain": domain_route.domain if domain_route else None,
        "domain_route": domain_route.to_dict() if domain_route else None,
