...
from experiments.exp_158_scratch_notes.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_158_scratch_notes.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _safe_note_part(value: str) -> str:
    safe = "".join(ch if ch.isalnum() or ch in "._-" else "_" for ch in value)
    return safe[:100] or "unknown"


def _notes_file(task: PublicTask) -> Path:
    root = Path(os.environ.get("EXP158_NOTES_ROOT", "/tmp/kobushi_exp158_notes"))
    task_id = _safe_note_part(getattr(task, "task_id", "task"))
    return root / task_id / "notes.jsonl"


def _append_note(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    table = str(action_input.get("table", "")).strip()
    source_ref = str(action_input.get("source_ref", "")).strip()
    fields = action_input.get("fields")
    if not table:
        return ToolExecutionResult(ok=False, content={"error": "table is required"})
    if not source_ref:
        return ToolExecutionResult(ok=False, content={"error": "source_ref is required"})
    if not isinstance(fields, dict) or not fields:
        return ToolExecutionResult(ok=False, content={"error": "fields must be a non-empty object"})
    note_text = str(action_input.get("note", "")).strip()
    record = {
        "table": table[:120],
    ...
    if note_text:
        record["note"] = note_text[:2000]
    line = json.dumps(record, ensure_ascii=False, sort_keys=True)
    if len(line) > 20_000:
        return ToolExecutionResult(ok=False, content={"error": "note too large", "max_chars": 20000})
    path = _notes_file(task)
...
def _read_notes(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    table = str(action_input.get("table", "")).strip()
    max_rows = min(max(int(action_input.get("max_rows", 200)), 1), 500)
    max_chars = min(max(int(action_input.get("max_chars", 12000)), 1000), 40000)
    path = _notes_file(task)
    if not path.exists():
        return ToolExecutionResult(
            ok=True,
            content={"path": str(path), "row_count": 0, "notes": [], "truncated": False},
        )
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            continue
        if table and rec.get("table") != table:
            continue
        all_notes.append(rec)

    notes = all_notes[:max_rows]
    content = {
        "path": str(path),
        "table_filter": table or None,
        "row_count": len(all_notes),
        "returned_count": len(notes),
        "truncated": len(all_notes) > len(notes),
        "notes": notes,
    }
    while len(json.dumps(content, ensure_ascii=False)) > max_chars and content["notes"]:
        content["notes"] = content["notes"][:-1]
        content["returned_count"] = len(content["notes"])
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP158_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_158_scratch_notes.video_summarizer import find_video
...
def _answer_from_sql(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    if not rows:
        return ToolExecutionResult(ok=False, content={"error": "SQL returned 0 rows.", "sql": sql})
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_158_scratch_notes.column_auditor import audit_columns, apply_audit
...
def create_default_tool_registry(
    ...
    *,
    ...
    specs.update({
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP158_PROSE=1). Inspired by Claude "
    ...
    if flags.scratch_notes_on():
        specs["append_note"] = ToolSpec(
            name="append_note",
            description=(
                "EXPLORE-only scratch memory. Append observed prose/PDF/doc facts to a "
                "task-local JSONL note file. Use only for facts visible in a tool "
                "observation, with source_ref line/range; do not infer or solve here."
            ),
            input_schema={
                "table": "short logical note group, e.g. candidates",
                "source_ref": "file:line-range or other exact observation reference",
                "fields": {"field_name": "observed value"},
                "note": "optional short note",
            },
        )
        specs["read_notes"] = ToolSpec(
            name="read_notes",
            description=(
                "Read task-local scratch notes previously written with append_note. "
                "Use before constructing SQL VALUES from prose/PDF/doc-derived facts."
            ),
            input_schema={
                "table": "optional exact note group",
                "max_rows": 200,
                "max_chars": 12000,
            },
        )
    ...
    if flags.scratch_notes_on():
        handlers["append_note"] = _append_note
        handlers["read_notes"] = _read_notes
    if doc_only:
