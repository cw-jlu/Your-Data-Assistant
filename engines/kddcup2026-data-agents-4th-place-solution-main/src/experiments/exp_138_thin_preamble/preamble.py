...
from experiments.exp_138_thin_preamble.profile import (
    csv_profile_text,
    json_array_profile,
...
def _csv_full(path: Path) -> str:
    """Return [CSV] section: profile (per-col stats + 5-row sample) ONLY.

    Thin-preamble variant (exp_138): raw content dropped. Profile already includes
    column stats + a 5-row sample which is enough for agent SQL writing.
    Agent can `execute_sql SELECT * FROM <table> LIMIT N` for more rows on demand.
    """
    rel = path.name
    profile = csv_profile_text(path)
    if profile:
        return f"## [CSV] {rel}\n[profile]\n{profile}"
    # Fallback if profile failed (e.g. unreadable CSV): emit small clipped raw.
    try:
...
def _sqlite_full(path: Path) -> str:
    """Return [SQLite] section: per-table profile + 5-row sample. DDL dropped
    (= column names/types are in profile + DuckDB views section)."""
    import pandas as pd
...
def _json_full(path: Path) -> str:
    ...
    # Thin-preamble (exp_138): always emit profile (+ small sample), never full raw.
    try:
        obj = json.loads(raw)
    except Exception:
        clipped, _ = _truncate_chars(raw, 3000)
        return f"## [JSON] {rel}\n[parse error — clipped]\n{clipped}"

    # Case A: top-level list of dicts → profile only.
    if isinstance(obj, list) and obj and isinstance(obj[0], dict):
        profile = json_array_profile(obj)
        if profile:
            return f"## [JSON] {rel}\n[profile]\n{profile}"

    # Case B: wrapper object { "table": ..., "records": [...] } → profile the records[].
    if isinstance(obj, dict):
        ...
        if records_key:
            ...
            if recs and isinstance(recs[0], dict):
                profile = json_array_profile(recs)
                if profile:
                    shell = {k: v for k, v in obj.items() if k != records_key}
                    shell_str = (json.dumps(shell, ensure_ascii=False, indent=2, default=str)
                                 if shell else "")
                    head = f"total_{records_key}={len(recs)}\n"
                    return f"## [JSON] {rel}\n{head}{shell_str}\n[profile]\n{profile}"

    # Fallback: pretty-print head (= small clipped raw for non-tabular JSON).
    if isinstance(obj, list):
        ...
        clipped, _ = _truncate_chars(pretty, 3000)
        body = clipped
    return f"## [JSON] {rel}\n{body}"


def _doc_full(path: Path) -> str:
    """Thin-preamble (exp_138): return [DOC] section OUTLINE only.
    Structure: section tree from `### / ####` headers + line ranges.
    Agent uses `read_doc(path, offset=L_start, max_chars=N)` to fetch any section
    on demand. No domain-specific extraction (= no leak of test data).
    """
    rel = path.name
    try:
    ...
    lines = text.split("\n")
    n_lines = len(lines)
    # Parse #+ headers and compute their line ranges
    headers: list[dict] = []
    ...
    for idx, h in enumerate(headers):
        ...
        h["line_end"] = end_line
    if not headers:
        # No headers → just show line count, agent can scan via read_doc
        return f"## [DOC] {rel}\n[outline] (no section headers, {n_lines} lines total)"
    lines_out = [f"## [DOC] {rel}", f"[outline] ({n_lines} lines, {len(headers)} sections)"]
    for h in headers:
        indent = "  " * max(0, h["level"] - 2)
        title = h["title"][:80]
        lines_out.append(f"{indent}L{h['line_start']:>4}-{h['line_end']:>4}  {title}")
    lines_out.append(
        "Use `read_doc(path, offset=L_start, max_chars=N)` to fetch any section."
    )
    return "\n".join(lines_out)
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_138_thin_preamble.tools.duckdb_unified import describe_catalog
    ...
    # MATH ADVISOR INJECTION (= exp_137): prepend formula hint when CALC.
    try:
        from experiments.exp_138_thin_preamble.math_advisor import generate_formula
        ...
        if formula and not formula.strip().upper().startswith("NO_CALC"):
            ...
            full_text = advisor_block + full_text
    except Exception:
        pass  # advisor failure → skip injection, keep raw preamble
