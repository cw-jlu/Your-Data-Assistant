...
def _get_file_label(path: Path) -> str:
    """Return a short file-type label for the workspace overview listing."""
...
def build_preamble(
    ...
    *,
    ...
    overview = "\n".join(
        f"- {_get_file_label(p):<10} {p.relative_to(task.context_dir)} ({p.stat().st_size} bytes)"
        for p in files
    )
