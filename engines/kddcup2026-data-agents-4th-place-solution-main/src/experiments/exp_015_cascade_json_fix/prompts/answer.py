...
3. Use DuckDB's native functions for multi-source access:
   ...
   - JSON (配列ルート):           read_json_auto('path.json')
   - JSON ({table, records}形式): WITH rows AS (SELECT unnest(records, recursive:=true) FROM read_json_auto('path.json')) SELECT ... FROM rows
   - JSON (単一オブジェクト):     SELECT unnest(records, recursive:=true) FROM read_json_auto('path.json')
