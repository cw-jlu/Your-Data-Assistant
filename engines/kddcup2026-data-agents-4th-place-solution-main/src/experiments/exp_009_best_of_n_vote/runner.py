...
from experiments.exp_009_best_of_n_vote.agent import ReActAgent, ReActAgentConfig
from experiments.exp_009_best_of_n_vote.config import AppConfig
from experiments.exp_009_best_of_n_vote.preamble import build_preamble
from experiments.exp_009_best_of_n_vote.tools.registry import (
    ToolRegistry,
    create_default_tool_registry,
)
from experiments.exp_009_best_of_n_vote.voting import (
    score_candidates,
    select_best_index,
)
...
# Derive experiment prefix from the parent directory name (e.g. "exp_009_best_of_n_vote")
...
def _run_single_task_core(
    *,
    ...
    for _ in range(N_CANDIDATES):
        ...
        candidate_results.append(agent.run(task))

    answers = [c.answer for c in candidate_results]
    scores = score_candidates(answers)
    selected_index = select_best_index(answers)
        ...
        "scores": [
            {
                ...
                "overlap_score": s.score,
                "n_columns": len(s.signatures),
            }
            for s in scores
        ],
    }
    payload["candidates"] = [c.to_dict() for c in candidate_results]
    return payload
