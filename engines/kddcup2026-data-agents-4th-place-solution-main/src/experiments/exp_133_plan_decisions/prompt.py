...
EVERY column choice MUST be written as <table>.<column> (= fully qualified, no
aliases such as `e.cost`). Use real names from the schema preview.

Your `thought` must include these labeled lines:
  - "PLAN: column_count=<n>, per_column=[<short description per column>]."
  - "INTERPRET COLUMN: '<noun>' could be <table>.<colA> or <table>.<colB>;
    CHOOSING <table>.<col> because <rule citation + reason>." (or
    "INTERPRET COLUMN: '<noun>' — unambiguous (<table>.<col>, short reason)"
    if only one plausible column exists).
  - "INTERPRET SEMANTIC: '<phrase>' could mean <option A> (per-row) or
    <option B> (entity-aggregate); CHOOSING <option> because <reason>."
    (REQUIRED for superlative / numeric-relation questions.)
  - "INTERPRET DESCRIPTOR: '<entity>' returned as <table>.<col> = '<value>'
    because <reason>." (REQUIRED for "what is the X / which X / name the X".)

Then end the PLAN with a machine-readable DECISIONS block (REQUIRED):
  DECISIONS:
    col_1: <table>.<column>
    col_2: <table>.<column>                  # only if column_count >= 2
    filter_1: <table>.<column> = <value>
    filter_2: <table>.<column> <op> <value>  # optional
    join_1: <table>.<col> = <table>.<col>    # optional
    join_2: <table>.<col> = <table>.<col>    # optional
...
Workflow:
  ...
  2. When the plan (with DECISIONS block) is committed in `thought`, call
     `complete_phase` with action_input={"next_phase":"explore"} to advance.
""".strip()
