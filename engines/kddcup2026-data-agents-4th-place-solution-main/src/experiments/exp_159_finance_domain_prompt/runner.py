...
from experiments.exp_159_finance_domain_prompt.agent import ReActAgent, ReActAgentConfig
from experiments.exp_159_finance_domain_prompt.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_159_finance_domain_prompt.adaptive_vote import adaptive_vote
from experiments.exp_159_finance_domain_prompt.config import AppConfig
from experiments.exp_159_finance_domain_prompt import flags
from experiments.exp_159_finance_domain_prompt.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_159_finance_domain_prompt.prefix_cache import with_prefix_cache_header
from experiments.exp_159_finance_domain_prompt.tools.registry import (
    ToolRegistry,
    create_default_tool_registry,
)
from experiments.exp_159_finance_domain_prompt.audio_asr import audio_preamble_block
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP159_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP159_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp159_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP159_KEYFRAME_ROOT"):
        _os.environ["EXP159_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp159_keyframe_cache" / run_name
        )


def _use_precomputed_preprocess() -> bool:
    raw = _os.environ.get("EXP159_USE_PRECOMPUTED_PREPROCESS", "")
...
def _precomputed_task_dir(task_id: str) -> Path | None:
    if not _use_precomputed_preprocess():
        return None
    root = _os.environ.get("EXP159_PREPROCESS_ROOT")
    if not root:
        return None
...
def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp159 bench script."""
...
def _run_single_task_core(
    *,
    ...
    if flags.audio_asr_on():
        ...
        if asr_text:
            preamble_text = audio_preamble_block(
                task.question,
                asr_text,
                max_chars=_env_int("EXP159_ASR_MAX_CHARS", 2600),
            ) + preamble_text
    ...
    if tools is None:
        ...
        # v4/exp159 = v1 (exp149) base + audio_asr: no projection pruner and no
    ...
    if asr_text and _env_flag("EXP159_AUDIO_TO_ROUTER", True):
        router_video_note = (
            (router_video_note or "")
            + "\n\n# VIDEO AUDIO TRANSCRIPT (ASR; may contain errors)\n"
            + asr_text[: _env_int("EXP159_ROUTER_ASR_MAX_CHARS", 1600)]
        ).strip()
    ...
    trace_root = _os.environ.get("EXP159_TRACE_ROOT")
    if trace_root:
    ...
    if domain_route is not None:
        _write_json(task_trace_dir / "domain_route.json", domain_route.to_dict())
        if domain_note:
            (task_trace_dir / "domain_prompt.md").write_text(domain_note, encoding="utf-8")
    ...
    payload["exp159_runtime_metadata"] = {
        ...
        "domain_route": domain_route.to_dict() if domain_route is not None else None,
