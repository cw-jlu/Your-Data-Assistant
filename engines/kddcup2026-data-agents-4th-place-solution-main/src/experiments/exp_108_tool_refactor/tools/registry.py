...
from experiments.exp_108_tool_refactor.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_108_tool_refactor.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_108_tool_refactor.tools.sqlite import (
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        # exp_108: removed read_csv / read_json / read_doc — preview content
        # is now in the rich preamble's [profile] + first-5-rows + (small files)
        # full raw text. Steps that previously did "list_context → read_csv →
        # read_csv → read_json → execute_python" now go directly to
        # execute_python after looking at the preamble. read_doc was kept
        # before but knowledge.md is in the preamble Glossary section, so
        # also removed. If a task has a non-knowledge .md the agent can use
        # execute_python with open(...).read() for raw access.
    }
    handlers = {
        "answer": _answer,
