...
from experiments.exp_169_ehr_limit.profile import (
    csv_profile_text,
    json_array_profile,
...
def _doc_full(task: PublicTask, path: Path) -> str:
    ...
    if suffix == ".pdf":
        try:
            from experiments.exp_169_ehr_limit.pdf_text_cache import (
...
def _build_prose_index_section(task: PublicTask) -> str | None:
    ...
    if not flags.prose_index_on():
        return None
    try:
        from experiments.exp_169_ehr_limit.prose_indexer import index_cache_root
    except Exception:
        return None
    ...
    try:
        max_chars = int(os.environ.get("EXP169_PROSE_INDEX_MAX_CHARS", "50000"))
    except Exception:
        max_chars = 50_000
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_169_ehr_limit.tools.duckdb_unified import describe_catalog
