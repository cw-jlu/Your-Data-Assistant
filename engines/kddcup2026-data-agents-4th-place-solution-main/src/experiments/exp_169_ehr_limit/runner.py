...
from experiments.exp_169_ehr_limit.agent import ReActAgent, ReActAgentConfig
from experiments.exp_169_ehr_limit.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_169_ehr_limit.adaptive_vote import adaptive_vote
from experiments.exp_169_ehr_limit.config import AppConfig
from experiments.exp_169_ehr_limit import flags
from experiments.exp_169_ehr_limit.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_169_ehr_limit.prefix_cache import with_prefix_cache_header
from experiments.exp_169_ehr_limit.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP169_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP169_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP169_KEYFRAME_ROOT"):
        _os.environ["EXP169_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    # unknown general DB that may need such calc). Toggle with EXP169_MATH_OTHER_ONLY.
    ...
    trace_root = _os.environ.get("EXP169_TRACE_ROOT")
    if trace_root:
