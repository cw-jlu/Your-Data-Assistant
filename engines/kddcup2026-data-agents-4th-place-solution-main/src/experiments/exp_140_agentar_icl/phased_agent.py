...
from experiments.exp_137_math_advisor.agent import (
    parse_model_step,
)
from experiments.exp_137_math_advisor.prompt import (
    PHASE_NAME_TO_TOOLS,
    build_observation_prompt,
    build_phased_system_prompt,
    build_task_prompt,
)
from experiments.exp_137_math_advisor.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_137_math_advisor.tools.registry import ToolRegistry
...
def _validate_action(
    ...
    # Handle give_up as a virtual action — agent signals dead-end.
    if action == GIVE_UP_ACTION:
        return True, current, None, "give_up"
...
class PhasedReActAgent:
    ...
    def run(self, task: PublicTask) -> AgentRunResult:
        ...
        for step_index in range(1, self.config.max_steps + 1):
            ...
            # Capture thinking trace (vLLM reasoning-parser → adapter stash).
            _thinking = getattr(self.model, "last_reasoning_content", "") or ""
            ...
            try:
                ...
                if _thinking:
                    _log(
                        ...
                        f"THINKING={_thinking.replace(chr(10), ' | ')}"
                    )
                _log(
                    f"[{task.task_id}] phase={phase} step {step_index} "
                    f"action={model_step.action} "
                    f"thought={(model_step.thought or '').replace(chr(10),' | ')}"
                )
                ...
                if kind == "give_up":
                    # Agent voluntarily aborts current attempt.
                    reason = ""
                    if isinstance(model_step.action_input, dict):
                        reason = str(model_step.action_input.get("reason", ""))[:500]
                    _log(f"[{task.task_id}] phase={phase} step {step_index} GIVE_UP reason={reason[:200]}")
                    state.steps.append(
                        StepRecord(
                            step_index=step_index,
                            thought=model_step.thought,
                            ...
                            observation={
                                ...
                                "give_up": True,
                                "reason": reason,
                                "content": f"Agent gave up at {phase}: {reason}",
                            },
                            ok=True,
                        )
                    )
                    state.failure_reason = f"GIVE_UP[{phase}]: {reason}"
                    break
                ...
                _content_str = str(tool_result.content).replace("\n", " | ")
                _log(
                    f"[{task.task_id}] phase={phase} step {step_index} "
