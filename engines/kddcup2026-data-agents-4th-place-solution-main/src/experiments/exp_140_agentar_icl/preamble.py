...
from experiments.exp_137_math_advisor.profile import (
    csv_profile_text,
    json_array_profile,
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_137_math_advisor.tools.duckdb_unified import describe_catalog
    ...
    # CELL-VALUE LINK INJECTION (= exp_140 AgentAR style): dense + BM25 over the
    # task's own cell values. Surfaces entities like 'European Grand Prix' to
    # the correct table.column when sample-row preview misses them.
    try:
        from experiments.exp_140_agentar_icl.value_match import build_value_hints_block
        vh_block = build_value_hints_block(task.question, task.context_dir)
        if vh_block:
            full_text = vh_block + "\n\n" + full_text
    except Exception:
        pass
    # BIRD-ICL INJECTION (= exp_140): top-k similar SQL patterns from BIRD train.
    try:
        from experiments.exp_140_agentar_icl.bird_icl import retrieve_and_format
        icl_block, _ = retrieve_and_format(task.question)
        if icl_block:
            full_text = icl_block + "\n\n" + full_text
    except Exception:
        pass  # retrieval failure → skip injection, keep raw preamble
    # MATH ADVISOR INJECTION (= exp_137): prepend formula hint when CALC.
    try:
        from experiments.exp_137_math_advisor.math_advisor import generate_formula
