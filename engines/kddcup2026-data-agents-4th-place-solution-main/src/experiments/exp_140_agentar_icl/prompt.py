...
Default precedence (= when multiple guides agree, no conflict): literal
column-name match > ICL exact match > schema-semantic guess.
...
DEAD-END ESCAPE: if EXPLORE results clearly contradict your PLAN
interpretation (= the columns you planned don't exist, the values you
expected aren't present, the join keys don't link, or you discover the
question's literal column-name maps to a different table than you planned),
call `give_up` with action_input={"reason": "<one-sentence diagnosis>"}.
This aborts the attempt cleanly; a new agent will retry with a different
direction informed by your reason.
""".strip()
...
PHASE_NAME_TO_TOOLS = {
    # `complete_phase` and `give_up` are virtual actions handled by
    # PhasedReActAgent (= no tool dispatch). `give_up` signals the agent has
    # hit an interpretation dead-end and the framework should restart with a
    # different approach; action_input={"reason": "..."} carries the message.
    "plan":    ("list_context", "complete_phase", "give_up"),
    "explore": ("list_context", "describe_data", "execute_sql", "read_doc",
                "grep", "complete_phase", "give_up"),
    "answer":  ("answer_from_sql", "complete_phase", "give_up"),
    # VERIFY: no give_up — the agent has already committed an answer path,
    # refinement should happen via answer_from_sql rewrite or cross-check.
