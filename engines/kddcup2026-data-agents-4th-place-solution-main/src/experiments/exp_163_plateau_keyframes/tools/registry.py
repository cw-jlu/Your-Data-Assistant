...
from experiments.exp_163_plateau_keyframes.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_163_plateau_keyframes.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP163_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_163_plateau_keyframes.video_summarizer import find_video
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_163_plateau_keyframes.column_auditor import audit_columns, apply_audit
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not rows:
            return ToolExecutionResult(
                ...
                content={
                    "error": (
                        "SQL returned 0 rows — no row satisfied the filter. Re-check your "
                        "WHERE clauses against the data before re-running, and do NOT "
                        "re-submit the identical SQL — change the filter first."
                    ),
        ...
        review_text = (
            ...
            f"           证券代码/基金代码.\n"
            f"  RULE 18: COUNT GRAIN — for 'how many / 多少 / 数目 / 总数 / count'\n"
            f"           questions the count grain follows the QUESTION's wording.\n"
            f"           Default: COUNT(*) over the qualifying rows — keep legitimate\n"
            f"           base-row duplicates; do NOT collapse to one-row-per-entity via\n"
            f"           COUNT(DISTINCT ...), ROW_NUMBER()/RANK() = 1, QUALIFY, or a\n"
            f"           GROUP BY used only to dedup.\n"
            f"           ALLOWED: use a distinct / one-per-entity count when the\n"
            f"           QUESTION explicitly asks for distinct / unique / different\n"
            f"           entities (distinct/unique/去重/唯一/不同/不重复) — then\n"
            f"           COUNT(DISTINCT ...) is correct. The thing being counted —\n"
            f"           its noun, its plural form, or the 'how many / 多少 / 几位'\n"
            f"           phrasing itself — is NOT a distinctness word and does NOT by\n"
            f"           itself require COUNT(DISTINCT); infer a distinct count ONLY\n"
            f"           from an explicit distinct/unique/different/去重/唯一/不同/\n"
            f"           不重复 in the question.\n"
            f"           If the SQL collapses base-table grain but the question did not\n"
            f"           ask for distinctness, mark RULE 18 VIOLATION and rewrite with\n"
            f"           COUNT(*); when unsure, prefer COUNT(*).\n"
            f"           A wide gap between COUNT(*) and COUNT(DISTINCT) is NOT evidence\n"
            f"           for DISTINCT, and neither is wording that merely SOUNDS like it\n"
            f"           wants unique / individual entities. Do NOT downgrade COUNT(*) to\n"
            f"           COUNT(DISTINCT) in VERIFY from a duplicate-count cross-check or\n"
            f"           from how entity-like the phrasing feels — switch ONLY when the\n"
            f"           QUESTION contains an explicit distinctness word\n"
            f"           (distinct / unique / different / 去重 / 唯一 / 不同 / 不重复).\n"
            f"  RULE 19: SOURCE-FAITHFUL LITERALS — a value COPIED from a source\n"
            f"           (video / document / prose) must be reproduced EXACTLY as it\n"
            f"           appears. Do NOT change the literal in any way. The scorer\n"
            f"           compares characters exactly, so a character is only equal to\n"
            f"           itself — visually-similar characters are DIFFERENT values\n"
            f"           (for example ',' is not '、'). Do not normalize, transliterate,\n"
            f"           re-tokenize, or re-join the value with a different separator;\n"
            f"           emit one literal identical to the source. This governs COPIED\n"
            f"           values; results you COMPUTE via SQL follow normal formatting.\n"
            f"\n"
    ...
    specs.update({
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP163_PROSE=1). Inspired by Claude "
