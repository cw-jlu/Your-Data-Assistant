...
from experiments.exp_163_plateau_keyframes.agent import (
    parse_model_step,
)
from experiments.exp_163_plateau_keyframes.prompt import (
    allowed_tools_for_phase,
    build_observation_prompt,
    build_phased_system_prompt,
    build_task_prompt,
)
from experiments.exp_163_plateau_keyframes.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_163_plateau_keyframes.tools.registry import ToolRegistry
from experiments.exp_163_plateau_keyframes import flags
from experiments.exp_163_plateau_keyframes.source_router import route_answer_source
...
# exp_163 lever ② (prose_on): detect a DuckDB catalog-404 so we can redirect
...
def _validate_action(
    ...
    *, watch_video_required: bool = False, watched_video_ok: bool = False,
    ...
    if action == TRANSITION_ACTION:
        ...
        if current == "explore" and next_phase == "answer":
            ...
            if watch_video_required and not watched_video_ok:
                return False, current, (
                    ...
                    "EXP163_WATCH_VIDEO=1, but no successful watch_video call has "
        ...
        # ANSWER → VERIFY is reached ONLY by a SUCCESSFUL answer_from_sql (which
        # auto-advances to VERIFY). Block manual complete_phase out of ANSWER so
        # the agent cannot escape into VERIFY without a valid answer and then
        # thrash there (answer_from_sql keeps failing, confirm_answer has nothing
        # to commit). With no valid answer, stay in ANSWER and call answer_from_sql
        # (a failure bounces back to EXPLORE to diagnose).
        if current == "answer":
            return False, current, (
                "complete_phase from ANSWER is not allowed. Call answer_from_sql: a "
                "successful one auto-advances to VERIFY, and a failing one bounces you "
                "back to EXPLORE to diagnose. Do not advance without a valid answer."
            ), "transition"
...
class PhasedReActAgent:
    ...
    def _find_keyframe_paths(task: PublicTask) -> list:
        ...
        root = _P(os.environ.get("EXP163_KEYFRAME_ROOT", "/tmp/kobushi_exp163_keyframe_cache"))
        ...
        try:
            max_images = max(1, int(os.environ.get("EXP163_KEYFRAME_MAX_IMAGES", "50")))
        except ValueError:
            max_images = 18
    ...
    def _build_messages(
        ...
        if not flags.visual_attach_on():
            # note-only ablation: text keyframe note is already in the preamble;
            # attach NO images and NO raw video.
            first_user_content = task_prompt
        elif flags.keyframes_on():
            ...
            # exp_163 lever ③ (video_on): the raw video is NOT attached — a
    ...
    def run(self, task: PublicTask) -> AgentRunResult:
        ...
        for step_index in range(1, self.config.max_steps + 1):
            ...
            try:
                ...
                # exp_163 lever ② (prose_on): a catalog-404 on a knowledge-named
                ...
                if (
                    ...
                    phase = "verify"
                # AUTO-TRANSITION: ANSWER → EXPLORE when answer_from_sql FAILS
                # (0 rows, SQL error, router violation, ...). The ANSWER phase
                # only exposes answer_from_sql, so a failure would otherwise trap
                # the agent re-submitting blind. Bouncing back to EXPLORE restores
                # execute_sql / describe_data / read_doc / grep so it can diagnose
                # (inspect the data, fix the filter, gather literal values) before
                # retrying. n_explore_ok is already past the min gate, so the
                # EXPLORE → ANSWER re-entry via complete_phase is not blocked.
                elif (
                    phase == "answer"
                    and model_step.action == "answer_from_sql"
                    and not tool_result.ok
                ):
                    _log(f"[{task.task_id}] phase answer → explore (auto on answer_from_sql failure)")
                    phase = "explore"
