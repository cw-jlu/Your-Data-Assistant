...
- SOURCE-FAITHFUL: when copying a value from the source (video/doc/prose), do
  not change the literal — reproduce it exactly. Similar-looking characters are
  different values (e.g. ',' is not '、'); never normalize, transliterate,
  re-tokenize, or re-join with another separator. Numbers you COMPUTE via SQL
...
# ====================================================================
# exp_163 LEVER ① — answer_shape_guard (DISTINCT → JOIN-fanout-only +
...
=== exp_163 ANSWER-SHAPE OVERRIDE (supersedes the DISTINCT rule above) ===
...
- COUNT GRAIN follows the QUESTION's wording. For "how many / 多少 / 数目 /
  总数 / count" questions, default to COUNT(*) over the qualifying rows; do NOT
  collapse to one-row-per-entity via COUNT(DISTINCT ...), ROW_NUMBER()/RANK()=1,
  QUALIFY, or a dedup GROUP BY. Use COUNT(DISTINCT ...) ONLY when the QUESTION
  explicitly asks for distinct / unique / different
  (distinct/unique/去重/唯一/不同/不重复). The thing being counted — its noun,
  its plural form, or the "how many / 多少 / 几位" phrasing itself — is NOT a
  distinctness word and does NOT by itself require COUNT(DISTINCT). When unsure,
  prefer COUNT(*)."""

_VERIFY_SHAPE_OVERRIDE = """

=== exp_163 ANSWER-SHAPE OVERRIDE (supersedes the DUPLICATE SANITY step above) ===
...
# ====================================================================
# exp_163 LEVER ② — prose_doc_forced_read, H4a parse protocol. Appended to
...
def build_phased_system_prompt(
    ...
    exp_163 ablation: when EXP163_ANSWER_SHAPE / EXP163_PROSE are on, the
