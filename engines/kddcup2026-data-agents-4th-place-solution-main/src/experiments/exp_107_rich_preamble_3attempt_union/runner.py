...
from experiments.exp_107_rich_preamble_3attempt_union.agent import ReActAgent, ReActAgentConfig
from experiments.exp_107_rich_preamble_3attempt_union.config import AppConfig
from experiments.exp_107_rich_preamble_3attempt_union.preamble import build_preamble
from experiments.exp_107_rich_preamble_3attempt_union.tools.registry import (
    ToolRegistry,
...
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6, 0.6, 0.7)  # Qwen3.5 R1 official 3-attempt union (k=1, exp_086 confirmed +0.045)
...
def _run_single_task_with_timeout(*, task_id: str, config: AppConfig) -> dict[str, Any]:
    ...
    if not attempt_answers:
        # Preserve the first available run_result as backbone so that steps are
        # written to trace.json and remain inspectable for debugging. Without this,
        # single-attempt timeouts produce a steps=[] trace that is opaque.
        if attempt_run_results:
            backbone = dict(attempt_run_results[0])
            backbone["answer"] = None
            backbone["succeeded"] = False
            backbone["failure_reason"] = (
