...
from experiments.exp_129_rule_checkers.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_129_rule_checkers.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_129_rule_checkers.column_auditor import audit_columns, apply_audit
    from experiments.exp_129_rule_checkers.checkers import run_checker_chain
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        sql = str(ai["sql"])
        # ----- Rule checker chain (exp_129) -----
        # Run rule-based checkers BEFORE execution. If any rule fires, the SQL
        # is rewritten via LLM (or directly for TimeChecker), then executed.
        checker_meta = None
        if auditor_model is not None:
            chk = run_checker_chain(sql, auditor_model)
            if chk.triggered:
                sql = chk.final_sql
                checker_meta = {
                    "triggered": chk.triggered,
                    "notes": chk.notes,
                }
        try:
        ...
        if refines_left <= 0:
            ...
            if checker_meta: content["rule_checkers"] = checker_meta
