...
FIXED_EXAMPLES = """
Here are three complete solved examples from similar tasks, showing the expected reasoning and answer format:

--- Example A (single-value COUNT) ---
Question: Among the events attended by more than 10 members of the Student_Club, how many of them are meetings?
Final answer call:
```json
{"thought":"Plan was column_count=1, row_count=1. Counted meetings with attendance>10. Plan satisfied.","action":"answer_from_sql","action_input":{"path":"context/database.sqlite","sql":"SELECT COUNT(*) FROM event JOIN attendance ON event.event_id = attendance.event_id WHERE event.type = 'Meeting' GROUP BY event.event_id HAVING COUNT(*) > 10"}}
```
Gold output:
columns: ["COUNT(*)"], rows: [["4"]]

--- Example B (multi-column, multi-row JOIN) ---
Question: List the full name of the Student_Club members that grew up in Illinois state.
Final answer call:
```json
{"thought":"Plan was column_count=2, per_column=[first_name, last_name], row_count=multi. Filtered zip_code to Illinois. Plan satisfied.","action":"answer_from_sql","action_input":{"path":"context/database.sqlite","sql":"SELECT m.first_name, m.last_name FROM member m JOIN zip_code z ON m.zip = z.zip_code WHERE z.state = 'Illinois'"}}
```
Gold output:
columns: ["first_name","last_name"], rows: [["Trent","Smith"],["Tyler","Hewitt"],["Annabella","Warren"]]

--- Example C (single-column, multi-row DATE filter) ---
Question: Please list the countries of the gas stations with transactions taken place in June, 2013.
Final answer call:
```json
{"thought":"Plan was column_count=1, per_column=[Country], row_count=multi. Filtered transactions to 2013-06. Plan satisfied.","action":"answer_from_sql","action_input":{"path":"context/database.sqlite","sql":"SELECT DISTINCT gs.Country FROM gas_station gs JOIN transactions_1k t ON gs.GasStationID = t.GasStationID WHERE strftime('%Y-%m', t.Date) = '2013-06'"}}
```
Gold output:
