...
from experiments.exp_143_dynamic_doc.profile import (
    csv_profile_text,
    json_array_profile,
...
def _doc_full(path: Path) -> str:
    ...
    try:
        ...
        return f"## [DOC] {rel}\n[read error: {exc!r}]"
    # First non-empty heading or first ~200 chars as a one-line peek.
    head_line = ""
    for line in raw.splitlines():
        ...
        if s:
            ...
            break
    return (
        f"## [DOC] {rel} ({len(raw)} chars) — NOT inlined.\n"
        f"  first line: {head_line}\n"
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_143_dynamic_doc.tools.duckdb_unified import describe_catalog
    ...
    try:
        from experiments.exp_143_dynamic_doc.math_advisor import generate_formula
