...
15. Large document files (e.g. doc/*.md) are excerpted in the preamble (first 5 KB only).
    To find specific records in a large doc, call `search_doc(path, query)` with 1–3
    space-separated keywords. Example: search_doc("doc/items.md", "category rare") to
    locate lines matching those keywords with surrounding context. Use this instead of
    read_doc when the doc is truncated.
