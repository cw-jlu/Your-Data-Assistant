...
from experiments.exp_045_adaptive_doc_search.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
    resolve_context_path,
    search_doc as search_doc_impl,
)
from experiments.exp_045_adaptive_doc_search.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_045_adaptive_doc_search.tools.sqlite import (
...
def _search_doc(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    return ToolExecutionResult(ok=True, content=search_doc_impl(
        task, path, query, max_chunks=max_chunks, context_lines=context_lines,
    ))
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "search_doc": ToolSpec(
            name="search_doc",
            description=(
                "Keyword-search a markdown/text doc inside context. Returns up to "
                "`max_chunks` snippets (each surrounded by `context_lines` lines) "
                "where ALL space-separated keywords match (case-insensitive). "
                "Use this on large docs (e.g. doc/Patient.md, doc/superhero.md) "
                "instead of read_doc when you need to find specific records. "
                "Returns match_count=0 with empty chunks when no lines match (not an error)."
            ),
            input_schema={
                "path": "doc/example.md",
