...
from experiments.exp_125_difficulty_split.agent import (
    parse_model_step,
)
from experiments.exp_125_difficulty_split.prompt import (
    PHASE_NAME_TO_TOOLS,
...
from experiments.exp_125_difficulty_split.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_125_difficulty_split.tools.registry import ToolRegistry
...
class PhasedReActAgent:
    ...
    def _build_messages(
        ...
        # SQL-only mode for easy / medium tasks: drops read_doc/grep from
        # the allowed tools and omits the prose-guidance rule.
        sql_only = task.difficulty in ("easy", "medium")
        system_content = build_phased_system_prompt(
            phase, self.tools.describe_for_prompt(), sql_only=sql_only,
        )
