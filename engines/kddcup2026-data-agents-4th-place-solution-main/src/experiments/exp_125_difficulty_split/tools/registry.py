...
from experiments.exp_125_difficulty_split.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_125_difficulty_split.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_125_difficulty_split.column_auditor import audit_columns, apply_audit
...
def create_default_tool_registry(
    ...
    *,
    ...
    # Difficulty-based tool gating: easy/medium tasks have no doc/ files in
    # context (= empirically confirmed across DABench public 50), so exposing
    # read_doc / grep just adds action-space noise. SQL-only registry is used
    # for these. Hard/extreme keep full tools. doc_only overrides (= doc-only
    # tasks always need read_doc + grep regardless of difficulty label).
    sql_only = (difficulty in ("easy", "medium")) and not doc_only
    ...
    # SQL-only mode: strip doc-reading tools from both specs and handlers.
    # Easy/medium tasks have no doc/ files in their context (= public 50 evidence).
    if sql_only:
        for k in ("read_doc", "grep"):
            specs.pop(k, None)
            handlers.pop(k, None)
