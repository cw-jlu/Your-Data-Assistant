...
17. ENTITY DESCRIPTOR (default). For questions of the form
    ...
    primary-key ID column. Return the ID column when EITHER:
    - the question explicitly says "ID" / "id" / "identifier" / "number" / "code", OR
    - the entity's table has NO natural textual column (= e.g. molecule, bond,
      atom, account, district 系の schema where the _id IS the canonical
      identifier and no name/title/description exists).
...
PHASE_NAME_TO_TOOLS = {
    # FULL phase-to-tools mapping (= for hard / extreme tasks with doc tools).
    # `complete_phase` is a virtual transition action handled by PhasedReActAgent
    # (= no tool dispatch). Always allowed in every phase.
...
# SQL-only phase-to-tools (= for easy / medium tasks; no doc/ files in context).
# Strips read_doc and grep — they're not in the registry for these tasks, and
# their absence from the allowed list keeps the prompt and runtime gating
# consistent.
PHASE_NAME_TO_TOOLS_SQL = {
    "plan":    ("list_context", "complete_phase"),
    "explore": ("list_context", "describe_data", "execute_sql", "complete_phase"),
    "answer":  ("answer_from_sql", "complete_phase"),
    "verify":  ("execute_sql", "answer_from_sql", "confirm_answer", "complete_phase"),
}


# Rule 20: prose narrative guidance — relevant only when doc tools are
# available (= hard / extreme tasks). Appended to GLOBAL CONSTRAINTS so it
# applies across all phases that may read `.md`/`.txt` files.
RULE_20_PROSE_GUIDANCE = (
    "- `.md` / `.txt` files are prose, not tables. Use `read_doc` to read them "
    "and answer directly — do not SQL-parse them via `read_csv` or regex."
)


def build_phased_system_prompt(
    phase: str, tool_descriptions: str, *, sql_only: bool = False
) -> str:
    """Build the phase-specific system prompt for PhasedReActAgent.

    sql_only=True swaps the allowed-tools map to the doc-free version and
    omits the prose-guidance rule (= no `.md`/`.txt` reading paths).
    """
    instr = PHASE_NAME_TO_INSTRUCTION[phase]
    tools_map = PHASE_NAME_TO_TOOLS_SQL if sql_only else PHASE_NAME_TO_TOOLS
    allowed = tools_map[phase]
    base = PHASED_BASE_SYSTEM_PROMPT.format(
