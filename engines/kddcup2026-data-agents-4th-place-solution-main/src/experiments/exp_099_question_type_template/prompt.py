...
# (exp_099 [prompt:type-template]: + heuristic question type classifier
#  + type-specific advisory guidance blocks injected into build_task_prompt
#  when classifier confidence is high/med, otherwise base behavior.)
...
def build_task_prompt(task: PublicTask) -> str:
    # exp_099 [prompt:type-template]: prepend an advisory question-type
    # guidance block when the heuristic classifier reaches high/med
    # confidence. Empty string when classifier is uncertain — the
    # original task prompt is then byte-identical to exp_086.
    type_guidance = build_question_type_guidance(task.question)
    base_prompt = (
...
# ============================================================================
# Question type-aware planner template (exp_099, [prompt:type-template] axis)
# ============================================================================
# Heuristic classifier + type-specific guidance blocks. Goals:
#   1) Detect the question's analytic shape (COUNTING / SINGLE_VALUE / LIST /
#      AGGREGATION) plus a MULTI_ASK modifier for "X and Y"-style conjunctions.
#   2) Inject an advisory guidance block into the task prompt only when
#      classifier confidence is high or med — low-confidence cases skip to
#      avoid misleading the agent.
#   3) Keep wording strictly advisory ("Recommended approach (candidate)",
#      "may be incorrect", "agent may consider other approaches") so each
#      attempt remains free to disagree — explicitly avoiding the exp_088
#      Plan-first commitment trap that produced extras inflation.
# Stdlib regex only (no LLM call), all patterns use generic English NLP
# cues (How many / What is / List / average / sum / and / or). NO public
# domain vocabulary (student_club / formula1 / financial / molecule /
# cards / patient / schools / matches / posts) — leak-safe by construction.

_QT_COUNTING_RE = re.compile(r"\b(how many|count of|number of|count\b)", re.IGNORECASE)
_QT_SINGLE_RE = re.compile(r"\b(what is the|who is|which (?!.*\b(?:are|list|all)\b))", re.IGNORECASE)
_QT_LIST_RE = re.compile(r"\b(list (?:all|the)|show me|what are|which .*\bare\b|all the )", re.IGNORECASE)
_QT_AGG_RE = re.compile(r"\b(average|mean|total|sum|max(?:imum)?|min(?:imum)?|highest|lowest)\b", re.IGNORECASE)
_QT_MULTI_RE = re.compile(r"\b(\w{3,})\s+and\s+(\w{3,})\b", re.IGNORECASE)
_QT_FIRST_WORDS = 5  # tokens at the start of the question that get "high" boost


_QT_GUIDANCE: dict[str, str] = {
    "COUNTING": (
        "# Question type detected: COUNTING (heuristic, may be incorrect)\n"
        "# Recommended approach (candidate): the answer is a single integer\n"
        "#   -> 1 column x 1 row. Prefer `answer_from_sql` with `SELECT COUNT(*) FROM ...`.\n"
        "# This is advisory; agent may consider other approaches if reasoning indicates otherwise."
    ),
    "SINGLE_VALUE": (
        "# Question type detected: SINGLE_VALUE (heuristic, may be incorrect)\n"
        "# Recommended approach (candidate): the answer is one cell\n"
        "#   -> 1 column x 1 row. If the question uses superlative wording (highest/lowest/most/least),\n"
        "#   filter back to capture ties (`WHERE col = (SELECT MAX(col) ...)`) per Rule 10.\n"
        "# This is advisory; agent may consider other approaches if reasoning indicates otherwise."
    ),
    "LIST": (
        "# Question type detected: LIST (heuristic, may be incorrect)\n"
        "# Recommended approach (candidate): the answer is a list of values\n"
        "#   -> 1 column x N rows. Prefer `answer_from_sql` with deterministic ORDER BY.\n"
        "# This is advisory; agent may consider other approaches if reasoning indicates otherwise."
    ),
    "AGGREGATION": (
        "# Question type detected: AGGREGATION (heuristic, may be incorrect)\n"
        "# Recommended approach (candidate): the answer is one or more aggregate values\n"
        "#   -> typically 1 column x 1 row (single aggregate) or grouped result.\n"
        "#   Use `answer_from_python` (`df.groupby(...).agg(...)`) or `answer_from_sql` with AVG/SUM/MAX/MIN.\n"
        "#   Apply ROUND() at the outermost SELECT only (Rule 14).\n"
        "# This is advisory; agent may consider other approaches if reasoning indicates otherwise."
    ),
    "MULTI_ASK": (
        "# Question modifier detected: MULTI_ASK (heuristic, may be incorrect)\n"
        "# Recommended approach (candidate): the question asks for two or more distinct quantities\n"
        "#   joined by 'and' (e.g. 'X and Y'). The answer should include both as SEPARATE columns,\n"
        "#   not merged. This combines with the primary type detected above.\n"
        "# This is advisory; agent may consider other approaches if reasoning indicates otherwise."
    ),
}


def _classify_question_type(question: str) -> tuple[str, str, bool]:
    """Heuristic, deterministic classifier. No LLM. Returns
    (primary_type, confidence, is_multi_ask). Confidence is one of
    {"high","med","low"}. On low confidence the caller MUST skip injection.

    Patterns use only generic English NLP cues. No public-task vocabulary.
    """
    if not question or len(question.strip()) < 5:
        return ("UNKNOWN", "low", False)
    q = question.strip()
    q_first = " ".join(q.split()[:_QT_FIRST_WORDS])

    primary: str = "UNKNOWN"
    confidence: str = "low"
    matches: list[tuple[str, str]] = []  # (type, confidence)
    for type_name, pat in (
        ("COUNTING", _QT_COUNTING_RE),
        ("SINGLE_VALUE", _QT_SINGLE_RE),
        ("LIST", _QT_LIST_RE),
        ("AGGREGATION", _QT_AGG_RE),
    ):
        m = pat.search(q)
        if m is None:
            continue
        # Boost to "high" if the match falls in the first 5 tokens.
        conf = "high" if pat.search(q_first) else "med"
        matches.append((type_name, conf))

    if matches:
        # Pick highest-confidence match; on ties prefer COUNTING > AGGREGATION
        # > SINGLE_VALUE > LIST (more specific signals first).
        priority = {"COUNTING": 0, "AGGREGATION": 1, "SINGLE_VALUE": 2, "LIST": 3}
        matches.sort(key=lambda mc: (0 if mc[1] == "high" else 1, priority.get(mc[0], 9)))
        primary, confidence = matches[0]

    is_multi = bool(_QT_MULTI_RE.search(q))
    return (primary, confidence, is_multi)


def build_question_type_guidance(question: str) -> str:
    """Return a multi-line guidance block, or empty string when nothing to inject.

    Empty string => caller does NOT prepend anything (preserves base behavior).
    """
    try:
        primary, conf, is_multi = _classify_question_type(question)
    except Exception:
        return ""
    blocks: list[str] = []
    if primary != "UNKNOWN" and conf in {"high", "med"} and primary in _QT_GUIDANCE:
        blocks.append(_QT_GUIDANCE[primary])
    if is_multi:
        blocks.append(_QT_GUIDANCE["MULTI_ASK"])
    if not blocks:
        return ""
    return "\n".join(blocks)
