...
def describe_catalog(context_dir: Path) -> str:
    ...
    Includes top-5 DISTINCT sample values per VARCHAR column (= helps the agent
    use the exact case/format the data actually contains, reducing 0-rows errors).
    """
    ...
    for entry in cat:
        ...
        # Add sample VARCHAR values (= up to 5 columns, 5 distinct values each)
        try:
            col_types = conn.execute(f"DESCRIBE {view}").fetchall()
        except Exception:
            continue
        text_cols = [r[0] for r in col_types if str(r[1]).upper().startswith(("VARCHAR", "TEXT"))][:5]
        for col in text_cols:
            try:
                vals = conn.execute(
                    f'SELECT DISTINCT "{col}" FROM {view} WHERE "{col}" IS NOT NULL LIMIT 5'
                ).fetchall()
                samples = [str(r[0])[:30] for r in vals]
                if samples:
                    lines.append(f"    {col}: [{', '.join(repr(s) for s in samples)}]")
            except Exception:
                pass
    return "\n".join(lines)
