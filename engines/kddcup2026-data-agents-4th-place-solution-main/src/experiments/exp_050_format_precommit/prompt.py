...
1. Your VERY FIRST action's `thought` must contain an explicit answer plan with three parts:
   ...
   This plan is a COMMITMENT. Do not add columns beyond your stated column_count, even if
   additional data seems relevant. If you discover the plan was wrong, write "Revised plan:"
   and justify why the question requires a different column structure before changing column_count.
...
6. Before calling `answer`, restate your plan in the `thought` and confirm your output matches it
   (column_count and approximate row_count). Your answer MUST NOT exceed your committed
   column_count. If your current result has extra columns, drop them before calling `answer`.
