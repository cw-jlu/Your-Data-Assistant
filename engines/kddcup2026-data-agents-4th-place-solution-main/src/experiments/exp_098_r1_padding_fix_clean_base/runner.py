...
from experiments.exp_098_r1_padding_fix_clean_base.agent import ReActAgent, ReActAgentConfig
from experiments.exp_098_r1_padding_fix_clean_base.config import AppConfig
from experiments.exp_098_r1_padding_fix_clean_base.preamble import build_preamble
from experiments.exp_098_r1_padding_fix_clean_base.tools.registry import (
    ToolRegistry,
...
#
# (exp_098 row-padding fix bundle, 2026-05-06): The merge no longer pads
# short columns with None to reach max_len. Instead it selects a single
# intrinsic length group (mode by attempt count, smaller wins on tie)
# and drops columns from other length groups. This preserves the
# multiset signature when the answer round-trips through prediction.csv.
# Bundled from exp_093_runner_padding_fix where it was first verified
# (task_259 +0.031 trace-level confirmation; aggregate Δ noise but
# defense-in-depth value confirmed for downstream axis ablations).
...
def _signature_majority_merge(
    ...
    Row-padding fix (bundled from exp_093, 2026-05-06):
    The previous implementation padded short columns with `None` to reach
    `max(len(c))`. When the merged answer is written to prediction.csv and
    re-read by the official eval (`_column_signature` = `tuple(sorted(...))`),
    those `None` cells normalize to `""` and inflate the multiset
    cardinality, destroying the matching signature of the originally-shorter
    column (task_25 / task_259 / task_11 root cause).

    Fix strategy: never pad. After deciding which signatures to keep,
    group the kept columns by their intrinsic length and select a SINGLE
    length group: the length contributed by the most distinct attempts
    (mode by attempt count). On tie, the smaller length wins (DABench gold
    skews toward minimal answers, and shorter cols reduce extras_ratio).
    Columns from non-chosen length groups are dropped. The chosen group's
    columns all share one length, so no padding is needed and the
    multiset signature round-trips through CSV intact.
    ...
    # Score each length: (num_distinct_attempts_contributing, -length).
    # max() picks the length with most distinct attempts; on tie the smaller
    # length wins because -length is larger for smaller length.
...
# Derive experiment prefix from the parent directory name (e.g. "exp_098_r1_padding_fix_clean_base")
