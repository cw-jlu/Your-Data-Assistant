...
from experiments.exp_162_full_tool_descriptions.agent import ReActAgent, ReActAgentConfig
from experiments.exp_162_full_tool_descriptions.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_162_full_tool_descriptions.adaptive_vote import adaptive_vote
from experiments.exp_162_full_tool_descriptions.config import AppConfig
from experiments.exp_162_full_tool_descriptions import flags
from experiments.exp_162_full_tool_descriptions.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_162_full_tool_descriptions.prefix_cache import with_prefix_cache_header
from experiments.exp_162_full_tool_descriptions.tools.registry import (
    ToolRegistry,
    create_default_tool_registry,
)
from experiments.exp_162_full_tool_descriptions.audio_asr import audio_preamble_block
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP162_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP162_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp162_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP162_KEYFRAME_ROOT"):
        _os.environ["EXP162_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp162_keyframe_cache" / run_name
        )


def _use_precomputed_preprocess() -> bool:
    raw = _os.environ.get("EXP162_USE_PRECOMPUTED_PREPROCESS", "")
...
def _precomputed_task_dir(task_id: str) -> Path | None:
    if not _use_precomputed_preprocess():
        return None
    root = _os.environ.get("EXP162_PREPROCESS_ROOT")
    if not root:
        return None
...
def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp162 bench script."""
...
def _run_single_task_core(
    *,
    ...
    if flags.audio_asr_on():
        ...
        if asr_text:
            preamble_text = audio_preamble_block(
                task.question,
                asr_text,
                max_chars=_env_int("EXP162_ASR_MAX_CHARS", 2600),
            ) + preamble_text
    ...
    if tools is None:
        ...
        # exp162 keeps the v4/exp155 runner shape: no projection pruner and no
        # exp152-style auto-rewrite guard. The soft final SQL guard is
        # deterministic inside the registry and needs no guard_model.
    ...
    if asr_text and _env_flag("EXP162_AUDIO_TO_ROUTER", True):
        router_video_note = (
            (router_video_note or "")
            + "\n\n# VIDEO AUDIO TRANSCRIPT (ASR; may contain errors)\n"
            + asr_text[: _env_int("EXP162_ROUTER_ASR_MAX_CHARS", 1600)]
        ).strip()
    ...
    trace_root = _os.environ.get("EXP162_TRACE_ROOT")
    if trace_root:
    ...
    payload["exp162_runtime_metadata"] = {
