...
from experiments.exp_162_full_tool_descriptions.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_162_full_tool_descriptions.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
    normalize_sql,
...
_EXPLICIT_UNIQUE_RE = re.compile(
...
_EXPLICIT_NONNULL_RE = re.compile(
    r"\b(?:non[- ]?null|non[- ]?empty|known values?|available values?|"
    r"excluding (?:missing|blanks?|nulls?)|not missing)\b"
    r"|有值|非空|不为空|已知|排除空值|排除缺失|不缺失",
    re.IGNORECASE,
)
_MEASURE_NULL_OK_RE = re.compile(
    r"\b(?:highest|lowest|top|bottom|maximum|minimum|max|min|average|avg|sum)\b"
    r"|最高|最低|最大|最小|排名|前\s*\d+|后\s*\d+",
    re.IGNORECASE,
)


def _distinct_forms(sql: str) -> list[str]:
    forms: list[str] = []
    if _COUNT_DISTINCT_RE.search(sql):
        forms.append("COUNT(DISTINCT ...)")
    if _SELECT_DISTINCT_RE.search(sql):
        forms.append("SELECT DISTINCT")
    if not forms and _DISTINCT_RE.search(sql):
        forms.append("DISTINCT")
    return forms


def _soft_final_sql_guard_violation(question: str, sql: str) -> dict[str, Any] | None:
    """Return a rejection payload for high-risk final SQL grain changes.

    This is intentionally deterministic. The failed exp152-style guard rewrote
    SQL after classification; exp162 only rejects and asks the model to rewrite.
    """
    if _DISTINCT_RE.search(sql) and not _EXPLICIT_UNIQUE_RE.search(question or ""):
        forms = _distinct_forms(sql)
        return {
            "error": (
                "[DISTINCT guard] This benchmark usually preserves source row "
                "grain, including duplicate source rows. Do not use SELECT "
                "DISTINCT or COUNT(DISTINCT ...) unless the question explicitly "
                "asks for distinct/unique/deduplicated/去重/唯一/不重复. "
                "If you added DISTINCT to avoid JOIN fan-out, rewrite without "
                "DISTINCT: start from the requested base table/entity and use "
                "EXISTS or a pre-aggregated subquery so joins do not duplicate "
                "the base rows. Do not convert a requested record/snapshot count "
                "into an entity count unless uniqueness is explicit."
            ),
            "guard": "soft_final_sql_guard",
            "variant": "distinct",
            "forms": forms,
            "explicit_unique_terms": False,
            "sql": sql,
        }

    if _NULL_FILTER_RE.search(sql) and not _EXPLICIT_NONNULL_RE.search(question or ""):
        # Keep common ranking/aggregate guards: SQL NULL semantics already ignore
        # NULLs for MAX/MIN/AVG/SUM, but agents often add IS NOT NULL around those
        # exact measures. Blocking them tends to waste refines without restoring
        # row-preserving behavior.
        if _MEASURE_NULL_OK_RE.search(question or "") or re.search(
            r"\b(?:MAX|MIN|AVG|SUM)\s*\(", sql, re.IGNORECASE
        ):
            return None
        return {
            "error": (
                "[NULL/blank guard] This benchmark usually preserves NULL and "
                "blank source rows. Do not add IS NOT NULL, <> '', or != '' "
                "unless the question explicitly asks for non-null/non-empty/"
                "known/available/excluding missing/非空/不为空/有值/排除空值. "
                "For plain show/list/retrieve/display questions, keep NULL/"
                "blank rows in the answer."
            ),
            "guard": "soft_final_sql_guard",
            "variant": "null_filter",
            "explicit_nonnull_terms": False,
            "sql": sql,
        }
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        from experiments.exp_162_full_tool_descriptions import flags

        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP162_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_162_full_tool_descriptions.video_summarizer import find_video
...
def _describe_data(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    # exp_149 lever ② (prose_on): advertise prose-only docs so the agent learns
...
def _normalize_action_sql(action_input: dict[str, Any] | str) -> tuple[dict[str, Any], dict[str, Any]]:
    action_input = dict(_coerce_sql_input(action_input))
    normalized_sql, meta = normalize_sql(str(action_input["sql"]))
    action_input["sql"] = normalized_sql
    return action_input, meta


def _execute_sql(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    action_input, normalization_meta = _normalize_action_sql(action_input)
    ...
    try:
        ...
        content.update(normalization_meta)
        return ToolExecutionResult(
            ok=False,
            content=content,
        )
    result.update(normalization_meta)
...
def _is_literal_answer_sql(sql: str) -> bool:
    """True for literal/VALUES materialization with no broad table access."""
    return not _source_router_avoid_violation(sql)
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_162_full_tool_descriptions.column_auditor import audit_columns, apply_audit

    state = {"refines": 0, "pending_answer": None}

    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ai, normalization_meta = _normalize_action_sql(action_input)
        sql = str(ai["sql"])
        source_route = ai.get("_source_route") if isinstance(ai, dict) else None
        source_avoid_literal = (
            isinstance(source_route, dict)
            and source_route.get("sql_role") == "avoid"
            and _is_literal_answer_sql(sql)
        )
        import re as _re
        if (
            isinstance(source_route, dict)
        ...
        if flags.soft_final_sql_guard_on():
            soft_guard_violation = _soft_final_sql_guard_violation(q_text, sql)
            if soft_guard_violation is not None:
                soft_guard_violation.update(normalization_meta)
                return ToolExecutionResult(ok=False, content=soft_guard_violation)
        ...
        if not source_avoid_literal and _re.search(r"\b(percent|percentage|%)\b", q_text, _re.IGNORECASE):
        ...
        if not source_avoid_literal and _re.search(r"\baverage\s+(monthly|yearly|daily|hourly|weekly|annual)\b", q_text, _re.IGNORECASE):
        ...
        if not source_avoid_literal and _re.search(r"\bper\s+(unit|item|day|hour|minute|capita|person|customer)\b", q_text, _re.IGNORECASE):
        ...
        universal_guard_meta = {}
        if source_avoid_literal:
            universal_guard_meta = {
                "guard": "universal_rule_guard",
                "skipped": True,
                "reason": (
                    "source_router sql_role=avoid and SQL is literal/VALUES; "
                    "source evidence has priority over broad SQL recomputation guards"
                ),
            }
        ...
        if rule_violations:
            content = {
                "error": "[Universal rule violation] " + " | ".join(rule_violations),
                "sql": sql,
            }
            content.update(normalization_meta)
            return ToolExecutionResult(
        ...
        pending_meta = {}
        if source_route:
            pending_meta["source_route"] = source_route
        if normalization_meta:
            pending_meta["sql_normalization"] = normalization_meta
        if universal_guard_meta:
            pending_meta["universal_rule_guard"] = universal_guard_meta
        state["pending_meta"] = pending_meta
        ...
        if refines_left <= 0:
            ...
            if normalization_meta: content["sql_normalization"] = normalization_meta
            if universal_guard_meta: content["universal_rule_guard"] = universal_guard_meta
...
def _make_confirm_answer(answer_handler) -> ToolHandler:
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if meta.get("sql_normalization"): content["sql_normalization"] = meta["sql_normalization"]
        if meta.get("universal_rule_guard"): content["universal_rule_guard"] = meta["universal_rule_guard"]
        return ToolExecutionResult(ok=True, content=content, is_terminal=True, answer=answer)
    return handler
...
class ToolRegistry:
    ...
    def describe_for_prompt(self, allowed_names: Iterable[str] | None = None) -> str:
    ...
    specs.update({
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP162_PROSE=1). Inspired by Claude "
