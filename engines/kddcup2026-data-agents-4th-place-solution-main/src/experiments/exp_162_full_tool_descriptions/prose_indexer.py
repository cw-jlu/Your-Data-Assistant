...
from experiments.exp_162_full_tool_descriptions.tools.filesystem import resolve_context_path
...
def index_cache_root() -> Path:
    root = Path(os.environ.get("EXP162_PROSE_INDEX_ROOT", "artifacts/prose_doc_index"))
...
def _load_doc_text(task: PublicTask, relative_path: str) -> str:
    ...
    if path.suffix.lower() == ".pdf":
        try:
            from experiments.exp_162_full_tool_descriptions.pdf_text_cache import read_cached_pdf_text
...
- Use the stable record id shown in text, usually a short label phrase immediately followed by a number.
...
{{
  ...
  "join_key": "<record-anchor label found in this doc>",
  "sections": [
    {{
  ...
  "records": [
    {{
      "record_id": "<record id>",
      "display_name": "<fund display name>",
      "secu_code": "<fund code>",
      "total_profit": 12345678.90,
      "end_date": "2021-12-31",
  ...
  "top_records_for_question": [
    {{
      "rank": 1,
      "record_id": "<record id>",
      "display_name": "<another fund display name>",
      "secu_code": "<fund code>",
