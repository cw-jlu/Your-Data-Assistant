...
from experiments.exp_060_search_doc_additive.tools.filesystem import (  # noqa: I001
    search_doc,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_060_search_doc_additive.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_060_search_doc_additive.tools.sqlite import (
...
def _search_doc(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    return ToolExecutionResult(
        ...
        content=search_doc(
            task, path, query,
            max_chunks=max_chunks, context_lines=context_lines,
        ),
    )
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "search_doc": ToolSpec(
            name="search_doc",
            description=(
                "Keyword-search inside a markdown/text doc. Returns up to `max_chunks` "
                "snippets (each surrounded by `context_lines` lines) where ALL "
                "space-separated keywords match the same line, case-insensitive. "
                "Useful for finding a specific record (a patient ID, a hero's name, "
                "a recipe title) inside a large narrative doc without reading it whole."
            ),
            input_schema={
                "path": "doc/SomeDoc.md",
                "query": "patient_id 12345 abnormal",
                "max_chunks": 5,
                "context_lines": 5,
            },
        ),
