...
def search_doc(
    ...
    *,
    ...
    """Keyword search inside a doc-like file (markdown / text).

    Returns up to ``max_chunks`` snippets where ALL space-separated keywords
    in ``query`` (case-insensitive) appear in the same line. Each snippet is
    the matching line plus ``context_lines`` lines before and after.

    Designed for large narrative documents (e.g. doc/Patient.md, doc/superhero.md)
    where the agent needs to look up a specific record (a patient ID, a hero's
    name, a recipe name) without paying the token cost of reading the whole file.
    """
    path = resolve_context_path(task, relative_path)
    text = path.read_text(errors="replace")
    ...
    if not keywords:
        return {
            ...
            "match_count": 0,
            "chunks": [],
            "note": "Empty query — provide one or more space-separated keywords.",
        }
    matched: list[int] = []
    ...
    return {
        ...
        "chunks": chunks,
        "truncated": len(matched) > max_chunks,
    }
