...
18. When an observation contains "__error__", before writing your next action you MUST include
    in your thought:
      ERROR ANALYSIS: [1 sentence: what caused this specific error]
      NEXT ACTION: [1 sentence: what you will do differently this time]
    Do NOT repeat the exact same code after 2 or more consecutive errors — change your approach
    (use a different method, different column name, or different logic).
    If execute_python returns an empty DataFrame (0 rows), verify your filter condition against
    actual data values (e.g., check unique values of the filtered column) before concluding
