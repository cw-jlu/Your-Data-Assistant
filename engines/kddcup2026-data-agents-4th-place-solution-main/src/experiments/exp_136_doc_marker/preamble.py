...
from experiments.exp_136_doc_marker.profile import (
    csv_profile_text,
    json_array_profile,
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_136_doc_marker.tools.duckdb_unified import describe_catalog
    ...
    # Detect mixed tasks: some prose files exist (= contain entity tables) but
    # also some SQL-queryable data. The agent must understand which entities are
    # in prose (= use read_doc) vs SQL (= use answer_from_sql).
    prose_files = [
        p.relative_to(task.context_dir)
        for p in files
        if p.suffix.lower() in (".md", ".markdown")
        and p.name.lower() not in ("knowledge.md",)  # exclude knowledge guide
    ]
    routing_note = ""
    if not has_data:
        routing_note = (
        ...
        # Mixed task: prose tables exist alongside SQL data
        prose_list = "\n".join(f"  - {p}" for p in prose_files)
        routing_note = (
            "\n\n# IMPORTANT: prose-entity files (use read_doc, NOT SQL)\n"
            "The following files are PROSE narrative — they contain entity data "
            "but NOT as queryable tables. SQL like "
            "`SELECT * FROM <name>` will FAIL with 'table not exist':\n"
            f"{prose_list}\n"
            "For these entities, use `read_doc` (with `search=` for specific terms) to "
            "extract values, then either:\n"
            "  (a) hardcode IDs/values into SQL when small lookup needed (= JOIN against "
            "the listed prose data), or\n"
            "  (b) for full prose-table aggregation, use `answer` tool directly with "
            "hand-computed rows."
        )
