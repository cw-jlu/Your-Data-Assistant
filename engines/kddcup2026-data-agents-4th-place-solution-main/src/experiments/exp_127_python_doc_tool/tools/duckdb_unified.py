...
def execute_sql(context_dir: Path, sql: str, limit: int = 200) -> dict[str, Any]:
    ...
    Read-only enforced via prefix check; prose-file guard rejects attempts
    to SQL-parse `.md` / `.txt` / `.markdown` files (= structurally not
    tabular) and directs the agent at `read_doc` instead.
    """
