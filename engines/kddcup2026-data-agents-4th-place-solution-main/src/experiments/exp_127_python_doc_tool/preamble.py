...
_DOMAIN_DB_DIR = Path(os.environ.get(
    "KDD_DOMAIN_DB_DIR",
    str(Path(__file__).resolve().parents[3] / "data/external/domain_db"),
))
...
def _detect_db_id(context_dir: Path) -> str | None:
    """Parse knowledge.md header for the BIRD-style db_id."""
...
# Stop tokens excluded from question keyword extraction (= reduce noise).
_DOMAIN_STOPWORDS = {
    "a", "an", "and", "are", "as", "at", "be", "by", "for", "from", "had",
    "has", "have", "in", "is", "it", "its", "of", "on", "or", "that", "the",
    "their", "this", "to", "was", "were", "what", "when", "where", "which",
    "who", "with", "how", "many", "much", "all", "any", "more", "than",
    "list", "show", "find", "name", "names", "give", "get", "tell", "do",
    "does", "did", "can", "could", "would", "should", "may", "might",
    "i", "you", "he", "she", "they", "we", "them", "us", "me", "my", "your",
    "please", "calculate", "determine", "identify", "between", "among",
    "those", "these", "such", "each", "every", "some",
}


def _extract_keywords(question: str) -> list[str]:
    """Tokenize question into lowercase content words (= for retrieval scoring)."""
    tokens = re.findall(r"[A-Za-z][A-Za-z_]+", question.lower())
    return [t for t in tokens if len(t) >= 2 and t not in _DOMAIN_STOPWORDS]


def _load_domain_reference(
    context_dir: Path, question: str | None = None, top_k: int = 8,
    max_chars: int = 6000,
) -> str | None:
    """Return a RAG-style subset of the per-DB domain reference.

    Scoring: each "### Field: ..." entry is scored by how many of the question's
    content tokens appear in it (case-insensitive). Returns the top-k entries
    plus the file header. Falls back to the full file when the question is
    empty / has no usable tokens (= cap by max_chars to avoid bloat).
    """
    db_id = _detect_db_id(context_dir)
    if not db_id:
        return None
    ...
    header_match = re.match(r"^(.*?)(?=\n## Table:|\n### Field:)", text, re.DOTALL)
    header = header_match.group(0).strip() if header_match else ""

    entries = re.split(r"\n(?=### Field:)", text)
    # Drop the prefix portion (= up to the first Field heading) — the rest is per-field.
    entries = [e for e in entries if e.lstrip().startswith("### Field:")]

    keywords = _extract_keywords(question or "")
    if not keywords or not entries:
        # Fall back to full reference, capped at max_chars to avoid bloat.
    ...
    out_parts = [f"# Domain Reference: {db_id} (top {len(top)} matched fields)"]
    out_parts.extend(e for _, e in top)
    out = "\n\n".join(out_parts)
    if len(out) > max_chars:
        out = out[:max_chars] + "\n…[truncated]"
    return out

from experiments.exp_127_python_doc_tool.profile import (
    csv_profile_text,
    json_array_profile,
...
def _budget_cut(
    ...
    """
    _DROP_ORDER = ["json", "csv", "doc", "sqlite", "knowledge_full", "domain_reference"]
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_127_python_doc_tool.tools.duckdb_unified import describe_catalog
    ...
    # Domain reference auto-injection (RAG-style): pick top-k field entries
    # whose text matches the question's content tokens. Restores field-level
    # schema notes (= thresholds, value mappings, format hints) that the
    # LLM-generated knowledge.md often loses, while avoiding bloat from
    # unrelated fields.
    domain_ref = _load_domain_reference(task.context_dir, question=task.question)
    if domain_ref:
        labeled.append(("domain_reference", domain_ref))
