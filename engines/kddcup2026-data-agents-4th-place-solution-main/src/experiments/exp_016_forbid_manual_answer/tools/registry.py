...
from experiments.exp_016_forbid_manual_answer.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_016_forbid_manual_answer.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_016_forbid_manual_answer.tools.sqlite import (
...
def _answer(_: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    is_large = len(normalized_rows) > 1 or len(columns) > 1
    content: dict[str, Any] = {
        "status": "submitted",
        "column_count": len(columns),
    ...
    if is_large:
        content["warning"] = (
            f"answer tool called with {len(columns)} column(s) × {len(normalized_rows)} row(s). "
            "Prefer answer_from_sql or answer_from_python for table results."
        )
