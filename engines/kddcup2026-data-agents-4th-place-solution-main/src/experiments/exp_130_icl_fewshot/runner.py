...
from experiments.exp_130_icl_fewshot.agent import ReActAgent, ReActAgentConfig
from experiments.exp_130_icl_fewshot.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_130_icl_fewshot.adaptive_vote import adaptive_vote
from experiments.exp_130_icl_fewshot.config import AppConfig
from experiments.exp_130_icl_fewshot.preamble import build_preamble
...
def _run_single_task_core(
    *,
    ...
    agent_type: str = "phased",
) -> dict[str, Any]:
    """Run a single attempt with the specified agent.

    agent_type:
      - "phased" (default): PhasedReActAgent (= plan/explore/answer/verify)
      - "icl_react": ICL ReAct (= fewshot-enriched single-stage ReAct, fewer steps)
    """
    ...
    if agent_type == "icl_react":
        from experiments.exp_130_icl_fewshot.icl_react_agent import (
            run_icl_react_attempt,
        )
        icl_result = run_icl_react_attempt(
            task=task,
            model=agent_model,
            tools=tools,
            k=9,
        ...
        # Convert to the same payload shape as PhasedReActAgent.to_dict()
        if icl_result.succeeded and icl_result.answer is not None:
            payload = {
                "task_id": task_id,
                "answer": {
                    "columns": list(icl_result.answer.columns),
                    "rows": [list(r) for r in icl_result.answer.rows],
                },
                "steps": [],
                "failure_reason": None,
                "succeeded": True,
                "icl_react_meta": {
                    "n_fewshots": icl_result.n_fewshots,
                    "n_steps": icl_result.n_steps,
                },
            }
        else:
            payload = {
                ...
                "answer": None,
                "steps": [],
                "failure_reason": icl_result.failure_reason,
                "succeeded": False,
                "icl_react_meta": {
                    "n_fewshots": icl_result.n_fewshots,
                    "n_steps": icl_result.n_steps,
        ...
        queue.put(
            {
                ...
                "run_result": _run_single_task_core(
                    task_id=task_id, config=config, agent_type=agent_type
                ),
            }
        )
...
def _run_single_task_with_timeout(*, task_id: str, config: AppConfig) -> dict[str, Any]:
    ...
    # exp_130: replace LAST attempt (= index n-1) with ICL ReAct for mode diversity.
    # First n-1 attempts use PhasedReActAgent (= v5 baseline). Total subprocess
    # count stays at n_attempts so vLLM load matches v5.
    queues: list[multiprocessing.Queue[Any]] = []
    processes: list[multiprocessing.Process] = []
    for idx, temp in enumerate(_ATTEMPT_TEMPS):
        q = ctx.Queue()
        attempt_config = _config_with_temperature(config, temp)
        agent_type = "icl_react" if idx == n_attempts - 1 else "phased"
        p = ctx.Process(
            target=_run_single_task_in_subprocess,
            args=(task_id, attempt_config, q, agent_type),
        )
        p.start()
        queues.append(q)
        processes.append(p)
    ...
    # NOTE: exp_130 ICL inline-post-vote section removed.
    # ICL is now a SUBPROCESS attempt (= replaces last phased attempt).
    # See _run_single_task_with_timeout's spawn loop above.

    # Adaptive vote: 2 phased + 1 ICL ReAct candidates feed in.
    ...
    backbone["majority_3x_metadata"] = {
        ...
        "icl_react_attempt_idx": n_attempts - 1,  # last attempt is ICL ReAct
    }
