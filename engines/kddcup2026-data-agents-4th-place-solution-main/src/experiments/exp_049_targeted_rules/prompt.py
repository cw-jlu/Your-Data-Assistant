...
15. When computing averages, sums, counts, or any aggregate, do NOT exclude rows where
    a column equals 0 or is NULL, unless the question explicitly says to exclude zeros
    or non-null only (e.g. "non-zero", "positive values only", "excluding nulls").
    Zero is a valid data value. Do not add WHERE col > 0 or WHERE col IS NOT NULL
    unless the question explicitly requires it.
16. When counting bidirectional relationships — edges, bonds, connections, friendships,
    or any symmetric pair (A↔B is the same as B↔A) — count each unique pair exactly once.
    Deduplicate using MIN/MAX of IDs:
      SELECT COUNT(*) FROM (
        SELECT MIN(id1, id2) AS lo, MAX(id1, id2) AS hi
        FROM edges GROUP BY lo, hi
      )
    Never count A→B and B→A as two separate entries unless the question explicitly asks
    for directed edges or one-way relationships.
