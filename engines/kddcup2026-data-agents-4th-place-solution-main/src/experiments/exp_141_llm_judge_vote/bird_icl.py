...
def format_icl_block(
    ...
    *,
    ...
    if not examples:
        return None
    parts: list[str] = [
        "# ICL EXAMPLES (= relevant SQL patterns from BIRD train, different schemas)",
        (
            "Use these as STRUCTURAL templates only — the column/table names refer to a\n"
            "DIFFERENT database. Adapt the AGGREGATION style (COUNT/SUM/AVG, percentage\n"
            "formula = SUM(CASE) * 100 / COUNT), JOIN pattern, NULL/filter handling, and\n"
            "subquery shape. Do NOT copy column names verbatim.\n"
        ),
    ]
