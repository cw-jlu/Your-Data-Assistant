...
from experiments.exp_150_projection_pruner.agent import ReActAgent, ReActAgentConfig
from experiments.exp_150_projection_pruner.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_150_projection_pruner.adaptive_vote import adaptive_vote
from experiments.exp_150_projection_pruner.config import AppConfig
from experiments.exp_150_projection_pruner import flags
from experiments.exp_150_projection_pruner.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_150_projection_pruner.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP150_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP150_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp150_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP150_KEYFRAME_ROOT"):
        _os.environ["EXP150_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp150_keyframe_cache" / run_name
        )


def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp150 bench script."""
...
def _run_single_task_core(
    *,
    ...
    trace_root = _os.environ.get("EXP150_TRACE_ROOT")
    if trace_root:
    ...
    payload["exp150_runtime_metadata"] = {
