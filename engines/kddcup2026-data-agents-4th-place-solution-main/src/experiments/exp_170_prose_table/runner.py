...
from experiments.exp_170_prose_table.agent import ReActAgent, ReActAgentConfig
from experiments.exp_170_prose_table.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_170_prose_table.adaptive_vote import adaptive_vote
from experiments.exp_170_prose_table.config import AppConfig
from experiments.exp_170_prose_table import flags
from experiments.exp_170_prose_table.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_170_prose_table.prefix_cache import with_prefix_cache_header
from experiments.exp_170_prose_table.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP170_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP170_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP170_KEYFRAME_ROOT"):
        _os.environ["EXP170_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    # unknown general DB that may need such calc). Toggle with EXP170_MATH_OTHER_ONLY.
    ...
    if flags.prose_extract_on():
        try:
            ...
            _ext_root = _os.environ.get("EXP170_EXTRACTED_ROOT")
            if not _ext_root:
                _ext_root = str(config.run.output_dir / "_prose_extracted")
                _os.environ["EXP170_EXTRACTED_ROOT"] = _ext_root
            _df = _os.environ.get("EXP170_PROSE_DOC_FILTER")  # optional manual override
    ...
    trace_root = _os.environ.get("EXP170_TRACE_ROOT")
    if trace_root:
