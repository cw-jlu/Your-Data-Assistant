...
from experiments.exp_153_audio_asr.agent import ReActAgent, ReActAgentConfig
from experiments.exp_153_audio_asr.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_153_audio_asr.adaptive_vote import adaptive_vote
from experiments.exp_153_audio_asr.config import AppConfig
from experiments.exp_153_audio_asr import flags
from experiments.exp_153_audio_asr.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_153_audio_asr.prefix_cache import with_prefix_cache_header
from experiments.exp_153_audio_asr.tools.registry import (
    ToolRegistry,
    create_default_tool_registry,
)
from experiments.exp_153_audio_asr.audio_asr import audio_preamble_block
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP153_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP153_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp153_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP153_KEYFRAME_ROOT"):
        _os.environ["EXP153_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp153_keyframe_cache" / run_name
        )


def _use_precomputed_preprocess() -> bool:
    raw = _os.environ.get("EXP153_USE_PRECOMPUTED_PREPROCESS", "")
...
def _precomputed_task_dir(task_id: str) -> Path | None:
    if not _use_precomputed_preprocess():
        return None
    root = _os.environ.get("EXP153_PREPROCESS_ROOT")
    if not root:
        return None
...
def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp153 bench script."""
...
def _run_single_task_core(
    *,
    ...
    if flags.audio_asr_on():
        ...
        if asr_text:
            preamble_text = audio_preamble_block(
                task.question,
                asr_text,
                max_chars=_env_int("EXP153_ASR_MAX_CHARS", 2600),
            ) + preamble_text
    ...
    if tools is None:
        ...
        # The final-SQL guard must be deterministic (T=0.0) to match the local
        # bench (`make_model(temp=0.0)`); reusing agent_model would run the guard
        # classifiers at the agent's T=0.6 and flip KEEP/REMOVE decisions.
        guard_model = (
            build_model_adapter(config, task_id=task_id, temperature=0.0)
            if flags.final_sql_guard_on()
            else None
        )
    ...
    if asr_text and _env_flag("EXP153_AUDIO_TO_ROUTER", True):
        router_video_note = (
            (router_video_note or "")
            + "\n\n# VIDEO AUDIO TRANSCRIPT (ASR; may contain errors)\n"
            + asr_text[: _env_int("EXP153_ROUTER_ASR_MAX_CHARS", 1600)]
        ).strip()
    ...
    trace_root = _os.environ.get("EXP153_TRACE_ROOT")
    if trace_root:
    ...
    payload["exp153_runtime_metadata"] = {
