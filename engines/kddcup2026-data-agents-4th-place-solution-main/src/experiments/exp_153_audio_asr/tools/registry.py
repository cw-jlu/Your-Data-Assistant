...
from experiments.exp_153_audio_asr.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_153_audio_asr.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
_DISTINCT_GUARD_SYSTEM = """You classify SQL DISTINCT usage.
...
_NULL_FILTER_GUARD_SYSTEM = """You classify final SQL NULL/blank filtering.
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        from experiments.exp_153_audio_asr import flags

        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP153_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_153_audio_asr.video_summarizer import find_video
...
def _describe_data(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    # exp_153 lever ② (prose_on): advertise prose-only docs so the agent learns
...
def _select_list_has_output_aggregate(sql: str) -> bool:
    ...
    if not select_list:
        return False
    return bool(re.search(r"\b(?:SUM|AVG|COUNT)\s*\(", select_list, re.IGNORECASE))


def _rewrite_distinct(sql: str) -> str:
...
def _rewrite_null_filter(sql: str) -> str:
    out = sql
...
def _guard_user_prompt(
    *,
    ...
    return (
        f"Question:\n{question}\n\n"
        f"Final SQL:\n{sql[:2400]}\n\n"
        f"Executed result metadata:\n"
        f"columns: {columns}\n"
        f"row_count: {len(rows)}\n"
        f"preview: {preview}\n\n"
        f"Classify only the {operation} final SQL operation."
    )
...
def _apply_final_sql_guard(
    *,
    ...
    if model is None or not flags.final_sql_guard_on():
        return sql, columns, rows, None

    meta: dict[str, Any] = {
        "mode": "final_sql_guard",
        "stage": "post_execute_pre_review",
        "original_sql": sql,
        "final_sql": sql,
        "checks": [],
    }
    ...
    def maybe_rewrite(
        *,
        ...
        nonlocal current_sql, current_columns, current_rows
        check: dict[str, Any] = {
            "variant": variant,
            "label": label,
            "applied": False,
        ...
        if label.startswith("REMOVE_"):
            rewritten = rewrite_fn(current_sql)
            check["rewritten_sql"] = rewritten
            if rewritten.strip() == current_sql.strip():
                check["error"] = "rewrite_noop"
            else:
                try:
                    new_cols, new_rows = _execute_answer_sql(task, rewritten)
                    current_sql = rewritten
                    current_columns = new_cols
                    current_rows = new_rows
                    check["applied"] = True
                    check["row_count"] = len(new_rows)
                    check["columns"] = new_cols
                except Exception as exc:
                    check["error"] = repr(exc)
        meta["checks"].append(check)

    if _DISTINCT_RE.search(current_sql):
        label = _classify_guard(
            model,
            system=_DISTINCT_GUARD_SYSTEM,
            user=_guard_user_prompt(
                question=question,
                sql=current_sql,
            ...
            allowed={"KEEP_DISTINCT", "REMOVE_DISTINCT"},
        )
        maybe_rewrite(variant="distinct", label=label, rewrite_fn=_rewrite_distinct)

    if _NULL_FILTER_RE.search(current_sql):
        label = _classify_guard(
            model,
            system=_NULL_FILTER_GUARD_SYSTEM,
            user=_guard_user_prompt(
                question=question,
                sql=current_sql,
                columns=current_columns,
                rows=current_rows,
                operation="NULL-filter",
            ),
            allowed={"KEEP_NULL_FILTER", "REMOVE_NULL_FILTER"},
        )
        maybe_rewrite(variant="null_filter", label=label, rewrite_fn=_rewrite_null_filter)

    if not meta["checks"]:
        return sql, columns, rows, None
    meta["final_sql"] = current_sql
    meta["applied"] = any(bool(c.get("applied")) for c in meta["checks"])
    return current_sql, current_columns, current_rows, meta
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    guard_model: ModelAdapter | None = None,
    max_refines: int = 2,
) -> ToolHandler:
    ...
    Flow:
      1. Run SQL and keep the raw result table for self-review.
      2. Return review observation: question + SQL + raw result preview + checklist.
      ...
      4. Projection pruning is applied only at terminal commit time.
      5. After max_refines, ANY further answer_from_sql commits as terminal.
    """
    from experiments.exp_153_audio_asr.column_auditor import audit_columns, apply_audit

    def finalize_answer(task: PublicTask, answer: AnswerTable) -> tuple[AnswerTable, dict[str, Any] | None]:
        if auditor_model is None or len(answer.columns) <= 1:
            return answer, None
        question = question_provider() if question_provider else task.question
        try:
            audit = audit_columns(
                ...
                rows=list(answer.rows)[:50],
                model=auditor_model,
            )
            new_cols, new_rows = apply_audit(list(answer.columns), list(answer.rows), audit)
            kept_set = set(audit.keep_indices)
            meta = {
                "mode": "projection_pruner",
                "stage": "finalize",
                "kept_indices": audit.keep_indices,
                "kept_columns": [answer.columns[i] for i in audit.keep_indices],
                "dropped_columns": [c for i, c in enumerate(answer.columns) if i not in kept_set],
                "original_n_cols": len(answer.columns),
                "after_prune_n_cols": len(new_cols),
                "reason": audit.reason,
                "prompt_lang": audit.prompt_lang,
            }
            return AnswerTable(columns=new_cols, rows=new_rows), meta
        except Exception as exc:
            meta = {
                "mode": "projection_pruner",
                "stage": "finalize",
                "error": repr(exc),
                "kept_indices": list(range(len(answer.columns))),
                "kept_columns": list(answer.columns),
                "dropped_columns": [],
                "original_n_cols": len(answer.columns),
                "after_prune_n_cols": len(answer.columns),
                "reason": "projection pruner failed; kept all columns",
            }
            return answer, meta

    state = {"refines": 0, "pending_answer": None, "finalize_answer": finalize_answer}
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        try:
            ...
            # observation context.
            columns, rows = _execute_answer_sql(task, sql)
        except Exception as exc:
            return ToolExecutionResult(ok=False, content={"error": str(exc), "sql": sql})
        if not columns:
            return ToolExecutionResult(ok=False, content={"error": "SQL returned no columns.", "sql": sql})

        question = question_provider() if question_provider else task.question
        final_sql_guard_meta = None
        try:
            sql, columns, rows, final_sql_guard_meta = _apply_final_sql_guard(
                task=task,
                question=question,
                sql=sql,
                columns=columns,
        ...
        if not rows:
            return ToolExecutionResult(
                ...
                content={
                    ...
                    "final_sql_guard": final_sql_guard_meta,
                },
            )
        ...
        if final_sql_guard_meta:
            pending_meta["final_sql_guard"] = final_sql_guard_meta
        ...
        if refines_left <= 0:
            answer, prune_meta = finalize_answer(task, answer)
            content = {
            ...
            if prune_meta: content["projection_prune"] = prune_meta
            if final_sql_guard_meta: content["final_sql_guard"] = final_sql_guard_meta
...
def _make_confirm_answer(answer_handler) -> ToolHandler:
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        finalizer = state.get("finalize_answer")
        prune_meta = None
        if callable(finalizer):
            answer, prune_meta = finalizer(task, answer)
        ...
        if prune_meta: content["projection_prune"] = prune_meta
        if meta.get("final_sql_guard"): content["final_sql_guard"] = meta["final_sql_guard"]
...
def create_default_tool_registry(
    ...
    *,
    ...
    # SQL-backed tasks must go through answer_from_sql so the self-review and
    # final projection-pruner path can run.
    doc_only = False
    ...
    specs.update({
        "answer_from_sql": ToolSpec(
            ...
            description=(
                "Propose a SQL-based answer. Runs SQL and returns strict self-review "
                "feedback (= NON-terminal) so you can refine the SQL or call "
                "confirm_answer to commit. A projection pruner is applied only at "
                "terminal commit time. Up to 2 refines allowed before forced commit."
            ),
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP153_PROSE=1). Inspired by Claude "
