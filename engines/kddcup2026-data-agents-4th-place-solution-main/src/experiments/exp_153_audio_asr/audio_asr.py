"""Task-scoped video-audio ASR support for exp_153.

ASR is an auxiliary hint only. The main agent still receives the raw video when
the video/keyframe/watch levers are off, matching exp152 behavior.
"""
...
def audio_asr_cache_root() -> Path:
    root = Path(os.environ.get("EXP153_AUDIO_ASR_CACHE_ROOT", "/tmp/kobushi_exp153_audio_asr_cache"))
