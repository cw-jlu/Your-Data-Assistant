...
6. Before calling `answer`, restate your plan in the `thought` and confirm your output
   matches it (column_count and approximate row_count).
   If your current result has MORE columns than your plan's column_count, drop the extra
   columns before calling `answer`. Do not include intermediate calculation columns
   (e.g., helper columns used only for filtering or intermediate steps) in the final answer.
...
15. When computing averages or sums, do NOT silently exclude rows where a value is 0 or NULL
    unless the question explicitly says to exclude them (e.g., "ignore zero weights",
    "exclude nulls"). Include all rows by default.
...
19. Use `answer_from_sql` ONLY when ALL columns in your final answer are numeric,
    date/datetime, or boolean values. If any final column contains text values
    (names, URLs, descriptions, string identifiers), use `answer_from_python`
    instead: load the data into a DataFrame and call answer_from_python.
    Never treat an intermediate SQL result as the final answer.
