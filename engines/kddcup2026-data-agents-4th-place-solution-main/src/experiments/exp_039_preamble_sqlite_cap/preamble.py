...
# Skip row sampling for SQLite files larger than this; include DDL only.
_SQLITE_FILE_SIZE_LIMIT = 10 * 1024 * 1024  # 10 MB
...
def _sqlite_full(path: Path) -> str:
    ...
    For files larger than _SQLITE_FILE_SIZE_LIMIT, row sampling is skipped and
    only the schema (DDL) is included.  The agent is informed via an inline
    note so it knows to query the data itself with execute_python.
    """
    import pandas as pd

    rel = path.name
    file_size = path.stat().st_size
    schema_only = file_size > _SQLITE_FILE_SIZE_LIMIT
    ...
    try:
        ...
        header = f"tables: {tables}"
        if schema_only:
            header += (
                f"\n[schema-only: file is {size_mb:.0f}MB > {_SQLITE_FILE_SIZE_LIMIT // (1024*1024)}MB limit"
                " — use execute_python to query data]"
            )
