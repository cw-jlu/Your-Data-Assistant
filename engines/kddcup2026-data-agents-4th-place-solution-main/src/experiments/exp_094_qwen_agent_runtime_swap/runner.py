...
from experiments.exp_094_qwen_agent_runtime_swap.model import QwenAgentModelAdapter

from experiments.exp_094_qwen_agent_runtime_swap.agent import ReActAgent, ReActAgentConfig
from experiments.exp_094_qwen_agent_runtime_swap.config import AppConfig
from experiments.exp_094_qwen_agent_runtime_swap.preamble import build_preamble
from experiments.exp_094_qwen_agent_runtime_swap.tools.registry import (
    ToolRegistry,
...
#
# (exp_094 [arch:runtime] swap): model adapter swapped to QwenAgentModelAdapter
# via qwen_agent.llm; merge logic and all other runner logic unchanged.
...
# Derive experiment prefix from the parent directory name (e.g. "exp_094_qwen_agent_runtime_swap")
...
def build_model_adapter(config: AppConfig):
    """Build Qwen-Agent native runtime adapter (exp_094, [arch:runtime] swap).

    Same constructor kwargs as exp_086's OpenAIModelAdapter; only the wire
    layer changes. R1 official sampling (presence_penalty=1.0, top_p=0.95,
    top_k=20, max_tokens=32K, enable_thinking=True) is preserved end-to-end
    via QwenAgentModelAdapter's generate_cfg builder.
    """
    return QwenAgentModelAdapter(
        model=config.agent.model,
        api_base=config.agent.api_base,
        api_key=config.agent.api_key,
        extra_headers=config.agent.extra_headers,
        temperature=config.agent.temperature,
        presence_penalty=1.0,  # PRIORITIES.md R1 spec override
    )
