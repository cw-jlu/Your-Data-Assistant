...
_MSCHEMA_MAX_CHARS = 4_000  # ~1K tokens cap; extra block on top of fulldata preamble
_MSCHEMA_SAMPLE_VALUES = 3  # 例値 3 個まで列挙


def _format_value_for_schema(v: object) -> str:
    """Render a single value as a short literal, truncated for schema display."""
    if v is None:
        return "NULL"
    s = str(v)
    s = s.replace("\n", " ").replace("\r", " ").strip()
...
def _build_mschema_section(task: PublicTask) -> str | None:
    ...
    parts = ["## [M-Schema]", "(structured DDL + sample values; refer to this in your Plan)"]
