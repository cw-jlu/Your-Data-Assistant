...
1. Your VERY FIRST action's `thought` must contain an explicit answer plan with three parts:
   ...
   Then, also obeying Rule 12a, list the candidate column tuples by referring to `## [M-Schema]`.
   Example plan: "Answer plan: column_count=1, per_column=[<single aggregate value>], row_count=1.
                  # Plan: candidate columns are [(item_count, derived_from(COUNT(*)), INTEGER)]"
...
12a. Step 0 plan MUST include a line of this exact form, anchored at column level:
       # Plan: candidate columns are [(name1, source_table_or_csv, dtype), (name2, ..., ...)]
     Refer to the `## [M-Schema]` block in the preamble to fill `source_table_or_csv` and `dtype`.
     Pick column names from the M-Schema's `(col, dtype, e.g. ...)` tuples — do not invent new names.
     If the question implies a derived column (e.g. "ratio of X to Y"), name it explicitly and
     mark its source as `derived_from(<expr>)`.
12b. When writing SQL for `answer_from_sql`, name every column explicitly in your SELECT statement
     using the names you committed in Plan. Do not use `SELECT *` or `SELECT table.*`:
...
```json
{"thought":"Answer plan: column_count=1, per_column=[count of books published in target year], row_count=1. # Plan: candidate columns are [(book_count, derived_from(COUNT(*)), INTEGER)]. I will inspect the available files next.","action":"list_context","action_input":{"max_depth":4}}
```
