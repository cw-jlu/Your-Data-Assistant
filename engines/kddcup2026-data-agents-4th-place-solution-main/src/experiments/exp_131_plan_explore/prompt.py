...
PHASE: PLAN
Goal: build a SCHEMA-GROUNDED, DATA-VERIFIED answer plan with explicit
interpretation choices. You can — and SHOULD — explore the schema and sample
data BEFORE committing your interpretation.

Available tools in PLAN phase (exp_131): describe_data, execute_sql, read_doc,
grep, list_context, complete_phase. Use them to GROUND your plan in reality.

RECOMMENDED WORKFLOW (3-6 steps, NOT a 1-shot commit):

  Step 1: describe_data  → see the view catalog (= which tables/columns exist).
  Step 2: targeted execute_sql for ambiguity resolution. EXAMPLES:
    - If question mentions 'cost' and you see both `expense.cost` and
      `budget.spent`: query both — `SELECT MIN(cost), AVG(cost) FROM expense`
      and `SELECT MIN(spent), AVG(spent) FROM budget` — to see which one
      matches the question's scale and granularity.
    - If question uses a value-class noun like 'severe', 'lowest', 'cash
      withdrawal': sample DISTINCT values of the candidate column.
    - If `read_doc` on knowledge.md hints a domain mapping (e.g.,
      "Thrombosis: 1=most severe, 2=severe"), READ IT before assigning values.
  Step 3+: optionally read_doc/grep on knowledge.md or prose docs for
    domain-specific value mappings. Iterate on execute_sql if needed.
  Final step: commit the structured plan in `thought` AND call complete_phase.

YOUR FINAL `thought` MUST contain these labeled lines (= same format as exp_122,
but each choice now backed by EVIDENCE from steps 1-3):
  - "PLAN: column_count=<n>, per_column=[<short description per column>]."
  - "INTERPRET COLUMN: '<noun>' could be <option A> or <option B>; CHOOSING
    <pick> because <evidence from your exploration + rule citation>." (or
    "INTERPRET COLUMN: <noun> — unambiguous (<short reason>)" if obvious).
  - "INTERPRET SEMANTIC: '<phrase>' could mean <option A> (per-row min) or
    <option B> (entity-aggregate min); CHOOSING <pick> because <reason
    backed by data scale you observed in step 2>."
    (REQUIRED for superlative / numeric-relation questions.)
  - "INTERPRET DESCRIPTOR: '<entity>' returned as <pick> because <reason>."
    (REQUIRED for "what is the X / which X / name the X" questions.)
  - "SQL SKELETON: SELECT ... FROM ... JOIN ... WHERE ... [GROUP BY ...]
    [ORDER BY ...]." (= rough sketch, refined in EXPLORE/ANSWER.)

DEFAULTS when evidence is genuinely ambiguous:
  - per-row interpretation (Rule 15) — most "lowest/highest" questions ask
    for individual records, NOT entity aggregates.
  - textual descriptor (Rule 17) — return human-readable name, not opaque ID.
  - direct column match (Rule 19) — prefer column whose name matches the noun.
  - most natural English reading.

DO NOT skip the exploration if the question has ANY ambiguity (column
synonyms, aggregation level, scope of "X-related", per-unit vs total, etc.).
A grounded plan ALWAYS beats a 1-shot interpretation.
""".strip()
...
PHASE_NAME_TO_TOOLS = {
    ...
    # exp_131: PLAN phase now allows lightweight exploration (describe_data,
    # execute_sql, read_doc, grep) so the agent can GROUND its interpretation
    # in actual schema + sample values BEFORE committing to the plan.
    "plan":    ("list_context", "describe_data", "execute_sql", "read_doc",
                "grep", "complete_phase"),
