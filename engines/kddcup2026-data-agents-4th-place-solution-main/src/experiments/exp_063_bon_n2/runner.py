...
from experiments.exp_063_bon_n2.agent import ReActAgent, ReActAgentConfig
from experiments.exp_063_bon_n2.config import AppConfig
from experiments.exp_063_bon_n2.preamble import build_preamble
from experiments.exp_063_bon_n2.tools.registry import (
    ToolRegistry,
    create_default_tool_registry,
)
from experiments.exp_063_bon_n2.voting import select_best_index
...
# Derive experiment prefix from the parent directory name (e.g. "exp_063_bon_n2")
...
def _answer_table_from_payload(payload: dict[str, Any]) -> AnswerTable | None:
...
def _run_single_task_core(
    *,
    ...
    temperatures = config.bon.temperatures
    candidate_payloads: list[dict[str, Any]] = []
    for temperature in temperatures:
        candidate_model = OpenAIModelAdapter(
            model=config.agent.model,
            api_base=config.agent.api_base,
            api_key=config.agent.api_key,
            extra_headers=config.agent.extra_headers,
        ...
        candidate_tools = tools or create_default_tool_registry()
        agent = ReActAgent(
        ...
        payload["bon_temperature"] = temperature
        candidate_payloads.append(payload)

    answers = [_answer_table_from_payload(p) for p in candidate_payloads]
    best_idx = select_best_index(answers)
    best_payload = dict(candidate_payloads[best_idx])
    best_payload["bon_candidates"] = candidate_payloads
    best_payload["bon_best_index"] = best_idx
    return best_payload
