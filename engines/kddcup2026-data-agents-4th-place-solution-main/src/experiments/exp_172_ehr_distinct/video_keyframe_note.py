...
from experiments.exp_172_ehr_distinct.keyframe_cache import (
    KeyframeCacheResult,
    ensure_all_task_keyframes,
    ensure_task_keyframes,  # noqa: F401  (kept for compat: watch-video/keyframes paths)
)
...
def _max_images() -> int:
    try:
        return max(1, int(os.environ.get("EXP172_KEYFRAME_MAX_IMAGES", "18")))
    except ValueError:
        return 18
...
def extract_keyframe_note(task: PublicTask, model: ModelAdapter) -> KeyframeNoteResult | None:
    """Extract a plain text note covering EVERY video in the task.

    Single-video tasks (the demo-60 norm) behave exactly as before: one model
    call, unchanged prompt, unchanged note text. When a task ships several
    videos, each gets its own extraction call and the notes are concatenated
    under a `## video: <path>` header so no video is silently invisible."""
    all_keyframes = ensure_all_task_keyframes(task)
    notes: list[tuple[KeyframeCacheResult, KeyframeNoteResult]] = []
    for kf in all_keyframes:
        one = _extract_one_video_note(task, model, kf)
        if one is not None:
            notes.append((kf, one))
    if not notes:
        return None
    ...
    combined = "\n\n".join(
        f"## video: {kf.source_rel}\n{note.text}" for kf, note in notes
    )
    first = notes[0][1]
    return KeyframeNoteResult(
        text=combined,
        prompt_lang=first.prompt_lang,
        n_keyframes=sum(n.n_keyframes for _, n in notes),
        frames=[f for _, n in notes for f in n.frames],
        meta={
            "n_videos": len(notes),
            "videos": [n.meta for _, n in notes],
        },
    )


def _extract_one_video_note(
    task: PublicTask, model: ModelAdapter, keyframes: KeyframeCacheResult
) -> KeyframeNoteResult | None:
...
def preamble_block(note: KeyframeNoteResult, *, max_chars: int | None = None) -> str:
    if max_chars is None:
        try:
            max_chars = int(os.environ.get("EXP172_VIDEO_KEYFRAME_NOTE_MAX_CHARS", "1800"))
        except ValueError:
            max_chars = 1800
        # The cap is per-video in spirit: a multi-video note concatenates one
        # extraction per video, so scale it — otherwise video 2+ would be
        # truncated away and become invisible again.
        n_videos = int(note.meta.get("n_videos", 1) or 1)
        if n_videos > 1 and max_chars > 0:
            max_chars = max_chars * n_videos
