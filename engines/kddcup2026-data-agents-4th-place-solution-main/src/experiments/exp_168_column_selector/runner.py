...
from experiments.exp_168_column_selector.agent import ReActAgent, ReActAgentConfig
from experiments.exp_168_column_selector.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_168_column_selector.adaptive_vote import adaptive_vote
from experiments.exp_168_column_selector.config import AppConfig
from experiments.exp_168_column_selector import flags
from experiments.exp_168_column_selector.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_168_column_selector.prefix_cache import with_prefix_cache_header
from experiments.exp_168_column_selector.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP168_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP168_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP168_KEYFRAME_ROOT"):
        _os.environ["EXP168_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    # unknown general DB that may need such calc). Toggle with EXP168_MATH_OTHER_ONLY.
    ...
    trace_root = _os.environ.get("EXP168_TRACE_ROOT")
    if trace_root:
    ...
    # Final-step output-column selector (post VERIFY + vote). Projects the
    # committed answer to the question-requested output columns as the terminal
    # pipeline step — additive to the in-loop column auditor; finance/EHR only
    # (domain router), conservative keep-all on any error.
    _sel_domain = domain_route.domain if domain_route is not None else "other"
    if (
        flags.column_selector_on()
        and str(_sel_domain).strip().lower() != "other"
        and isinstance(payload.get("answer"), dict)
    ):
        try:
            from experiments.exp_168_column_selector.column_selector import (
                select_output_columns,
            )

            _ans = payload["answer"]
            _sel = select_output_columns(
                question=task.question,
                columns=list(_ans.get("columns", [])),
                rows=[list(r) for r in _ans.get("rows", [])],
                model=agent_model,
            )
            if _sel.applied:
                _ans["columns"] = _sel.columns
                _ans["rows"] = _sel.rows
            _meta = _sel.to_dict()
            _meta["domain"] = _sel_domain
            payload["column_selector"] = _meta
            try:
                import json as _json_sel

                (task_trace_dir / "column_selector.json").write_text(
                    _json_sel.dumps(_meta, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
            except Exception:
                pass
        except Exception as _sel_exc:
            payload["column_selector"] = {"applied": False, "error": repr(_sel_exc)}
