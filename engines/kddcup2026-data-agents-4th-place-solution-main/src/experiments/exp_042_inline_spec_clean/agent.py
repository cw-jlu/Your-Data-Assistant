...
from experiments.exp_042_inline_spec_clean.prompt import (
    REACT_SYSTEM_PROMPT,
    build_observation_prompt,
    build_system_prompt,
    build_task_prompt,
)
from experiments.exp_042_inline_spec_clean.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_042_inline_spec_clean.tools.registry import ToolRegistry
...
def _strip_json_fence(raw_response: str) -> str:
    ...
    # Case 3: unclosed fence — model emits ```json\n{...} without closing ```.
    # Match only to end of the fence-open line; capture the rest (avoids DOTALL gobble).
    unclosed = re.search(r"```(?:json)?\s*\n", text, re.IGNORECASE)
    if unclosed is not None:
        return text[unclosed.end():].strip()
    return text
