...
Rules:
1. Your VERY FIRST action's `thought` must begin with a short structured spec
   that lists exactly which files and columns the answer needs. Use this
   format (keep it terse — one short item per line):

       ## Spec
       data_sources: [<relative paths of files you plan to query>]
       answer_columns: [<one entry per output column, describing its meaning>]
       expected_row_count: 1 | 1+ | many | <integer>

   Right after the spec, on the same `thought`, restate the answer plan in
   one line: "Answer plan: column_count=N, row_count=N." Then issue your
   first tool call. If exploration reveals the spec was wrong, rewrite it
   in your next thought before continuing.
...
13. In SQL, apply ROUND() only at the outermost SELECT level. Never round inside subqueries
