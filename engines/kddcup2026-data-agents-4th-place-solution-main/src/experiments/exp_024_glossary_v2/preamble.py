...
def _build_doc_hint_section(files: list[Path], base: Path) -> str | None:
    """Return a hint listing plain-text files to prevent JSON-parser misuse."""
    doc_files = [p for p in files if p.suffix.lower() in _DOC_EXTENSIONS]
    if not doc_files:
        return None
    lines = ["# Text documents (plain text — use read_doc, NOT a JSON parser)"]
    for p in doc_files:
...
def _assemble(
    *,
    ...
    doc_hint = _build_doc_hint_section(files, task.context_dir)
    if doc_hint is not None:
        body_parts.append(doc_hint)
