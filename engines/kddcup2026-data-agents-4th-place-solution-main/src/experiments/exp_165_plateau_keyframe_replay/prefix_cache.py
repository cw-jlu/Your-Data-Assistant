"""Task-scoped prefix-cache routing headers for exp_165 runtime calls."""
...
def prefix_cache_enabled() -> bool:
    raw = os.environ.get("EXP165_PREFIX_CACHE", "1").strip().lower()
    if raw in _FALSE:
        return False
    if raw in _TRUE:
        return True
...
def prefix_cache_key(task_id: str | None) -> str | None:
    ...
    if not prefix_cache_enabled() or not task_id:
        return None
    namespace = os.environ.get("EXP165_PREFIX_CACHE_NAMESPACE", "kobushi-phase2-exp165").strip()
    if not namespace:
        return task_id
