"""Task-scoped video keyframe cache for exp_165.
...
def keyframe_cache_root() -> Path:
    root = Path(os.environ.get("EXP165_KEYFRAME_ROOT", "/tmp/kobushi_exp165_keyframe_cache"))
...
def _settings() -> dict[str, object]:
    ...
    return {
        "sample_fps": _float("EXP165_KEYFRAME_SAMPLE_FPS", 1.0),
        "hash_thresh": _int("EXP165_KEYFRAME_HASH_THRESH", 80),
        "diff_thresh": _float("EXP165_KEYFRAME_DIFF_THRESH", 7.0),
        "max_frames": _int("EXP165_KEYFRAME_MAX_IMAGES", 50),
        # Plateau detection: probe densely, keep one settled frame per stable
        # slide/panel, and skip transition or animation frames.
        "probe_fps": _float("EXP165_PLATEAU_PROBE_FPS", 5.0),
        "stable_eps": _float("EXP165_PLATEAU_STABLE_EPS", 2.0),
        "min_stable_s": _float("EXP165_PLATEAU_MIN_STABLE_S", 1.2),
        "settle_back_s": _float("EXP165_PLATEAU_SETTLE_BACK_S", 0.3),
    }
...
def ensure_task_keyframes(task: PublicTask, *, force: bool = False) -> KeyframeCacheResult | None:
    ...
    probes: list[tuple[float, int, object]] = []
    sampled = 0
    frame_idx = 0
    while True:
        ok, frame = cap.read()
    ...
    targets = [max(start, end - settle_back_s) for start, end in plateaus]
    if not targets and probes:
        n = min(max(1, int(settings["max_frames"])), len(probes))
        targets = [
            probes[int(i)][0]
            for i in np.linspace(0, len(probes) - 1, n).round().astype(int)
        ]
    ...
    if len(targets) > max_frames and plateaus:
        order = sorted(
            range(len(plateaus)),
            key=lambda i: plateaus[i][1] - plateaus[i][0],
            reverse=True,
        )
    ...
    for timestamp in targets:
        cap.set(cv2.CAP_PROP_POS_FRAMES, int(round(timestamp * src_fps)))
        ok, frame = cap.read()
        if ok:
