...
from experiments.exp_122_column_auditor.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_122_column_auditor.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_122_column_auditor.column_auditor import audit_columns, apply_audit
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        # Note: LIMIT 1 guard removed (= regressed task_80 in focused test by
        # removing the LIMIT 1 mask without fixing the underlying filter).
        # Math advisor's filter-back formula handles superlative ties instead.
        if rule_violations:
            return ToolExecutionResult(
                ok=False,
