...
from experiments.exp_047_skill_split.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_047_skill_split.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_047_skill_split.tools.sqlite import (
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "read_skill": ToolSpec(
            name="read_skill",
            description=(
                "Fetch the full guidance for a named skill. "
                "Use before complex queries (tie-aware, schema-first, explicit-select, sql-round) "
                "or after errors (self-debug)."
            ),
            input_schema={"name": "tie-aware"},
        ),
    }
    handlers = {
        "answer": _answer,
...
def _read_skill(_task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    if not path.exists():
        ...
        return ToolExecutionResult(
            ok=False,
            content={"error": f"Skill '{name}' not found.", "available": available},
        )
    return ToolExecutionResult(ok=True, content={"content": path.read_text()})
