...
from experiments.exp_110_plan_solve_verify.config import AppConfig
from experiments.exp_110_plan_solve_verify.preamble import build_preamble
from experiments.exp_110_plan_solve_verify.tools.registry import (
    ToolRegistry,
...
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6,)  # single-attempt for exp_110 PSV smoke / dev iteration
...
def _run_single_task_core(
    *,
    ...
    """exp_110: run the 4-phase Plan→Solve→Verify→Repair pipeline.

    pipeline.run_psv_pipeline returns a payload that already contains
    answer / steps / preamble / preamble_metadata + psv_metadata.
    """
    from experiments.exp_110_plan_solve_verify.pipeline import run_psv_pipeline

    public_dataset = DABenchPublicDataset(config.dataset.root_path)
    task = public_dataset.get_task(task_id)
    return run_psv_pipeline(
        task=task,
        config=config,
