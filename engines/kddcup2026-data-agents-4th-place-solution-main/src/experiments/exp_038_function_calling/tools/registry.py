...
from experiments.exp_038_function_calling.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_038_function_calling.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_038_function_calling.tools.sqlite import (
...
# OpenAI function-calling JSONSchemas — one per tool. These are used when the
# agent talks to the model via native function-calling instead of text-based JSON
# parsing. Keeping them here next to `ToolSpec` so per-experiment tool tweaks can
# also tweak schemas as needed.
_OPENAI_TOOL_PARAMETERS: dict[str, dict[str, Any]] = {
    "answer": {
        "type": "object",
        "properties": {
            "columns": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Column header names (string list).",
            },
            "rows": {
                "type": "array",
                "items": {"type": "array", "items": {}},
                "description": "Answer rows: a list of lists. Each inner list has one cell per column.",
            },
        },
    ...
    "answer_from_python": {
        ...
        "properties": {
            "code": {
                "type": "string",
                "description": (
                    "Python code that sets either `answer_df` (a pandas DataFrame) or "
                    "`answer_table = {'columns': [...], 'rows': [[...], ...]}`."
                ),
            },
        },
        "required": ["code"],
    ...
    "answer_from_sql": {
        "type": "object",
        "properties": {
            "path": {"type": "string", "description": "Relative path to the .sqlite/.db file."},
            "sql": {"type": "string", "description": "Read-only SQL query whose result is the answer."},
        },
        "required": ["path", "sql"],
    },
    "execute_context_sql": {
    ...
    "execute_python": {
        "type": "object",
        "properties": {
            "code": {"type": "string", "description": "Python code; stdout is returned."},
        },
        "required": ["code"],
    },
    "inspect_sqlite_schema": {
...
class ToolRegistry:
    ...
    def to_openai_tools(self) -> list[dict[str, Any]]:
        """Emit the registry as a list of OpenAI function-calling tool definitions.
        ...
        Used by ReAct agents that talk via native function-calling rather than
        text-based JSON parsing.
        """
        out: list[dict[str, Any]] = []
        for name in sorted(self.specs):
            spec = self.specs[name]
            params = _OPENAI_TOOL_PARAMETERS.get(name, {"type": "object", "properties": {}})
            out.append(
                {
                    "type": "function",
                    "function": {
