...
from experiments.exp_039_answer_validation.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_039_answer_validation.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_039_answer_validation.tools.sqlite import (
...
# ── Answer validation (exp_039) ────────────────────────────────────────────
# Every answer submission goes through `_validate_answer`. Suspicious shapes
# (empty, all-null, all-uniform, too many rows) get rejected on the first 1-2
# attempts so the agent has a chance to self-correct. After
# ANSWER_VALIDATION_MAX_REJECTS failures we soft-accept to avoid creating
# missing predictions.

ANSWER_VALIDATION_MAX_REJECTS = 2  # 3rd submission is always accepted

# Per-task attempt counter, keyed by task_id. Reset implicitly when each task
# starts (task_ids are unique across the benchmark run).
_validation_attempts: dict[str, int] = {}


def _is_null_cell(cell: Any) -> bool:
    if cell is None:
        return True
    if isinstance(cell, str):
        return cell.strip() == "" or cell.strip().lower() in {"null", "none", "nan", "nat", "<na>"}
    return False


def _validate_answer(
    task: PublicTask, columns: list[str], rows: list[list[Any]]
) -> tuple[bool, str]:
    """Return (ok, hint). If ok=False, the agent gets `hint` and a chance to retry."""
    # Empty
    if not rows:
        return False, (
            "Empty answer (zero rows). The query likely missed the actual matches; "
            "verify the WHERE clause / JOIN keys / value casing."
        )
    # All-null cells
    if all(all(_is_null_cell(c) for c in row) for row in rows):
        return False, (
            "All cells are null/empty/None. The columns selected probably don't exist "
            "or your filter eliminated every value — re-check column names and filters."
        )
    # Way too many rows (likely missing aggregation)
    if len(rows) > 1000:
        return False, (
            f"Way too many rows ({len(rows)}). The question almost certainly expects "
            "an aggregation (COUNT/SUM/GROUP BY) you forgot."
        )
    # Uniform: 3+ rows, all identical → suspicious GROUP BY / DISTINCT bug
    if len(rows) >= 3:
        signatures = {tuple(str(c) for c in row) for row in rows}
        if len(signatures) == 1:
            return False, (
                f"All {len(rows)} rows are identical. You likely forgot DISTINCT or a "
                "GROUP BY, or you're joining the wrong key."
            )
    return True, ""
...
def _maybe_reject(
    ...
    """Return a non-terminal rejection if the answer fails validation AND we
    haven't hit the retry budget. Returns None to mean "go ahead and submit"."""
    ok, hint = _validate_answer(task, columns, rows)
    if ok:
        return None
    rejects = _validation_attempts.get(task.task_id, 0)
    if rejects >= ANSWER_VALIDATION_MAX_REJECTS:
        # Soft accept — do not block; the agent has tried enough.
        return None
    _validation_attempts[task.task_id] = rejects + 1
    return ToolExecutionResult(
        ok=False,
        content={
            "status": "rejected_by_validator",
            "attempt": rejects + 1,
            "max_attempts_before_soft_accept": ANSWER_VALIDATION_MAX_REJECTS + 1,
            "hint": hint,
...
def _answer_from_python(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    answer_rows = [list(r) for r in rows]
    rejection = _maybe_reject(task, answer_columns, answer_rows)
    if rejection is not None:
        return rejection
    answer = AnswerTable(columns=answer_columns, rows=answer_rows)
    return ToolExecutionResult(
        ok=True,
        content={
            "status": "submitted",
...
def _answer_from_sql(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    rejection = _maybe_reject(task, columns, rows)
    if rejection is not None:
        return rejection
...
def _answer(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    answer_columns = list(columns)
    rejection = _maybe_reject(task, answer_columns, normalized_rows)
    if rejection is not None:
        return rejection
    answer = AnswerTable(columns=answer_columns, rows=normalized_rows)
    return ToolExecutionResult(
        ok=True,
        content={
            "status": "submitted",
