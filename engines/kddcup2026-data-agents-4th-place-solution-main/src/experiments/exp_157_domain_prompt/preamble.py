...
from experiments.exp_157_domain_prompt.profile import (
    csv_profile_text,
    json_array_profile,
...
def _doc_full(task: PublicTask, path: Path) -> str:
    ...
    if suffix == ".pdf":
        try:
            from experiments.exp_157_domain_prompt.pdf_text_cache import (
...
def _build_prose_index_section(task: PublicTask) -> str | None:
    ...
    if not flags.prose_index_on():
        return None
    try:
        from experiments.exp_157_domain_prompt.prose_indexer import index_cache_root
    except Exception:
        return None
    ...
    try:
        max_chars = int(os.environ.get("EXP157_PROSE_INDEX_MAX_CHARS", "50000"))
    except Exception:
        max_chars = 50_000
...
def build_preamble(
    ...
    *,
    ...
    exp_157 lever ③: when `video_summary` is provided (the harness pre-extracts
    ...
    try:
        from experiments.exp_157_domain_prompt.tools.duckdb_unified import describe_catalog
    ...
    # exp_157 lever ③: pre-extracted video summary (authoritative for video-defined
