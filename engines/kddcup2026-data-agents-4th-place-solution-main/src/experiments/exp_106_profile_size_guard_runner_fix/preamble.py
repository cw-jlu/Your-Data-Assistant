...
from experiments.exp_106_profile_size_guard_runner_fix.profile import (
    csv_profile_text,
    json_array_profile,
...
# Budget cap on profile chars (derived statistics) emitted per file.
# Rationale: 1 raw byte ≈ 1/25 profile char (80–200 chars/col × 30–100 cols ≈ 8 KB).
# When a file's estimated profile would exceed this, drop the profile and keep
# only the raw section so the agent can still read the actual data.
_PROFILE_BUDGET = 8_000
...
def _profile_budget_guard(raw_size: int) -> bool:
    """Return True when emitting a profile for this file is within budget.

    Estimate: 1 raw byte ≈ 1/25 profile chars (profile is summary statistics,
    far smaller than raw). Files whose profile would exceed _PROFILE_BUDGET are
    served raw-only so the agent still has access to the actual data.
    """
    estimated_profile_chars = raw_size // 25
    return estimated_profile_chars <= _PROFILE_BUDGET


def _csv_full(path: Path, *, profile_enabled: bool = True) -> str:
    ...
    emit_profile = profile_enabled and _profile_budget_guard(len(raw))
    profile = csv_profile_text(path) if emit_profile else None
    ...
    # No profile (either suppressed by budget guard or parse error) — clip raw.
    clipped, _ = _truncate_chars(raw, _CSV_MAX_CHARS)
    reason = "profile budget exceeded" if not emit_profile else "profile parse error"
    return f"## [CSV] {rel}\n[{reason} — raw clip below]\n{clipped}"


def _sqlite_full(path: Path, *, profile_enabled: bool = True) -> str:
    ...
    try:
        ...
        for table in tables:
            ...
            # Profile (per-column stats) — suppressed when file is too large.
            emit_profile = profile_enabled and _profile_budget_guard(path.stat().st_size)
            profile_lines = sqlite_table_profile(conn, table) if emit_profile else []
            ...
            profile_section = (
                "[profile]\n" + "\n".join(profile_lines) + "\n"
                if profile_lines
                else "[profile suppressed — file too large]\n"
            )
            parts.append(
                f"### table: {table}\nDDL:\n{ddl_text}\n"
                + profile_section
...
def _json_full(path: Path, *, profile_enabled: bool = True) -> str:
    ...
    if len(raw) <= _JSON_MAX_CHARS:
        ...
        # If it's a list-of-dicts and within profile budget, prepend the profile for clarity.
        if emit_profile:
            try:
                obj = json.loads(raw)
    ...
    if emit_profile and isinstance(obj, list) and obj and isinstance(obj[0], dict):
        profile = json_array_profile(obj)
        if profile:
            return f"## [JSON] {rel}\n[profile — file too large for full inclusion]\n{profile}"
...
def build_preamble(
    ...
    *,
    ...
    for path in files:
        ...
        # Guard profile emission per-file: skip profile for files whose estimated
        # profile size would exceed _PROFILE_BUDGET (e.g. 250 KB JSON files).
        profile_ok = _profile_budget_guard(path.stat().st_size)
        if suffix == ".csv":
            labeled.append(("csv", _csv_full(path, profile_enabled=profile_ok)))
        elif suffix in {".db", ".sqlite", ".sqlite3"}:
            labeled.append(("sqlite", _sqlite_full(path, profile_enabled=profile_ok)))
        elif suffix == ".json":
            labeled.append(("json", _json_full(path, profile_enabled=profile_ok)))
