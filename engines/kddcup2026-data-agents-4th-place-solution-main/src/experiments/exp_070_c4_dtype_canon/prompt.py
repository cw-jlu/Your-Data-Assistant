...
18. Normalize output values before calling any answer tool to match official scoring:
    - Numbers: round to exactly 2 decimal places with ROUND_HALF_UP.
      SQL: ROUND(col, 2) in the outermost SELECT (Rule 14 still applies).
      Python: use round(val, 2) or pandas Series.round(2).
    - Null / missing values: use empty string "".
      Never output "null", "None", "NaN", "N/A", "NA", "nat", or "<NA>" as cell values.
    - Dates and datetimes: use ISO 8601 format ("YYYY-MM-DD" or "YYYY-MM-DDTHH:MM:SS").
      Pandas: df['col'].dt.strftime('%Y-%m-%d'). SQLite: strftime('%Y-%m-%d', col).
    - String / ID / free-text columns: do NOT alter — only numeric, null, and date
      columns are subject to this rule.
