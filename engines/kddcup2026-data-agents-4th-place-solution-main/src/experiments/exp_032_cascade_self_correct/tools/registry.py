...
from experiments.exp_032_cascade_self_correct.tools.answer_validator import (
    AnswerValidatorState,
    validate_answer,
)
from experiments.exp_032_cascade_self_correct.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_032_cascade_self_correct.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_032_cascade_self_correct.tools.sqlite import (
...
def create_default_tool_registry() -> ToolRegistry:
    ...
    def _validated(
        ...
        if not result.is_terminal or result.answer is None:
            return result
        ok, is_terminal, hint = validate_answer(
    ...
    handlers = {
        "answer": lambda task, ai: _validated(_answer, task, ai),
    ...
    return ToolRegistry(specs=specs, handlers=handlers, _spec_recorder=validator_state.record_spec)
