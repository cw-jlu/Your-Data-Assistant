...
from experiments.exp_118_empty_block.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
)
from experiments.exp_118_empty_block.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _answer_from_sql(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    # Minimal-diff addition: block commit if the answer is empty (= 0 rows).
    # Empty answers are usually filter-too-restrictive; force the agent to
    # reconsider with the data layout in mind. This is non-terminal.
    if not rows:
        return ToolExecutionResult(
            ok=False,
            content={
                "error": "answer_from_sql returned 0 rows — your filter is likely "
                         "too restrictive or the column / value format does not "
                         "match the data. Run execute_sql to count rows for each "
                         "filter condition individually, then relax the most "
                         "restrictive one and retry.",
                "sql": sql,
            },
        )
