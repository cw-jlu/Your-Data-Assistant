...
def _create_json_view(conn: duckdb.DuckDBPyConnection, json_path: Path) -> str | None:
    ...
    try:
        ...
        probe = conn.execute(
            f"SELECT * FROM read_json_auto('{json_path.as_posix()}', maximum_object_size=104857600) LIMIT 1"
        ).fetchall()
        ...
        if "records" in cols:
            ...
            conn.execute(
                ...
                SELECT r.* FROM read_json_auto('{json_path.as_posix()}', maximum_object_size=104857600) AS j,
                       UNNEST(j.records) AS t(r)"""
            )
        else:
            conn.execute(
                f"CREATE OR REPLACE VIEW {view_name} AS SELECT * FROM read_json_auto('{json_path.as_posix()}', maximum_object_size=104857600)"
            )
        return view_name
    except duckdb.Error as exc:
        return f"!{view_name}: {exc}"
...
def get_catalog(context_dir: Path) -> list[dict[str, Any]]:
    ...
    return _CATALOG_CACHE[str(context_dir.resolve())]
