...
from experiments.exp_065_kira_double_confirm.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_065_kira_double_confirm.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_065_kira_double_confirm.tools.sqlite import (
...
_DOUBLE_CONFIRM_CHECKLIST = """\
[Checklist before final answer]
- Does column count match your plan? [TODO/DONE]
- Does row count match what the question implies? [TODO/DONE]
- Are you sure of column ordering / cell values? [TODO/DONE]
Restate the answer in your next thought after running through the checklist.
Then call the SAME answer tool with the same arguments to confirm."""
...
def create_default_tool_registry() -> ToolRegistry:
    # Shared state across all three answer tools for this registry instance.
    # Using a dict avoids closure rebinding issues with plain bool variables.
    _seen: dict[str, bool] = {"confirmed": False}

    def _wrap_answer(
        inner: ToolHandler,
    ) -> ToolHandler:
        """Return a handler that requires two consecutive calls before accepting.

        First call: return checklist prompt with is_terminal=False.
        Second call: delegate to inner handler and accept the result.
        After any successful submission, reset state so a retry also works.
        """
        def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
            if not _seen["confirmed"]:
                _seen["confirmed"] = True
                return ToolExecutionResult(
                    ok=True,
                    content={"checklist": _DOUBLE_CONFIRM_CHECKLIST},
                    is_terminal=False,
                )
            _seen["confirmed"] = False
            return inner(task, action_input)
    ...
    handlers = {
        "answer": _wrap_answer(_answer),
        "answer_from_python": _wrap_answer(_answer_from_python),
        "answer_from_sql": _wrap_answer(_answer_from_sql),
