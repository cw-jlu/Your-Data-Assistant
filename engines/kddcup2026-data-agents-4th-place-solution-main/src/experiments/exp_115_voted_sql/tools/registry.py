...
from experiments.exp_115_voted_sql.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
def _make_voted_answer_handler(
    ...
    tie_break_model: ModelAdapter | None,
    k: int = 5,
) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        from experiments.exp_115_voted_sql.voted_sql import voted_answer_from_sql
        sub_q = action_input.get("sub_question") or action_input.get("question") or action_input.get("q")
        if not sub_q:
            return ToolExecutionResult(ok=False, content={"error": "voted_answer_from_sql requires sub_question"})
        result = voted_answer_from_sql(
            task=task, sub_question=str(sub_q), model=voter_model,
            k=int(action_input.get("k", k)),
            tie_break_model=tie_break_model,
            tie_fallback="first_attempt",
        )
        if not result.ok:
            return ToolExecutionResult(
                ok=False,
...
def create_default_tool_registry(
    ...
    if voter_model is None:
        raise ValueError(
            "exp_115 registry requires a voter_model — voted_answer_from_sql needs an LLM at temp=1.0."
        )
    specs = {
        "voted_answer_from_sql": ToolSpec(
            name="voted_answer_from_sql",
            description=(
                "Terminal action. Specify a precise sub_question describing the final answer. "
                f"The system internally generates {k} candidate SQL queries at temperature 1.0, "
                "executes all, and majority-votes on the result rows (= ReFoRCE-style consensus). "
                "The most-voted result IS the submitted answer. The agent need NOT write SQL "
                "itself — only describe the intent precisely (columns, filters, aggregation)."
            ),
            input_schema={
                "sub_question": "Plain-English description of the final answer (= columns, filters, aggregation).",
                "k": k,
            },
        ),
    ...
    handlers = {
        "voted_answer_from_sql": _make_voted_answer_handler(voter_model, tie_break_model, k=k),
