...
from experiments.exp_084_qwen3_think_flag.agent import ReActAgent, ReActAgentConfig
from experiments.exp_084_qwen3_think_flag.config import AppConfig
from experiments.exp_084_qwen3_think_flag.preamble import build_preamble
from experiments.exp_084_qwen3_think_flag.tools.registry import (
    ToolRegistry,
...
_ATTEMPT_TEMPS: tuple[float, ...] = (0.0, 0.0, 0.0)  # diversification
...
class ThinkingModelAdapter(OpenAIModelAdapter):
    """OpenAIModelAdapter subclass that explicitly enables Qwen3 thinking mode.

    Passes ``extra_body={"chat_template_kwargs": {"enable_thinking": True}}``
    to the vLLM OpenAI-compat API so the Qwen3 chat template activates the
    ``<think>`` block unconditionally, even in multi-turn sessions where the
    default template might revert to non-thinking mode.
    """

    _THINK_EXTRA_BODY: dict = {"chat_template_kwargs": {"enable_thinking": True}}
    ...
    def complete(self, messages):  # type: ignore[override]
        ...
        try:
            response = self._client().chat.completions.create(
                ...
                extra_body=self._THINK_EXTRA_BODY,
            )
