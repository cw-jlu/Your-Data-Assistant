...
_DEC_QUANT = Decimal("0.01")


def _quant_2dp(v: Any) -> float | None:
    """Decimal.quantize(0.01, ROUND_HALF_UP) — exact match for the official
    scorer. Python's built-in round() uses banker's rounding and operates on
    binary floats, so values like 60.775 / 1.125 / 0.015 disagree with the
    scorer at half-boundaries. This function eliminates that divergence."""
    if v is None:
        return None
    try:
        return float(Decimal(str(v)).quantize(_DEC_QUANT, rounding=ROUND_HALF_UP))
...
def _norm_value(v: Any) -> tuple:
    ...
    # at 2dp (ROUND_HALF_UP). See _quant_2dp for the Python-round bug fix.
    ...
    for group in by_cols.values():
        ...
        for other_sig, other in sigs_sorted[1:]:
            ...
            if len(other_rows) < len(cand_rows):
                # Strictly smaller normalized rowset → can't contain cand.
                is_proper_subset_of_larger = False
                break
            if len(other_rows) == len(cand_rows) and other_rows != cand_rows:
                # Equal size but different content → distinct answers, fail.
                is_proper_subset_of_larger = False
                break
