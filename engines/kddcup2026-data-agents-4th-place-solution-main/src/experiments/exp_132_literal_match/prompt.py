...
RULE OF LITERAL SCHEMA MATCH (= exp_132, strongest priority):
For each noun N in the question:
  1. Search every table for a column whose name literally matches N
     (= or differs only by case / singular ↔ plural).
  2. If exactly one literal match exists → USE IT.
  3. If multiple tables share the literal column → pick the table whose
     entity is the noun's grammatical owner (= "X's Y" → table X).
  4. Use a semantic synonym ONLY when no literal match exists.

EXAMPLES (= different schema from your task):
  - Q "tip on each ride" with {ride.tip_amount, payment.amount}
    → use `ride.tip_amount` (literal). Do NOT substitute `payment.amount`.
  - Q "genre of the album" with {album.genre, track.description}
    → use `album.genre`. Do NOT substitute `track.description`.
  - Q "author's birthplace" with {authors.birthplace, reviews.location}
    → use `authors.birthplace` (possessive matches table).

SUPERLATIVE & TIE RULE:
For "lowest / highest / min / max":
  - Use filter-back: `WHERE col = (SELECT MIN/MAX col FROM t)`.
  - Never use `ORDER BY col LIMIT 1` (= silently drops tied rows).
