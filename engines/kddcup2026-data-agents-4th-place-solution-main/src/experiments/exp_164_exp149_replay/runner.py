...
from experiments.exp_164_exp149_replay.agent import ReActAgent, ReActAgentConfig
from experiments.exp_164_exp149_replay.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_164_exp149_replay.adaptive_vote import adaptive_vote
from experiments.exp_164_exp149_replay.config import AppConfig
from experiments.exp_164_exp149_replay import flags
from experiments.exp_164_exp149_replay.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_164_exp149_replay.prefix_cache import with_prefix_cache_header
from experiments.exp_164_exp149_replay.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP164_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP164_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp164_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP164_KEYFRAME_ROOT"):
        _os.environ["EXP164_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp164_keyframe_cache" / run_name
        )


def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp149 bench script."""
...
def _run_single_task_core(
    *,
    ...
    trace_root = _os.environ.get("EXP164_TRACE_ROOT")
    if trace_root:
    ...
    payload["exp149_runtime_metadata"] = {
