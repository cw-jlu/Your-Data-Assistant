...
from experiments.exp_109_hard_task_triage.agent import ReActAgent, ReActAgentConfig
from experiments.exp_109_hard_task_triage.config import AppConfig
from experiments.exp_109_hard_task_triage.preamble import build_preamble
from experiments.exp_109_hard_task_triage.tools.registry import (
    ToolRegistry,
...
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6,)  # single-attempt for new-axis exploration


# ============================================================================
# Hard-task triage: adaptive max_steps for preamble-heavy tasks
# ============================================================================

# exp_074 demonstrated that budget expansion yields +0.045 on λ0.5.
# This classifier localises that expansion to tasks whose preamble exceeds the
# threshold, so easy tasks bear no latency cost while hard tasks get deeper
# reasoning budget. Disable via _TRIAGE_ENABLED=False to reproduce exp_086
# single-attempt baseline exactly.
_HARD_PREAMBLE_CHARS: int = 30_000
_HARD_MAX_STEPS: int = 48
_TRIAGE_ENABLED: bool = True


def _is_hard_task(preamble_text: str) -> bool:
    """Return True when the preamble is long enough to indicate a hard task.

    Uses raw character count as a proxy for schema/data complexity.
    _TRIAGE_ENABLED=False makes this unconditionally return False, restoring
    vanilla exp_086 single-attempt behaviour for regression comparison.
    """
    if not _TRIAGE_ENABLED:
        return False
    return len(preamble_text) >= _HARD_PREAMBLE_CHARS
...
def _run_single_task_core(
    *,
    ...
    is_hard = _is_hard_task(preamble_result.text)
    effective_max_steps = _HARD_MAX_STEPS if is_hard else config.agent.max_steps
    if is_hard:
        print(
            f"[triage] {task_id} → hard"
            f" (preamble={len(preamble_result.text)}c,"
            f" max_steps {config.agent.max_steps}→{effective_max_steps})",
            flush=True,
        )
    ...
    payload["triage"] = {
        "is_hard": is_hard,
        "preamble_chars": len(preamble_result.text),
