...
from experiments.exp_128_abl_b_nophased.agent import ReActAgent, ReActAgentConfig
from experiments.exp_128_abl_b_nophased.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_128_abl_b_nophased.adaptive_vote import adaptive_vote
from experiments.exp_128_abl_b_nophased.config import AppConfig
from experiments.exp_128_abl_b_nophased.preamble import build_preamble
from experiments.exp_128_abl_b_nophased.tools.registry import (
    ToolRegistry,
...
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6,)  # ABLATION B: n=1 for fair comparison
...
def _run_single_task_core(
    *,
    ...
    # ABLATION B: no phased agent. Use plain ReActAgent (= single phase, all
    # tools available throughout, single system prompt). All other v6 features
    # (= column auditor, adaptive vote, prose-file guard) kept intact.
    from experiments.exp_128_abl_b_nophased.agent import ReActAgent as _ReActAgent
    from experiments.exp_128_abl_b_nophased.agent import ReActAgentConfig as _ReActAgentConfig
    agent = _ReActAgent(
        model=agent_model,
        tools=tools,
        config=_ReActAgentConfig(
