...
from experiments.exp_161_tool_recovery_trace.agent import (
    parse_model_step,
)
from experiments.exp_161_tool_recovery_trace.prompt import (
    allowed_tools_for_phase,
    build_observation_prompt,
    build_phased_system_prompt,
    build_task_prompt,
)
from experiments.exp_161_tool_recovery_trace.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_161_tool_recovery_trace.tools.registry import ToolRegistry
from experiments.exp_161_tool_recovery_trace import flags
from experiments.exp_161_tool_recovery_trace.source_router import route_answer_source
...
# exp_161 lever ② (prose_on): detect a DuckDB catalog-404 so we can redirect
        ...
        if current == "explore" and next_phase == "answer":
            ...
            if watch_video_required and not watched_video_ok:
                return False, current, (
                    ...
                    "EXP161_WATCH_VIDEO=1, but no successful watch_video call has "
...
class PhasedReActAgent:
    ...
    def _find_keyframe_paths(task: PublicTask) -> list:
        ...
        root = _P(os.environ.get("EXP161_KEYFRAME_ROOT", "/tmp/kobushi_exp161_keyframe_cache"))
        ...
        try:
            max_images = max(1, int(os.environ.get("EXP161_KEYFRAME_MAX_IMAGES", "18")))
        except ValueError:
            max_images = 18
    ...
    def _build_messages(
        ...
        if flags.keyframes_on():
            ...
            # exp_161 lever ③ (video_on): the raw video is NOT attached — a
    ...
    def run(self, task: PublicTask) -> AgentRunResult:
        ...
        for step_index in range(1, self.config.max_steps + 1):
            ...
            try:
                ...
                recover_to_explore = (
                    model_step.action == "answer_from_sql"
                    and not tool_result.ok
                    and phase in {"answer", "verify"}
                )
                if recover_to_explore:
                    observation["recovery"] = {
                        "from_phase": phase,
                        "to_phase": "explore",
                        "reason": (
                            "[RECOVERY] answer_from_sql failed; returning to "
                            "EXPLORE so you can debug with execute_sql/read_doc "
                            "before proposing a new final SQL."
                        ),
                    }
                # exp_161 lever ② (prose_on): a catalog-404 on a knowledge-named
                ...
                if recover_to_explore:
                    _log(
                        f"[{task.task_id}] phase {observation['recovery']['from_phase']} "
                        "→ explore (auto recovery after failed answer_from_sql)"
                    )
                    phase = "explore"
