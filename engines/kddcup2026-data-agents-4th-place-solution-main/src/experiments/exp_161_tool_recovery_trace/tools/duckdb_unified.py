...
def normalize_sql(sql: str) -> tuple[str, dict[str, Any]]:
    """Normalize harmless SQL dialect quirks before DuckDB execution.

    This intentionally does not rewrite query meaning. It only converts
    backtick-quoted identifiers and smart quote delimiters that the model often
    emits after reading Chinese schemas.
    """
    out: list[str] = []
    i = 0
    state = "normal"
...
def execute_sql(context_dir: Path, sql: str, limit: int | None = 200) -> dict[str, Any]:
    ...
    """
    sql, normalization_meta = normalize_sql(sql)
    normalized_lower = sql.lstrip().lower()
    if not normalized_lower.startswith(("select", "with", "pragma", "describe", "show", "explain")):
        raise ValueError(
