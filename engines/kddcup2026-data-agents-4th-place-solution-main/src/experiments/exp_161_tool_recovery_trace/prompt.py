...
explore queries.
If a final SQL proposal failed and the runtime returned you here, debug with
execute_sql/read_doc before proposing answer_from_sql again.
""".strip()
...
# ====================================================================
# exp_161 LEVER ① — answer_shape_guard (DISTINCT explicit-only +
...
=== exp_161 ANSWER-SHAPE OVERRIDE (supersedes the DISTINCT rule above) ===
- Do not use SELECT DISTINCT or COUNT(DISTINCT ...) unless the question
  explicitly says distinct/unique/deduplicated/去重/唯一/不重复. Preserve source
  duplicate rows by default. If you worry about JOIN fan-out, rewrite with
  EXISTS or a pre-aggregated subquery instead of DISTINCT.
...
=== exp_161 ANSWER-SHAPE OVERRIDE (supersedes the DUPLICATE SANITY step above) ===
- DUPLICATE SANITY: source duplicates are real unless the question explicitly
  asks for distinct/unique/deduplicated/去重/唯一/不重复. Do not fix JOIN fan-out
  with DISTINCT; rewrite with EXISTS or a pre-aggregated subquery.
- Do NOT add `WHERE ... IS NOT NULL` / `<> ''` at this stage to "clean" the
  result — blank/NULL rows are part of a plain list answer."""

# ====================================================================
# exp_161 LEVER ② — prose_doc_forced_read, H4a parse protocol. Appended to
...
def build_phased_system_prompt(
    ...
    exp_161 ablation: when EXP161_ANSWER_SHAPE / EXP161_PROSE are on, the
