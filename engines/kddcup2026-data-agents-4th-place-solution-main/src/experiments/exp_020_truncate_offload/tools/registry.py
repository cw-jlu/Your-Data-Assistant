...
from experiments.exp_020_truncate_offload.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_020_truncate_offload.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_020_truncate_offload.tools.sqlite import (
...
def _read_offloaded(_: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    path = str(action_input["path"])
    start_char = int(action_input.get("start_char", 0))
    length = int(action_input.get("length", 2000))
    return ToolExecutionResult(
        ok=True, content=read_offloaded(path, start_char=start_char, length=length)
    )
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "read_offloaded": ToolSpec(
            name="read_offloaded",
            description=(
                "Read a slice of a previously offloaded large file. "
                "When read_csv / read_json / read_doc returns offload_path, the full content "
                "was too large and has been saved to that path. "
                "Use start_char and length to paginate through it."
            ),
            input_schema={"path": "/tmp/kobushi_offload_...", "start_char": 0, "length": 2000},
        ),
    }
    handlers = {
        "answer": _answer,
        ...
        "read_offloaded": _read_offloaded,
    }
