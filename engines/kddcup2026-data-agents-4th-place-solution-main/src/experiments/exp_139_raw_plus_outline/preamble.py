...
from experiments.exp_139_raw_plus_outline.profile import (
    csv_profile_text,
    json_array_profile,
...
def _doc_outline(text: str) -> str:
    """Build section tree outline from markdown headers (= structure-only, no leak)."""
    lines = text.split("\n")
    n_lines = len(lines)
    headers: list[dict] = []
    ...
    for idx, h in enumerate(headers):
        ...
        h["line_end"] = end_line
    if not headers:
        return f"(no section headers, {n_lines} lines total)"
    out = [f"({n_lines} lines, {len(headers)} sections)"]
    for h in headers:
        indent = "  " * max(0, h["level"] - 2)
        title = h["title"][:80]
        out.append(f"{indent}L{h['line_start']:>4}-{h['line_end']:>4}  {title}")
    return "\n".join(out)


def _doc_full(path: Path) -> str:
    """exp_139: emit OUTLINE (section tree + line ranges) BEFORE raw clipped content.
    Outline guides agent's read_doc navigation, raw provides safety-net context.
    """
    rel = path.name
    try:
        raw = path.read_text(encoding="utf-8", errors="replace")
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_139_raw_plus_outline.tools.duckdb_unified import describe_catalog
    ...
    try:
        from experiments.exp_139_raw_plus_outline.math_advisor import generate_formula
