...
11. Before your FIRST tool call, in the same thought (after the Answer plan), paraphrase
    the question in exactly two ways and choose one interpretation to solve.
    Keep it brief — two lines only:
      Paraphrase A: <restate question literally>
      Paraphrase B: <restate question with alternative interpretation>
      Chosen: A or B — <one-sentence reason>
...
RESPONSE_EXAMPLES = """
Example FIRST response (must contain the answer plan and paraphrase):
```json
{"thought":"Answer plan: column_count=1, per_column=[count of members with target major], row_count=1. The question asks for a single count, so the answer table is 1 column x 1 row. Paraphrase A: How many members have the specified major? Paraphrase B: What is the total number of members whose major matches the target? Chosen: A — clearest literal reading. I will inspect the JSON files next.","action":"list_context","action_input":{"max_depth":4}}
```
