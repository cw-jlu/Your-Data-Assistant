...
4. TIME + TIES: "the last/first <event>" = ORDER BY the time column then LIMIT 1 — exactly ONE
   row even if several tie on that timestamp. No secondary key; never return all the tied rows.
...
# exp_173: GROUP-BY-dimension teaching. A DECISION PROCEDURE (not an answer map),
# so it generalises and carries no test-data leak. Two rules, both derived from the
# BULL GROUP-BY corpus analysis:
#   (1) the GROUP BY key is the dimension named right after the grouping cue
#       (不同/各/按…分 / different/by/per) — NOT the entity being counted/summed.
#       Corpus check: "不同X的Y数量" groups by X in q45/q132 alike; the failure was the
#       agent binding to the salient counted noun Y instead of X.
#   (2) pick among look-alike columns by sampling distinct VALUES, never by name.
#       Deliberately does NOT say "avoid name columns" — the corpus refutes that
#       (investadvisorname / firstindustryname are valid GROUP BY keys in many queries).
# Appended to fund/stock only (macro has 0% GROUP BY in BULL dev).
GROUPBY_DIM_NOTE = """
## Choosing the GROUP BY / category column
- "by / per / for each / for different <Y>" (按<Y> / 各<Y> / 不同<Y> / 分<Y>别) sets the
  GROUP BY key to <Y> — the dimension named right after that word, NOT the entity you are
  counting or summing. In "number of <X> by <Y>", GROUP BY <Y>, not <X>. (<Y> may be a
  property such as an organization form OR an entity name — take whichever the sentence
  puts after the grouping word; do not assume a name column is never the key.)
- Look-alike columns are common: more than one "…型 / …类型 / type" column (investment
  style vs asset class vs structure), or an entity that has both a name column and an
  attribute / 形式 column. Do NOT choose by column name alone — for each candidate run
  `SELECT "<col>", COUNT(*) GROUP BY 1 ORDER BY 2 DESC LIMIT 10` and pick the column whose
  distinct VALUES are the categories <Y> refers to.
...
# Domains that receive the exp_173 GROUP-BY teaching (macro excluded: 0% GROUP BY).
_GROUPBY_DIM_DOMAINS = {"fund", "stock"}


def domain_note(domain: str | None) -> str:
    """Return the per-domain note for fund/stock/macro; empty for ehr/other/None."""
    dom = (domain or "").strip().lower()
    note = _DOMAIN_NOTES.get(dom, "")
    if note and dom in _GROUPBY_DIM_DOMAINS and flags.groupby_dim_on():
        note = note + GROUPBY_DIM_NOTE
    return note
