...
from experiments.exp_173_groupby_dim.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_173_groupby_dim.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP173_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_173_groupby_dim.video_summarizer import find_video
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_173_groupby_dim.column_auditor import audit_columns, apply_audit

    # The source-router `avoid` role is ADVISORY, not a hard block. It used to
    # reject any broad-table SQL under avoid; that killed correct SQL paths
    # (task_26: a correct COUNT(...) was blocked and the agent fell back to a
    # misread video literal) and could deadlock (task_60). Now `avoid` only WARNS
    # — the warning is already surfaced in the self-review's SOURCE ROUTER block,
    # and the agent decides. With no reject, the old escape-valve machinery
    # (guard/zero-reject counters) is no longer needed.
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        import re as _re
        # A literal materialization (SELECT 'x' AS c, VALUES, CTE-only — no
        # real-table FROM) transcribes a value read from a non-SQL source
        # (video/prose); it does NOT compute anything. The computation-shape
        # guards below (anti-agg + percent/AVG/per-unit rules) police SQL that
        # COMPUTES over tables and are inapplicable to a literal, so skip them
        # for literals (else a literal `SELECT 'ETF'` is rejected by e.g. the
        # "average daily → needs AVG()" rule — observed on task_60).
        _is_literal_answer = not _source_router_avoid_violation(sql)
        # NOTE: `sql_role=avoid` no longer blocks a broad-table SQL. It is advisory
        # only — the SOURCE ROUTER block in the self-review (below) warns the agent,
        # which then decides. (Blocking killed correct SQL on task_26 and deadlocked
        # task_60.) No reject here.
        from experiments.exp_173_groupby_dim import flags
        anti_agg_label = None
        try:
        ...
        if rule_violations and not _is_literal_answer:
            return ToolExecutionResult(
                ok=False,
                content={"error": "[Universal rule violation] " + " | ".join(rule_violations), "sql": sql},
            )
        # DISTINCT hard guard. The soft prompt rule ("DISTINCT only when the question
        # literally says 去重/唯一/distinct") was ignored by the model on fan-out
        # "how many <X>" tasks (task_26 used COUNT(DISTINCT)=6 instead of COUNT(*)=53,
        # 3/3 runs). Make it deterministic: if the OUTPUT select list uses SELECT
        # DISTINCT or COUNT(DISTINCT) but the question has NO literal distinctness word,
        # reject and tell the agent to remove it. A literal distinctness word (e.g.
        # "how many DISTINCT patients", task_12) is allowed through.
        _out_sel = _top_level_select_list(sql)
        _DISTINCT_WORDS = ("distinct", "unique", "去重", "唯一", "不同", "不重复", "不重複")
        # exp_172: EHR domains legitimately need DISTINCT on set-valued questions
        # ("how is <drug> delivered / what routes/methods"), whose gold is
        # `SELECT DISTINCT route`. Skip this finance-tuned hard guard for EHR when
        # the ehr_distinct lever is on. Finance/other keep the guard unchanged.
        _guard_dom = ""
        try:
            _guard_dom = (domain_provider() or "").strip().lower() if domain_provider else ""
        except Exception:
            _guard_dom = ""
        _ehr_distinct_ok = flags.ehr_distinct_on() and _guard_dom in ("eicu", "mimic", "ehr")
        if (not _ehr_distinct_ok) and _re.search(r"\bDISTINCT\b", _out_sel, _re.IGNORECASE) and not any(
            w in q_text.lower() for w in _DISTINCT_WORDS
        ):
            return ToolExecutionResult(
                ok=False,
                content={
                    "error": (
                        "[DISTINCT guard] The output uses SELECT DISTINCT / COUNT(DISTINCT) "
                        "but the question contains NO distinctness word (去重 / 唯一 / 不同 / "
                        "distinct / unique). DISTINCT is not allowed here — REMOVE it: keep "
                        "every row and count with COUNT(*). 'how many <X>' does NOT imply "
                        "distinct; over a fan-out, COUNT(*) is the answer."
                    ),
                    "select_list": _out_sel[:300],
                    "sql": sql,
                },
            )
        try:
        ...
        # The domain (from the deterministic router) selects the stronger
        # finance/EHR auditor prompt for non-'other' tasks.
        col_audit_meta = None
        if auditor_model is not None and len(columns) > 1:
            try:
                _domain = domain_provider() if domain_provider else None
            except Exception:
                _domain = None
            audit = audit_columns(
                question=question, columns=columns, rows=rows[:50],
                model=auditor_model, domain=_domain,
            )
    ...
    specs.update({
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP173_PROSE=1). Inspired by Claude "
