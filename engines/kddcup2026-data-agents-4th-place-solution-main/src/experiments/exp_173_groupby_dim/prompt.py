...
- DISTINCT only for proven JOIN fan-out. For single-table lists, preserve
  duplicate source rows; do NOT use SELECT DISTINCT just because the output is
  one descriptor column. Use DISTINCT only after verifying a JOIN duplicated
  the same base row. For multi-column answers, default to NO DISTINCT.
...
of the same question. Possible alternative formulations:
  ...
  - DUPLICATE SANITY: if your terminal SQL returns ONE descriptor column
    using a JOIN, verify whether duplicates came from JOIN fan-out. If the
    base table itself has duplicate descriptors, those duplicates are real and
    must be preserved. No JOIN means no DISTINCT.
...
Then output one of:
  ...
  (b) `answer_from_sql` with REWRITTEN SQL — if cross-check reveals the
      terminal interpretation is wrong, OR if the duplicate sanity check proves
      JOIN fan-out duplicated the same base rows (= add SELECT DISTINCT).
...
def allowed_tools_for_phase(phase: str) -> tuple[str, ...]:
    ...
    from experiments.exp_173_groupby_dim import flags
...
# ====================================================================
# exp_149 LEVER ① — answer_shape_guard (DISTINCT → JOIN-fanout-only +
...
=== exp_149 ANSWER-SHAPE OVERRIDE (supersedes the DISTINCT rule above) ===
- DISTINCT is almost never correct: keep every row and count with `COUNT(*)`,
  not `SELECT DISTINCT` / `COUNT(DISTINCT ...)`. Repeated values in a base table
  ARE part of the answer — return rows AS-IS. Use DISTINCT ONLY when the question
  LITERALLY contains a distinctness word (去重 / 唯一 / 不同 / distinct / unique);
  never infer it from "how many <X>".
...
=== exp_149 ANSWER-SHAPE OVERRIDE (supersedes the DUPLICATE SANITY step above) ===
- DISTINCT check: if your SQL has `SELECT DISTINCT` or `COUNT(DISTINCT ...)`,
  mark a VIOLATION and switch to keeping all rows / `COUNT(*)` — unless the
  question LITERALLY contains a distinctness word (去重 / 唯一 / distinct / unique);
  "how many <X>" alone is NOT such a word.
- Do NOT add `WHERE ... IS NOT NULL` / `<> ''` at this stage to "clean" the
  result — blank/NULL rows are part of a plain list answer."""

# The DISTINCT and NULL/BLANK-preservation rules above are injected only in the
# ANSWER and VERIFY phases — but the agent often settles on its result query
# during EXPLORE, where it adds `WHERE <col> IS NOT NULL` or a spurious DISTINCT
# and then carries that row-dropping probe into the commit. This block keeps the
# goal in mind during EXPLORE: the RETURNED answer must preserve NULL/blank and
# duplicate rows. It is framed around the returned query only (not a blanket ban
# on DISTINCT / IS NOT NULL, which are fine as diagnostic probes mid-exploration)
# and does not spell out explicit permissions. Gated by explore_shape_on for a
# clean A/B against v7.
_EXPLORE_SHAPE_OVERRIDE = """

=== exp_149 ANSWER-SHAPE NOTE (keep in mind as you work toward the answer) ===
- The answer you will RETURN must INCLUDE blank/NULL rows and must KEEP
  legitimate duplicate rows — the returned row set is scored as-is.
- So the query you return must not drop them: no `WHERE <col> IS NOT NULL` /
  `<> ''` filter unless the question literally says "non-null", "non-empty",
  "with a known X", or "excluding blanks". Return a NULL/blank value as an empty
  cell.
- DISTINCT is almost never correct: keep every row and count with `COUNT(*)`,
  not `SELECT DISTINCT` / `COUNT(DISTINCT ...)`. Use DISTINCT ONLY when the
  question LITERALLY contains a distinctness word (去重 / 唯一 / 不同 / distinct /
  unique); never infer it from "how many <X>"."""

# ====================================================================
# exp_149 LEVER ② — prose_doc_forced_read, H4a parse protocol. Appended to
...
EHR OVERRIDE (clinical EHR task) — for THIS task the "NO LIMIT / filter-back"
rule (rule 6 and the superlative default above) does NOT apply:
- A singular "the last / first / most recent / earliest <event>" answer = ORDER BY
  the time column (DESC = last/recent, ASC = first/earliest) then LIMIT 1. LIMIT 1
  is REQUIRED — return exactly ONE row even if several events share that boundary
  timestamp.
- Do NOT use `WHERE time = (SELECT MAX/MIN(time)...)` (filter-back): it returns
  every tied row, which is wrong for a singular "the last/first <event>".
- A "list ALL <X>" question still returns every matching row — no LIMIT there."""


# ====================================================================
# exp_172 — EHR DISTINCT variants. The exp_149 anti-DISTINCT guard ("DISTINCT is
# almost never correct") is finance-empirical and INVERTS the correct behaviour on
# EHR set-valued questions ("how is <drug> delivered", "what routes/methods for X")
# whose gold is `SELECT DISTINCT route`. For domain in {eicu,mimic,ehr} we swap the
# DISTINCT bullet for an EHR-appropriate one; the NULL/BLANK preservation (domain-
# neutral) is kept verbatim. Gated by flags.ehr_distinct_on() for a clean A/B.
# ====================================================================
_ANSWER_SHAPE_OVERRIDE_EHR = """

=== exp_172 ANSWER-SHAPE OVERRIDE (EHR) — DISTINCT is often correct here ===
- DISTINCT (EHR): a question asking for the SET of values of one column — e.g.
  "how is <drug> delivered / administered", "what are the (standard) methods /
  routes / ways for <X>", "which distinct <values>" — wants the UNIQUE set:
  use `SELECT DISTINCT <col>`, do NOT return one row per raw prescription/event.
  For "how many distinct <entities>" use `COUNT(DISTINCT key)`. (Plain single-value
  lookups — "when / the last <event> / the value of X" — still keep all rows.)
- NULL / BLANK PRESERVATION. Do NOT add `WHERE <col> IS NOT NULL` or
  `WHERE <col> <> ''` unless the question literally says "non-null",
  "non-empty", "with a known X", or "excluding blanks". For plain
  List/Show/Retrieve questions, blank/NULL rows ARE part of the answer —
...
_VERIFY_SHAPE_OVERRIDE_EHR = """

=== exp_172 ANSWER-SHAPE OVERRIDE (EHR) — do NOT flag DISTINCT as a violation ===
- DISTINCT check: for EHR set-valued questions ("what routes / methods / how is X
  delivered / which distinct values"), `SELECT DISTINCT` / `COUNT(DISTINCT ...)`
  is CORRECT — do NOT mark it a violation. Only flag DISTINCT when the question is a
  plain per-record list/lookup that should keep duplicate rows.
- Do NOT add `WHERE ... IS NOT NULL` / `<> ''` at this stage to "clean" the
  result — blank/NULL rows are part of a plain list answer."""

_EXPLORE_SHAPE_OVERRIDE_EHR = """

=== exp_172 ANSWER-SHAPE NOTE (EHR) — keep in mind as you work toward the answer ===
- The answer you will RETURN must INCLUDE blank/NULL rows; no `WHERE <col> IS NOT
  NULL` / `<> ''` filter unless the question literally says so. Return a NULL/blank
  value as an empty cell.
- DISTINCT (EHR): if the question asks for the SET of distinct values of a column
  ("how is <drug> delivered / what routes / methods for X"), return the UNIQUE set
  with `SELECT DISTINCT <col>` — the raw table repeats the value once per event, and
  the answer is the deduplicated set, not every row."""


def build_phased_system_prompt(
    phase: str, tool_descriptions: str, domain: str | None = None
) -> str:
    """Build the phase-specific system prompt for PhasedReActAgent.

    exp_149 ablation: when EXP173_ANSWER_SHAPE / EXP173_PROSE are on, the
    ...
    When `domain` is EHR (eicu/mimic/ehr), the ANSWER/EXPLORE instructions get an
    appended OVERRIDE that supersedes the global no-LIMIT/filter-back rule for the
    singular "last/first <event>" case (which the gold collapses to one via LIMIT 1).
    """
    from experiments.exp_173_groupby_dim import flags
    ...
    # exp_172: for EHR domains with the ehr_distinct lever on, swap the anti-DISTINCT
    # answer-shape blocks for EHR variants that allow DISTINCT on set-valued questions.
    _is_ehr = bool(domain and domain.strip().lower() in ("eicu", "mimic", "ehr"))
    _ehr_distinct = _is_ehr and flags.ehr_distinct_on()
    if flags.answer_shape_on():
        if phase == "answer":
            instr = instr + (_ANSWER_SHAPE_OVERRIDE_EHR if _ehr_distinct else _ANSWER_SHAPE_OVERRIDE)
        elif phase == "verify":
            instr = instr + (_VERIFY_SHAPE_OVERRIDE_EHR if _ehr_distinct else _VERIFY_SHAPE_OVERRIDE)
    if flags.explore_shape_on() and phase == "explore":
        instr = instr + (_EXPLORE_SHAPE_OVERRIDE_EHR if _ehr_distinct else _EXPLORE_SHAPE_OVERRIDE)
    if flags.prose_on() and phase == "explore":
        instr = instr + "\n" + _EXPLORE_PROSE_PROTOCOL
    if flags.watch_video_on() and phase == "explore":
        instr = instr + "\n" + _EXPLORE_WATCH_VIDEO_PROTOCOL
    if domain and domain.strip().lower() in ("eicu", "mimic", "ehr") and phase in ("answer", "explore"):
        instr = instr + _EHR_LIMIT_OVERRIDE
