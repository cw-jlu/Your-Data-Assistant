...
from experiments.exp_173_groupby_dim.agent import ReActAgent, ReActAgentConfig
from experiments.exp_173_groupby_dim.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_173_groupby_dim.adaptive_vote import adaptive_vote
from experiments.exp_173_groupby_dim.config import AppConfig
from experiments.exp_173_groupby_dim import flags
from experiments.exp_173_groupby_dim.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_173_groupby_dim.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP173_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP173_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP173_KEYFRAME_ROOT"):
        _os.environ["EXP173_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    icl_note_text = ""
    if flags.icl_fewshot_on() and domain_route is not None:
        try:
            from experiments.exp_173_groupby_dim.icl_fewshot import fewshot_note

            icl_note_text = fewshot_note(task.question, domain_route.domain)
        except Exception:
            icl_note_text = ""
    ...
    # unknown general DB that may need such calc). Toggle with EXP173_MATH_OTHER_ONLY.
    ...
    if flags.prose_extract_on():
        try:
            from experiments.exp_173_groupby_dim.prose_extractor import (
                extract_prose_tables, select_prose_docs, prose_gate,
            )
            _ext_root = _os.environ.get("EXP173_EXTRACTED_ROOT")
            if not _ext_root:
                _ext_root = str(config.run.output_dir / "_prose_extracted")
                _os.environ["EXP173_EXTRACTED_ROOT"] = _ext_root
            _df = _os.environ.get("EXP173_PROSE_DOC_FILTER")  # optional manual override
    ...
    trace_root = _os.environ.get("EXP173_TRACE_ROOT")
    if trace_root:
