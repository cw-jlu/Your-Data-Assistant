...
from experiments.exp_066_kira_obs_truncate.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_066_kira_obs_truncate.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_066_kira_obs_truncate.tools.sqlite import (
    execute_read_only_sql,
    inspect_sqlite_schema,
)

EXECUTE_PYTHON_TIMEOUT_SECONDS = 30
_OBS_MAX_CHARS = 30_000


def _truncate_obs(content: dict[str, Any], max_chars: int) -> dict[str, Any]:
    """Cap large string observation fields to keep context size bounded."""
    result = content
    for field in ("output", "preview"):
...
def _read_csv(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    return ToolExecutionResult(ok=True, content=_truncate_obs(content, _OBS_MAX_CHARS))
...
def _read_json(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    content = read_json_preview(task, path, max_chars=max_chars)
    return ToolExecutionResult(ok=True, content=_truncate_obs(content, _OBS_MAX_CHARS))
...
def _read_doc(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    path = str(action_input["path"])
    max_chars = int(action_input.get("max_chars", 4000))
    content = read_doc_preview(task, path, max_chars=max_chars)
    return ToolExecutionResult(ok=True, content=_truncate_obs(content, _OBS_MAX_CHARS))
...
def _execute_python(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    return ToolExecutionResult(ok=bool(content.get("success")), content=_truncate_obs(content, _OBS_MAX_CHARS))
