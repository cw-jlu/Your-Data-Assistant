...
def _build_fk_map_section(conn: sqlite3.Connection, tables: list[str]) -> str:
    """Return a [FK Map] section string listing foreign key relationships, or '' if none."""
    lines: list[str] = []
    try:
        cur = conn.cursor()
        for tbl in tables:
            rows = cur.execute(f'PRAGMA foreign_key_list("{tbl}")').fetchall()
            for row in rows:
                ref_table, from_col, to_col = row[2], row[3], row[4]
                lines.append(f"  {tbl}.{from_col} → {ref_table}.{to_col}")
    except Exception:
        return ""
    if not lines:
        return ""
...
def _sqlite_full(path: Path) -> str:
    ...
    try:
        ...
        fk_section = _build_fk_map_section(conn, tables)
        if fk_section:
