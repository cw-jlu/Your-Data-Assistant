...
from experiments.exp_149_modality.agent import ReActAgent, ReActAgentConfig
from experiments.exp_149_modality.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_149_modality.adaptive_vote import adaptive_vote
from experiments.exp_149_modality.config import AppConfig
from experiments.exp_149_modality import flags
from experiments.exp_149_modality.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_149_modality.prefix_cache import with_prefix_cache_header
from experiments.exp_149_modality.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP149_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP149_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP149_KEYFRAME_ROOT"):
        _os.environ["EXP149_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    trace_root = _os.environ.get("EXP149_TRACE_ROOT")
    if trace_root:
