...
from experiments.exp_027_loop_break.prompt import (
    REACT_SYSTEM_PROMPT,
    build_observation_prompt,
    build_system_prompt,
    build_task_prompt,
)
from experiments.exp_027_loop_break.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_027_loop_break.tools.registry import ToolRegistry
    ...
    def run(self, task: PublicTask) -> AgentRunResult:
        ...
        for step_index in range(1, self.config.max_steps + 1):
            ...
            try:
                ...
                # Detect identical consecutive observations to break out of silent loops.
                obs_hash = hashlib.md5(
                    json.dumps(observation, sort_keys=True).encode()
                ).hexdigest()
                if obs_hash == _prev_obs_hash:
                    observation["loop_detected"] = True
                    observation["loop_hint"] = (
                        "This observation is identical to the previous one. "
                        "You are stuck in a loop. Try a completely different action or call answer now."
                    )
                _prev_obs_hash = obs_hash
