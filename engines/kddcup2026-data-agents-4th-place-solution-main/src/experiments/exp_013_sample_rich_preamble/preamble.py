...
_LEVEL_PARAMS: dict[DetailLevel, dict[str, int | bool]] = {
    DetailLevel.FULL: {"sample_rows": 5, "include_describe": True, "json_preview_chars": 4000, "include_value_sample": True},
    DetailLevel.MEDIUM: {"sample_rows": 3, "include_describe": False, "json_preview_chars": 2000, "include_value_sample": False},
    DetailLevel.MINIMAL: {"sample_rows": 1, "include_describe": False, "json_preview_chars": 800, "include_value_sample": False},
}
...
def _describe_csv(path: Path, level: DetailLevel) -> str:
    ...
    include_value_sample = bool(params["include_value_sample"])
    try:
    ...
    if include_describe:
        try:
            ...
            pass
    if include_value_sample:
        # Top-5 representative values + null ratio per column help the agent
        # identify categorical values without running a full exploration step.
        sample_parts: list[str] = []
        for col in list(df.columns)[:20]:
            null_ratio = df[col].isna().mean()
            top = df[col].dropna().value_counts().head(5)
            top_str = ", ".join(f"{v!r}({c})" for v, c in top.items())
            sample_parts.append(f"  {col}: null={null_ratio:.2f} top=[{top_str}]")
        parts.append("value_sample (top-5 per col):\n" + "\n".join(sample_parts))
    return "\n\n".join(parts)
...
def _describe_sqlite(path: Path, level: DetailLevel) -> str:
    ...
    try:
        ...
        for table in tables:
            ...
            if include_value_sample:
                # Top-5 distinct values per column help detect categorical columns
                # (e.g. event type names) without a separate exploration query.
                ...
                for col in col_names[:20]:
                    try:
                        ...
                        top_str = ", ".join(f"{r[0]!r}({r[1]})" for r in rows)
                        val_lines.append(f"  {col}: [{top_str}]")
                    except Exception:
                        val_lines.append(f"  {col}: [error]")
                table_parts.append("value_sample (top-5 per col):\n" + "\n".join(val_lines))
            parts.append("\n".join(table_parts))
        return "\n\n".join(parts)
    finally:
        conn.close()
