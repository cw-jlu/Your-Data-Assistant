...
def _collect_column_names(task: PublicTask) -> list[tuple[str, str]]:
    """Return (source, col_name) pairs from all SQLite tables and CSV headers."""
    ...
    for path in sorted(task.context_dir.rglob("*")):
        ...
        if suffix in {".db", ".sqlite", ".sqlite3"}:
                ...
                for table in tables:
                    try:
                        ...
                        for desc in cur.description or []:
                            results.append((f"{source}:{table}", desc[0]))
                    except Exception:
                        pass
                conn.close()
            except Exception:
            ...
            try:
                ...
                for col in header.rstrip("\n").split(","):
                    col = col.strip().strip('"')
                    if col:
                        results.append((source, col))
            except Exception:
                pass
    return results


def _build_schema_linking_section(question: str, col_pairs: list[tuple[str, str]]) -> str | None:
    """Match question keywords against column names via substring or fuzzy match.

    Returns a `## Column hints` section string, or None if no matches found.
    """
    # Extract candidate keywords: alphanumeric tokens of sufficient length.
    keywords = [
        w.lower()
        for w in re.findall(r"[A-Za-z0-9_]+", question)
    ...
    for source, col in col_pairs:
        ...
        if matched:
            ...
            if key not in seen:
                seen.add(key)
                matches.append(f"  - `{col}` (from {source})")
        if len(matches) >= _SCHEMA_HINTS_MAX_MATCHES:
            break

    if not matches:
        return None
    lines = ["## Column hints"] + matches
    return "\n".join(lines)
...
def build_preamble(
    ...
    *,
    ...
    col_pairs = _collect_column_names(task)
    schema_hints = _build_schema_linking_section(task.question, col_pairs)
    if schema_hints is not None:
        fixed_parts.append(schema_hints)
