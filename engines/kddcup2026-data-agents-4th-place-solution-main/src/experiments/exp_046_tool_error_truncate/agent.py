...
from experiments.exp_046_tool_error_truncate.prompt import (
    REACT_SYSTEM_PROMPT,
    build_observation_prompt,
    build_system_prompt,
    build_task_prompt,
)
from experiments.exp_046_tool_error_truncate.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_046_tool_error_truncate.tools.registry import ToolRegistry
...
def _get_error_hint(exc: Exception) -> str:
    """Return a short, actionable hint for common tool errors."""
    msg = str(exc).lower()
    if "no such table" in msg or "no such column" in msg:
        return "Try: call inspect_sqlite_schema first to verify table/column names."
    if "syntax error" in msg or "parse error" in msg:
        return "Try: check SQL syntax; use execute_python if SQL dialect is unsupported."
    if "keyerror" in msg or "key error" in msg:
        return "Try: check column name with read_csv_preview before accessing."
    if "typeerror" in msg or "type error" in msg:
        return "Try: check data types and cast if needed (int(), float(), str())."
    if "filenotfounderror" in msg or "no such file" in msg:
        return "Try: call list_context_tree to find the correct file path."
    return "Try: read the error and adjust your approach."
