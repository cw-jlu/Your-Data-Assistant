...
from experiments.exp_011_spec_driven.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_011_spec_driven.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_011_spec_driven.tools.sqlite import (
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "answer_from_sql": ToolSpec(
            ...
            description=(
                "Run a read-only SQL query against a **single SQLite** (.db/.sqlite) file "
                "and submit the result rows as the final answer table. Use this only when "
                "the answer comes from one SQLite source — much safer than retyping result "
                "values into `answer`. For multi-source JOINs (CSV + JSON + SQLite), use "
                "`answer_from_python` with `import duckdb` instead. This is a terminating "
                "action."
            ),
            input_schema={
