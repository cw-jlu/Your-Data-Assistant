...
from experiments.exp_019_column_count_coercion.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_019_column_count_coercion.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_019_column_count_coercion.tools.sqlite import (
...
def _answer_from_python(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    if isinstance(action_input, str):
        action_input = {"code": action_input}
    max_columns: int | None = action_input.get("__max_columns__")  # type: ignore[assignment]
    code = str(action_input["code"])
