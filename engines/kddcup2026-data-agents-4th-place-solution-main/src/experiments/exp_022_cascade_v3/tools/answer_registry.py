...
from experiments.exp_022_cascade_v3.tools.registry import (
    ToolExecutionResult,
    ToolRegistry,
    ToolSpec,
)
...
def _make_answer_handler(spec_data: dict[str, Any]):
    ...
    # Fix #2: track attempts so shape mismatch becomes a soft warning after the first retry.
    attempt_count = [0]
    ...
    def _answer_from_duckdb(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        # Fix #2: Shape mismatch — first attempt triggers a retry prompt; subsequent
        # attempts accept the result to avoid missing_prediction from a retry loop.
        if expected_col_count > 0 and len(columns) != expected_col_count:
            if attempt_count[0] == 1:
                return ToolExecutionResult(
                    ...
                    content={
                        "error": (
                            f"Shape mismatch (attempt {attempt_count[0]}): SQL returned "
                            f"{len(columns)} column(s) but spec.answer_columns has "
                            f"{expected_col_count}. Rewrite the SQL to produce exactly "
                            "the expected columns."
                        )
                    },
                )
            # After the first retry, accept the result with a warning rather than looping.
            ...
            return ToolExecutionResult(
                ...
                content={
                    ...
                    "row_count": len(rows),
                    "warning": (
                        f"Shape mismatch accepted on attempt {attempt_count[0]}: "
                        f"SQL returned {len(columns)} column(s), spec expected "
