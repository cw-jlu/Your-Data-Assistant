...
from experiments.exp_146_modality.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_146_modality.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP146_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_146_modality.video_summarizer import find_video
...
def _describe_data(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    # exp_146 lever ② (prose_on): advertise prose-only docs so the agent learns
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_146_modality.column_auditor import audit_columns, apply_audit
    ...
    specs.update({
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP146_PROSE=1). Inspired by Claude "
