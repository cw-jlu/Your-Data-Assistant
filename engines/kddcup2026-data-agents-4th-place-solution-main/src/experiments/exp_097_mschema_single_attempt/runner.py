...
from experiments.exp_097_mschema_single_attempt.agent import ReActAgent, ReActAgentConfig
from experiments.exp_097_mschema_single_attempt.config import AppConfig
from experiments.exp_097_mschema_single_attempt.preamble import build_preamble
from experiments.exp_097_mschema_single_attempt.tools.registry import (
    ToolRegistry,
...
# Each task is run N=1 time at T=0.6 (exp_097 [context-single] axis:
# single-attempt class to isolate the M-Schema preamble effect from
# T-diversity recall gain). With n_attempts=1, the merge function
# receives a single answer and passes it through; k=1 union is a no-op.
# This is intentionally NOT the SOTA path — exp_086 (3-attempt union
# T-diversity, k=1) remains the winner. The single-attempt class trades
# T-diversity recall (-0.06 cost per exp_092) for clean attribution of
# preamble-level changes (here the M-Schema injection ported from
# exp_088 without the Plan-first Rule 12a addition).

_MAJORITY_K = 1  # union (k=1): keep any signature appearing in 1+ attempt
# exp_097 [context-single] axis: single-attempt class to remove the union
# constituent of the exp_088 three-way trap (Plan-first × T-diversity ×
# union). With n_attempts=1, `_signature_majority_merge` receives a single
# answer and passes it through unchanged (k=1 union is a no-op for n=1).
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6,)  # Qwen3.5 precise mode, single-attempt
...
# Derive experiment prefix from the parent directory name (e.g. "exp_097_mschema_single_attempt")
