...
def _inspect_dataframe(df: pd.DataFrame, max_rows: int) -> dict[str, object]:
...
def inspect_table(
    task: PublicTask, relative_path: str, *, max_rows: int = 5
) -> dict[str, object]:
    path = resolve_context_path(task, relative_path)
    ext = path.suffix.lower()
    if ext == ".csv":
        df = pd.read_csv(path)
        return {"ok": True, "file_type": "csv", **_inspect_dataframe(df, max_rows)}
        ...
        try:
            ...
            for tbl in tables:
                df = pd.read_sql_query(f"SELECT * FROM [{tbl}] LIMIT {max_rows}", conn)
                result[tbl] = _inspect_dataframe(df, max_rows)
        finally:
            conn.close()
        return {"ok": True, "file_type": "sqlite", "tables": result}
    else:
        return {
            "ok": False,
            "error": f"Unsupported extension: {ext}. Use read_csv or read_doc instead.",
        }
