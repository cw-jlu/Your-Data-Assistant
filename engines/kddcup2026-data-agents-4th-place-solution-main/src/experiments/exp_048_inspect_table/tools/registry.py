...
from experiments.exp_048_inspect_table.tools.filesystem import (
    inspect_table,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_048_inspect_table.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_048_inspect_table.tools.sqlite import (
...
def _inspect_table(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    result = inspect_table(task, path)
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "inspect_table": ToolSpec(
            name="inspect_table",
            description=(
                "Inspect a CSV or SQLite file: returns column dtypes, null%, and top-5 sample rows "
                "in a single call. Faster than calling inspect_sqlite_schema + SELECT LIMIT separately."
            ),
            input_schema={"path": "relative/path/to/file.csv or file.sqlite"},
        ),
