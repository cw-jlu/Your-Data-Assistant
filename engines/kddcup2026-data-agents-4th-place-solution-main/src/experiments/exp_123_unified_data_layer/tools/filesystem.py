...
def read_doc_preview(
    ...
    - offset=<int> (default 0 = line 1): start at LINE NUMBER `offset` (1-based).
      Returns up to `n_lines` lines, capped by `max_chars`. Output is
      `cat -n`-style (= "<line_no>\\t<content>"), so the agent can directly
    ...
    # n_lines: how many lines to include (capped by max_chars).
