...
def describe_catalog(context_dir: Path) -> str:
    ...
    Format-agnostic output: only view name + row count + columns, no
    mention of underlying file format (csv/json/sqlite). The agent should
    treat all data as uniform SQL views.
    """
    ...
    for entry in cat:
        view = entry.get("view")
        if view is None:
            # Hide source filename from error display (= format-agnostic).
            lines.append(f"- ❌ <unknown view>: {entry.get('error', '')[:200]}")
            continue
        cols = entry.get("columns", [])
        # Fetch row count for each view (= lightweight aggregate).
        try:
            n_rows = conn.execute(f"SELECT COUNT(*) FROM {view}").fetchone()[0]
            shape = f"{n_rows} rows × {len(cols)} cols"
        except duckdb.Error:
            shape = f"? rows × {len(cols)} cols"
        cols_str = ", ".join(cols[:8]) + (f", … (+{len(cols)-8})" if len(cols) > 8 else "")
        lines.append(f"- `{view}` ({shape}): {cols_str}")
    return "\n".join(lines)
