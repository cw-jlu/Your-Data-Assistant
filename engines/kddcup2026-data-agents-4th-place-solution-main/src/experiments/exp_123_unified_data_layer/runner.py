...
from experiments.exp_123_unified_data_layer.agent import ReActAgent, ReActAgentConfig
from experiments.exp_123_unified_data_layer.config import AppConfig
from experiments.exp_123_unified_data_layer.preamble import build_preamble
from experiments.exp_123_unified_data_layer.tools.registry import (
    ToolRegistry,
...
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6, 0.6, 0.6, 0.6, 0.6)  # 5-attempt vote, uniform temp

# Early-stop K-of-N (AIMO3-inspired): once any answer signature appears
# in K attempts, terminate the rest. K=3 for N=5 (= simple majority lock-in).
# Set KOBUSHI_EARLY_STOP_K=0 to disable (= original behavior, run all N).
import os as _os
_EARLY_STOP_K = int(_os.environ.get("KOBUSHI_EARLY_STOP_K", "3"))
...
def _run_single_task_with_timeout(*, task_id: str, config: AppConfig) -> dict[str, Any]:
    ...
    # Simple majority vote (= pick most common signature). Replaces the
    # union-style _signature_majority_merge based on PoC findings showing
    # majority outperforms union when paired with the column auditor.
    merged_answer = _majority_pick(attempt_answers)
