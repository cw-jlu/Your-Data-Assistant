"""Entry point for experiment exp_123_unified_data_layer.

Usage:
    python -m experiments.exp_123_unified_data_layer.run [COMMAND] [OPTIONS]
...
from experiments.exp_123_unified_data_layer.config import load_app_config
from experiments.exp_123_unified_data_layer.runner import (
    TaskRunArtifacts,
...
# Tasks that scored 1.00 across every historic run (= rock-solid stable).
# Skipping these in iterative experiments saves ~44% wall clock; the score
# baseline is added back as a fixed +22 / 50 contribution.
STABLE_TASK_IDS: frozenset[str] = frozenset({
    "task_11", "task_145", "task_19", "task_194", "task_214", "task_218",
    "task_22",  "task_24",  "task_26", "task_261", "task_269", "task_283",
    "task_287", "task_292", "task_305", "task_349", "task_350", "task_415",
    "task_420", "task_64",  "task_74", "task_75",
})
...
def run_benchmark_command(
    ...
    skip_stable: bool = typer.Option(
        False, "--skip-stable",
        help=f"Skip the {len(STABLE_TASK_IDS)} rock-solid tasks that always score 1.00.",
    ),
) -> None:
    ...
    if skip_stable:
        all_tasks = [t for t in all_tasks if t.task_id not in STABLE_TASK_IDS]
        console.print(
            f"[yellow]--skip-stable: skipping {len(STABLE_TASK_IDS)} stable tasks "
            f"→ running {len(all_tasks)} tasks[/yellow]"
        )
    ...
    with Progress(*progress_columns, console=console) as progress:
        ...
        try:
            run_output_dir, artifacts = run_benchmark(
                ...
                exclude_task_ids=STABLE_TASK_IDS if skip_stable else None,
            )
        except (ValueError, FileExistsError) as exc:
            raise typer.BadParameter(str(exc), param_hint="run.run_id") from exc
        progress.update(
