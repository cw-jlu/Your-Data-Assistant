...
from experiments.exp_152_final_sql_guard.agent import ReActAgent, ReActAgentConfig
...
from experiments.exp_152_final_sql_guard.config import AppConfig
...
from experiments.exp_152_final_sql_guard.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP152_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP152_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp152_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP152_KEYFRAME_ROOT"):
        _os.environ["EXP152_KEYFRAME_ROOT"] = str(
            Path("/tmp") / "kobushi_exp152_keyframe_cache" / run_name
        )


def _run_advisors(question: str, task_id: str) -> tuple[str, Any]:
    """Run the same planning sub-agents used by the exp152 bench script."""
    ...
    *,
    ...
    if tools is None:
        ...
        tools = create_default_tool_registry(
            ...
            guard_model=agent_model if flags.final_sql_guard_on() else None,
    ...
    trace_root = _os.environ.get("EXP152_TRACE_ROOT")
    if trace_root:
    ...
    payload["exp152_runtime_metadata"] = {
