...
from experiments.exp_092_verify_rule_single.agent import ReActAgent, ReActAgentConfig
from experiments.exp_092_verify_rule_single.config import AppConfig
from experiments.exp_092_verify_rule_single.preamble import build_preamble
from experiments.exp_092_verify_rule_single.tools.registry import (
    ToolRegistry,
...
# ============================================================================
# Single-attempt run with verifier hook (verify-rule axis exploration)
# ============================================================================

# Single attempt (N=1) for fast iteration; _signature_majority_merge receives
# exactly one answer and passes it through unchanged (no row-padding path),
# which structurally avoids the _signature_majority_merge row-padding bug.
# _MAJORITY_K = 1 (union) is kept for code-path consistency.

_MAJORITY_K = 1  # union (k=1): keep any signature appearing in 1+ attempt
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6,)  # Qwen3.5 precise, single-attempt for verify-rule axis
...
# Derive experiment prefix from the parent directory name (e.g. "exp_092_verify_rule_single")
