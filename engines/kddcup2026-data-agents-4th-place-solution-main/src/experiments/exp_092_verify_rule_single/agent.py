...
from experiments.exp_092_verify_rule_single.prompt import (
    REACT_SYSTEM_PROMPT,
    build_observation_prompt,
    build_system_prompt,
    build_task_prompt,
)
from experiments.exp_092_verify_rule_single.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_092_verify_rule_single.tools.registry import ToolRegistry
from experiments.exp_092_verify_rule_single.verifier import verify_answer_shape
        ...
        for step_index in range(1, self.config.max_steps + 1):
            ...
            try:
                ...
                if tool_result.is_terminal:
                    columns = list(tool_result.answer.columns) if tool_result.answer else []
                    ok, hint = verify_answer_shape(task.question, columns)
                    if ok or state.verifier_retries_used >= 1:
                        state.answer = tool_result.answer
                        break
                    state.verifier_retries_used += 1
                    # StepRecord is frozen — replace the last entry with verifier hint
                    # injected into the observation so the agent can correct itself.
                    state.steps[-1] = StepRecord(
                        step_index=step_record.step_index,
                        thought=step_record.thought,
                        action=step_record.action,
                        action_input=step_record.action_input,
                        raw_response=step_record.raw_response,
                        observation={
                            "ok": False,
                            "verifier_hint": hint,
                            "original_observation": step_record.observation,
                        },
                        ok=False,
                    )
                    # Do NOT break; loop continues so agent can revise its answer.
