...
from experiments.exp_166_domain_rules.agent import ReActAgent, ReActAgentConfig
from experiments.exp_166_domain_rules.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_166_domain_rules.adaptive_vote import adaptive_vote
from experiments.exp_166_domain_rules.config import AppConfig
from experiments.exp_166_domain_rules import flags
from experiments.exp_166_domain_rules.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
from experiments.exp_166_domain_rules.domain_router import classify_domain
from experiments.exp_166_domain_rules.domain_prompt import domain_note, modality_note
...
from experiments.exp_166_domain_rules.prefix_cache import with_prefix_cache_header
from experiments.exp_166_domain_rules.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP166_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP166_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP166_KEYFRAME_ROOT"):
        _os.environ["EXP166_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    # unknown general DB that may need such calc). Toggle with EXP166_MATH_OTHER_ONLY.
    ...
    trace_root = _os.environ.get("EXP166_TRACE_ROOT")
    if trace_root:
