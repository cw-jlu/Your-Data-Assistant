...
from experiments.exp_166_domain_rules.profile import (
    csv_profile_text,
    json_array_profile,
...
- Do not strip or add spaces / punctuation inside a name, and do not otherwise reformat text.
...
def _doc_full(task: PublicTask, path: Path) -> str:
    ...
    if suffix == ".pdf":
        try:
            from experiments.exp_166_domain_rules.pdf_text_cache import (
...
def _build_prose_index_section(task: PublicTask) -> str | None:
    ...
    if not flags.prose_index_on():
        return None
    try:
        from experiments.exp_166_domain_rules.prose_indexer import index_cache_root
    except Exception:
        return None
    ...
    try:
        max_chars = int(os.environ.get("EXP166_PROSE_INDEX_MAX_CHARS", "50000"))
    except Exception:
        max_chars = 50_000
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_166_domain_rules.tools.duckdb_unified import describe_catalog
