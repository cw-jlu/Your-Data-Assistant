...
from experiments.exp_166_domain_rules.agent import (
    parse_model_step,
)
from experiments.exp_166_domain_rules.prompt import (
    allowed_tools_for_phase,
    build_observation_prompt,
    build_phased_system_prompt,
    build_task_prompt,
)
from experiments.exp_166_domain_rules.runtime import (
    AgentRunResult,
    AgentRuntimeState,
    StepRecord,
)
from experiments.exp_166_domain_rules.tools.registry import ToolRegistry
from experiments.exp_166_domain_rules import flags
from experiments.exp_166_domain_rules.source_router import route_answer_source
        ...
        if current == "explore" and next_phase == "answer":
            ...
            if watch_video_required and not watched_video_ok:
                return False, current, (
                    ...
                    "EXP166_WATCH_VIDEO=1, but no successful watch_video call has "
...
class PhasedReActAgent:
    ...
    def _find_keyframe_paths(task: PublicTask) -> list:
        ...
        root = _P(os.environ.get("EXP166_KEYFRAME_ROOT", "/tmp/kobushi_exp149_keyframe_cache"))
        ...
        try:
            max_images = max(1, int(os.environ.get("EXP166_KEYFRAME_MAX_IMAGES", "18")))
        except ValueError:
            max_images = 18
