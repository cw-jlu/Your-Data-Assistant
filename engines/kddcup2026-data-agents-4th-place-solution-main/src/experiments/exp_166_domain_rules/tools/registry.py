...
from experiments.exp_166_domain_rules.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_166_domain_rules.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_watch_video(model: ModelAdapter | None, question_provider) -> ToolHandler:
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not flags.watch_video_on():
            return ToolExecutionResult(
                ok=False,
                content={"error": "watch_video is disabled. Set EXP166_WATCH_VIDEO=1."},
            )
        if model is None:
            return ToolExecutionResult(
                ok=False,
        ...
        import base64
        from experiments.exp_166_domain_rules.video_summarizer import find_video
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_166_domain_rules.column_auditor import audit_columns, apply_audit

    # Escape valve for the source-router `avoid` deadlock (issue #2): the agent
    # gets stuck producing empty results -- the avoid guard pushes it to empty
    # literals (0-rows) while blocking the only data-producing broad SQL, so no
    # move satisfies both constraints. Release the avoid guard for this task once
    # the stuck signature appears. Signature (backtested over all runs / 388
    # GUARD-or-ZERO task-runs): >= _ESCAPE_GUARD guard rejections AND
    # >= _ESCAPE_ZERO empty-result (0-rows) rejections, cumulative. This catches
    # 21/25 known task_1/task_32 deadlocks (fired-mean 0.065) with 0 false-fires
    # on wins -- task_60's converging retries emit GUARD only (ZERO ~= 0), so the
    # ZERO threshold never trips. The 4 it misses are pure-0-rows (GUARD=0) cases
    # where avoid is not involved and releasing it would not help anyway.
    state = {"refines": 0, "pending_answer": None,
             "guard_rejects": 0, "zero_rejects": 0, "avoid_released": False}
    _ESCAPE_GUARD = 1
    _ESCAPE_ZERO = 3

    def _stuck_on_avoid() -> bool:
        return state["guard_rejects"] >= _ESCAPE_GUARD and state["zero_rejects"] >= _ESCAPE_ZERO
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if (
            ...
            and not state["avoid_released"]
        ):
            # Count this would-be GUARD rejection; if the stuck signature is now
            # met (enough guard + empty-result rejections), release avoid for this
            # task instead of rejecting again -- there is no satisfiable move.
            state["guard_rejects"] += 1
            if _stuck_on_avoid():
                state["avoid_released"] = True  # sticky: stop blocking from now on
            else:
                return ToolExecutionResult(
                    ok=False,
                    content={
        ...
        if not rows:
            # Feed the escape-valve detector: empty results are the stuck signal
            # (avoid guard pushes the agent to empty literals; issue #2). The
            # empty literal itself is still 0-rows, but arm the release so the
            # NEXT broad SQL is allowed through (handles GUARD-early/ZERO-late).
            state["zero_rejects"] += 1
            if _stuck_on_avoid():
                state["avoid_released"] = True
            return ToolExecutionResult(
                ok=False,
                content={
    ...
    specs.update({
        ...
        "grep": ToolSpec(
            ...
            description=(
                ...
                "csv, json, plus pdf when EXP166_PROSE=1). Inspired by Claude "
