...
15. When you face a CSV / SQLite / JSON / Parquet file whose schema you have
    not yet seen, you MAY call `inspect_table(path)` first — it returns the
    column names, dtypes, true row_count, and 3 sample rows in one call.
    This is recommended (not required); skip it for files you already
    understand or for distributional / null-rate questions (use
    execute_python instead, since inspect_table does not return null-rate).
