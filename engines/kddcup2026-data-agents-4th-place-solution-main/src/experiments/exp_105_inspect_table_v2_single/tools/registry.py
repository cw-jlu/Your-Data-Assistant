...
from experiments.exp_105_inspect_table_v2_single.tools.filesystem import (
    inspect_table_v2,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_105_inspect_table_v2_single.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_105_inspect_table_v2_single.tools.sqlite import (
...
def _inspect_table(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    path = str(action_input.get("path", ""))
    result = inspect_table_v2(task, path)
    return ToolExecutionResult(ok=bool(result.get("ok", True)), content=result)
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "inspect_table": ToolSpec(
            name="inspect_table",
            description=(
                "Discover the schema of a CSV / SQLite / JSON / Parquet file in one call. "
                "Returns column names, dtypes, the first 3 sample rows, and the true row "
                "count. Does NOT return null-rate, cardinality, or min/max — keep using "
                "execute_python or execute_context_sql for distributional questions. "
                "Recommended (not required) when you have not yet seen the schema of a file."
            ),
            input_schema={"path": "relative/path/to/file.csv (or .sqlite, .json, .parquet)"},
        ),
