...
15. When computing averages or sums, do NOT silently exclude rows where a value is 0 or NULL
    unless the question explicitly says to exclude them. Use safe_mean(col) or safe_sum(col)
    from helpers.py for reliable results that correctly include zero values:
      from helpers import safe_mean
      result = safe_mean(df['col'])
