...
def safe_mean(series):
    \"\"\"Mean including zeros; only NaN (missing) are excluded.
    Use instead of series.mean() when zeros must be included in the average.\"\"\"
    return pd.to_numeric(series, errors='coerce').mean()


def safe_sum(series):
    \"\"\"Sum including zeros; NaN treated as 0.\"\"\"
...
def build_preamble(
    ...
    *,
    ...
    # Write helpers.py before scanning files so it appears in the workspace overview.
    (task.context_dir / "helpers.py").write_text(_HELPERS_PY, encoding="utf-8")
    ...
    fixed_parts: list[str] = [
        ...
        (
            ...
            "from helpers import safe_mean, safe_sum\n"
            "- safe_mean(series): average including zeros; NaN excluded only\n"
            "- safe_sum(series):  sum with NaN→0"
        ),
    ]
