...
from experiments.exp_101_tool_discovery_additive.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_101_tool_discovery_additive.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_101_tool_discovery_additive.tools.sqlite import (
...
def _grep_file(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    rel_path = str(action_input.get("path", ""))
    pattern = str(action_input.get("pattern", ""))
    max_lines = int(action_input.get("max_lines", 50) or 50)
    abs_path = resolve_context_path(task, rel_path)
    if not abs_path.is_file():
        return ToolExecutionResult(ok=False, content={"error": f"file not found: {rel_path}"})
    return ToolExecutionResult(ok=True, content={"matches": grep_file(abs_path, pattern, max_lines=max_lines)})


def _stat_file(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    rel_path = str(action_input.get("path", ""))
    abs_path = resolve_context_path(task, rel_path)
    if not abs_path.is_file():
        return ToolExecutionResult(ok=False, content={"error": f"file not found: {rel_path}"})
    return ToolExecutionResult(ok=True, content=stat_file(abs_path))


def create_default_tool_registry() -> ToolRegistry:
    """Build the default tool registry.

    exp_101 [tool:discovery-additive] axis (2026-05-06): adds grep_file and
    stat_file tools for narrow lookups in large files. Both are deliberately
    ADDITIVE — existing read_* tools are unchanged. stat_file omits null%/
    cardinality/dtype inference to avoid the exp_048 inspect_table early-stop
    failure mode.
    """
    specs = {
        "answer": ToolSpec(
        ...
        "grep_file": ToolSpec(
            name="grep_file",
            description=(
                "Search for a regex pattern inside a single context file and return up "
                "to 50 matching lines (capped at ~500 chars). Use this only for narrow "
                "lookups in large files (>10MB) or when grep is faster than a full "
                "read; for normal-sized files prefer read_csv_preview / "
        ...
        "stat_file": ToolSpec(
            name="stat_file",
            description=(
                "Return mechanical metadata for a context file (size, line count, "
                "header for CSV, table list for SQLite, top-level structure for JSON, "
                "and the first 3 sample lines). Use this for an early peek at large "
                "files before deciding which read_*_preview to call. Does NOT return "
                "null-rate, cardinality, or dtype inferences."
            ),
            input_schema={
                "type": "object",
