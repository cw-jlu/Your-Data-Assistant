...
from experiments.exp_112_orchestrator.config import AppConfig
from experiments.exp_112_orchestrator.preamble import build_preamble
from experiments.exp_112_orchestrator.tools.registry import (
    ToolRegistry,
...
def _run_single_task_core(
    *,
    ...
    agent = ReActAgent(
        model=agent_model,
        tools=tools or create_default_tool_registry(model=agent_model),
...
def run_benchmark(
    *,
    ...
    if effective_workers == 1:
        ...
        shared_tools = tools or create_default_tool_registry(model=shared_model)
        task_artifacts = []
