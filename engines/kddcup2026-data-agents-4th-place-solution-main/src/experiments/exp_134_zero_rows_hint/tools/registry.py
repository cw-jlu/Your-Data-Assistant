...
from experiments.exp_134_zero_rows_hint.tools.filesystem import (
    grep_context,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
...
from experiments.exp_134_zero_rows_hint.tools.duckdb_unified import (
    describe_catalog,
    execute_sql,
    get_catalog,
)
...
def _make_answer_from_sql_with_audit(
    ...
    *,
    ...
    """
    from experiments.exp_134_zero_rows_hint.column_auditor import audit_columns, apply_audit
    ...
    def handler(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
        ...
        if not rows:
            # Extract column.table references from the SQL to build a DISTINCT hint.
            ref_pattern = re.compile(r"\b([A-Za-z_][\w]*)\.([A-Za-z_][\w]*)\b")
            refs = []
            seen = set()
            for m in ref_pattern.findall(sql):
                key = (m[0], m[1])
                if key in seen:
                    continue
                seen.add(key)
                refs.append(f'SELECT DISTINCT "{m[1]}" FROM {m[0]} LIMIT 5')
                if len(refs) >= 3:
                    break
            hint = (
                "SQL returned 0 rows. The filter likely doesn't match the actual data values. "
                "Suggested diagnostic: run these to inspect what values exist (= for each "
                "filter column, look at the actual values BEFORE assuming case/format):"
            )
            return ToolExecutionResult(
                ok=False,
                content={
                    "error": hint,
                    "suggested_distinct_queries": refs,
