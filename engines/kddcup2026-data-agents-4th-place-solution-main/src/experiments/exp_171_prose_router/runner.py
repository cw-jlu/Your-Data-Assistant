...
from experiments.exp_171_prose_router.agent import ReActAgent, ReActAgentConfig
from experiments.exp_171_prose_router.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_171_prose_router.adaptive_vote import adaptive_vote
from experiments.exp_171_prose_router.config import AppConfig
from experiments.exp_171_prose_router import flags
from experiments.exp_171_prose_router.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_171_prose_router.prefix_cache import with_prefix_cache_header
from experiments.exp_171_prose_router.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP171_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP171_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP171_KEYFRAME_ROOT"):
        _os.environ["EXP171_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    # unknown general DB that may need such calc). Toggle with EXP171_MATH_OTHER_ONLY.
    ...
    if flags.prose_extract_on():
        try:
            from experiments.exp_171_prose_router.prose_extractor import (
                extract_prose_tables, select_prose_docs, prose_gate,
            )
            _ext_root = _os.environ.get("EXP171_EXTRACTED_ROOT")
            if not _ext_root:
                _ext_root = str(config.run.output_dir / "_prose_extracted")
                _os.environ["EXP171_EXTRACTED_ROOT"] = _ext_root
            _df = _os.environ.get("EXP171_PROSE_DOC_FILTER")  # optional manual override
    ...
    trace_root = _os.environ.get("EXP171_TRACE_ROOT")
    if trace_root:
