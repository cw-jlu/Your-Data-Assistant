...
from experiments.exp_171_prose_router.profile import (
    csv_profile_text,
    json_array_profile,
...
def _doc_full(task: PublicTask, path: Path) -> str:
    ...
    if suffix == ".pdf":
        try:
            from experiments.exp_171_prose_router.pdf_text_cache import (
...
def _build_prose_index_section(task: PublicTask) -> str | None:
    ...
    if not flags.prose_index_on():
        return None
    try:
        from experiments.exp_171_prose_router.prose_indexer import index_cache_root
    except Exception:
        return None
    ...
    try:
        max_chars = int(os.environ.get("EXP171_PROSE_INDEX_MAX_CHARS", "50000"))
    except Exception:
        max_chars = 50_000
...
def _build_prose_extracted_section(task: PublicTask) -> str | None:
    """Profile (NOT the full table) of each prose-extracted table, labelled.

    exp_170: a startup pass reconstructs prose docs into queryable `prose__<doc>`
    views (materialised under a runtime dir). Here we surface a profile + a few
    sample rows so the agent knows the view exists and queries it (like any CSV),
    without inlining the whole table.
    """
    import csv as _csv

    from experiments.exp_171_prose_router import flags
    if not flags.prose_extract_on():
        return None
    try:
        from experiments.exp_171_prose_router.prose_extractor import extracted_csvs
        csvs = extracted_csvs(task.task_id)
    except Exception:
        return None
    if not csvs:
        return None
    blocks = [
        "# [PROSE-EXTRACTED] views — reconstructed from prose docs by a startup pass.",
        "Each is a queryable DuckDB view (like any CSV) holding a table that was "
        "NARRATED in a prose doc. It is a best-effort reconstruction — treat values "
        "as provisional and spot-check against the source doc (`read_doc`) before "
        "relying on a specific value. Only a profile + sample is shown; query the "
        "view for the full data.",
    ]
    for p in csvs:
        try:
        ...
        sample = "\n".join("    " + ", ".join(r) for r in data[:3])
        blocks.append(
            f"## view `{p.stem}`  ({len(data)} rows × {len(header)} cols)  [prose-extracted]\n"
            f"  columns: {', '.join(header)}\n"
            f"  sample (first 3 rows):\n{sample}\n"
            f"  → `SELECT ... FROM {p.stem}` for the full table."
        )
    return "\n\n".join(blocks)
...
def build_preamble(
    ...
    *,
    ...
    try:
        from experiments.exp_171_prose_router.tools.duckdb_unified import describe_catalog
    ...
    _prose_sec = _build_prose_extracted_section(task)
    if _prose_sec:
        fixed_parts.append(_prose_sec)
