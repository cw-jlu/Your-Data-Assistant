...
levers; env vars can still override each lever.
  ...
  EXP171_ANSWER_SHAPE=1  → lever ①  DISTINCT→JOIN-fanout-only + NULL/BLANK preserve
  EXP171_PROSE=1         → lever ②  pypdf read + prose_docs inventory +
                                     catalog-404→doc redirect + prose-parse protocol
  EXP171_VIDEO=1         → lever ③  pre-extract video summary into preamble
                                     (and stop attaching the raw video to the loop)
  EXP171_KEYFRAMES=1     → attach extracted unique video keyframes as images
                                     (and stop attaching the raw video to the loop)
  EXP171_VIDEO_KEYFRAME_NOTE=1 → extract visible task-relevant keyframe text
                                     into preamble + source router payload
  EXP171_WATCH_VIDEO=1   → expose a watch_video tool instead of auto-attaching
                                     raw video/keyframes to the first message
  EXP171_PDF_PREPROCESS=1 → use pre-extracted PDF text cache for prose docs
                                     and surface PDF doc previews in preamble
  EXP171_PROSE_INDEX=1 → surface cached Qwen sub-agent prose indexes in preamble
  EXP171_SOURCE_ROUTER=1 → classify whether SQL should be primary/support/avoided
                                     from the pre-answer exploration trace
  EXP171_ANTI_AGG=1 → classify lookup/list answer shape and suppress
                                     row-collapsing math hints
  EXP171_ANTI_AGG_SQL_GUARD=1 → POC: reject final SQL whose output SELECT list
                                     aggregates when anti_agg says NO_AGG
  EXP171_MATH_OTHER_ONLY=1 → fire the math-formula advisor ONLY when the domain
                                     router returns 'other' (BULL/EHR have ~0%
                                     complex calc); off → fire on every task
  EXP171_EXPLORE_SHAPE=1 → also inject the DISTINCT-fanout-only + NULL/BLANK-
  ...
  EXP171_PREFIX_CACHE=0 → disable task-scoped X-Prefix-Cache-Key runtime header
...
_DEFAULT_ON = {
    "EXP171_ANSWER_SHAPE",
    "EXP171_PROSE",
    "EXP171_VIDEO_KEYFRAME_NOTE",
    "EXP171_PDF_PREPROCESS",
    "EXP171_SOURCE_ROUTER",
    "EXP171_ANTI_AGG",
    "EXP171_ANTI_AGG_SQL_GUARD",
    "EXP171_DOMAIN_ROUTER",
    # EXP171_MATH_OTHER_ONLY intentionally NOT default-on: gating the math
...
def answer_shape_on() -> bool:
    return _on("EXP171_ANSWER_SHAPE")


def prose_on() -> bool:
    return _on("EXP171_PROSE")


def video_on() -> bool:
    return _on("EXP171_VIDEO")


def keyframes_on() -> bool:
    return _on("EXP171_KEYFRAMES")


def video_keyframe_note_on() -> bool:
    return _on("EXP171_VIDEO_KEYFRAME_NOTE")


def watch_video_on() -> bool:
    return _on("EXP171_WATCH_VIDEO")


def pdf_preprocess_on() -> bool:
    return _on("EXP171_PDF_PREPROCESS")


def prose_index_on() -> bool:
    return _on("EXP171_PROSE_INDEX")


def source_router_on() -> bool:
    return _on("EXP171_SOURCE_ROUTER")


def anti_agg_on() -> bool:
    return _on("EXP171_ANTI_AGG")


def anti_agg_sql_guard_on() -> bool:
    return _on("EXP171_ANTI_AGG_SQL_GUARD")


def domain_router_on() -> bool:
    return _on("EXP171_DOMAIN_ROUTER")


def math_other_only_on() -> bool:
    return _on("EXP171_MATH_OTHER_ONLY")


def explore_shape_on() -> bool:
    return _on("EXP171_EXPLORE_SHAPE")


def prose_extract_on() -> bool:
    return _on("EXP171_PROSE_EXTRACT")
