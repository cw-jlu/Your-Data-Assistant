...
from experiments.exp_014_cascade_two_agent.agents.exploration_agent import create_exploration_agent
from experiments.exp_014_cascade_two_agent.agents.answer_agent import create_answer_agent
from experiments.exp_014_cascade_two_agent.config import AppConfig
from experiments.exp_014_cascade_two_agent.preamble import build_preamble
from experiments.exp_014_cascade_two_agent.prompts.answer import build_spec_preamble
from experiments.exp_014_cascade_two_agent.runtime import AgentSpec
...
# Derive experiment prefix from the parent directory name (e.g. "exp_014_cascade_two_agent")
...
def _run_single_task_core(
    *,
    ...
    # Phase 1: Exploration — explore data, commit spec via commit_spec tool.
    exploration_agent = create_exploration_agent(
        model=effective_model,
        preamble=preamble_result.text,
        tools=tools,
    )
    phase1_result = exploration_agent.run(task)

    # Extract spec from the terminal commit_spec step (if Phase 1 succeeded).
    spec = AgentSpec.empty()
    for step in phase1_result.steps:
        if step.action == "commit_spec" and step.ok:
            spec = AgentSpec.from_step_record(step)
            break

    # Phase 2: Answer — receive spec, write one DuckDB SQL query, submit answer.
    spec_preamble = build_spec_preamble(spec.data)
    answer_agent = create_answer_agent(model=effective_model, spec_preamble=spec_preamble)
    phase2_result = answer_agent.run(task)

    # Label steps by phase so trace analysis can distinguish them.
    phase1_steps = [{"phase": 1, **s.to_dict()} for s in phase1_result.steps]
    phase2_steps = [{"phase": 2, **s.to_dict()} for s in phase2_result.steps]

    answer = phase2_result.answer
    failure_reason: str | None = None
    if answer is None:
        failure_reason = phase2_result.failure_reason or "Phase 2 did not produce an answer."
...
def _run_single_task_with_timeout(*, task_id: str, config: AppConfig) -> dict[str, Any]:
    ...
    # Use spawn (not fork) so the subprocess starts with a clean interpreter.
    # Forked children inherit pandas/numpy/sqlite/httpx state from the parent,
    # which intermittently deadlocks once a preamble pulls those imports in.
    ...
    # Drain the queue while the child runs so a large pickled result cannot
    # fill the pipe buffer and deadlock against process.join().
    ...
    while True:
        ...
        if remaining <= 0:
            break
        try:
            result = queue.get(timeout=min(remaining, 1.0))
            break
        except Exception:
...
def run_benchmark(
    *,
    ...
    if effective_workers == 1:
        ...
        shared_tools = tools or create_exploration_tool_registry()
        task_artifacts = []
