...
15. The context preamble shows SCHEMA ONLY (column names + dtypes + row counts; no actual data values).
    You MUST use discovery tools to fetch actual values before drawing any conclusions:
    - stat_file(path): check file size and exact line/row count
    - grep_file(path, pattern): search for specific values in CSV, doc, or JSON files
    - read_csv(path, max_rows=N): fetch sample rows to understand data distributions
    - read_doc(path, max_chars=N): read sections of a document
    Never assume data values from column names alone — always verify with tools first.
