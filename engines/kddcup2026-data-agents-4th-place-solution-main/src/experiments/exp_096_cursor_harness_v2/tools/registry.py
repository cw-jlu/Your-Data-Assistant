...
from experiments.exp_096_cursor_harness_v2.tools.filesystem import (
    grep_file as grep_file_impl,
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
    resolve_context_path,
    stat_file as stat_file_impl,
)
from experiments.exp_096_cursor_harness_v2.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_096_cursor_harness_v2.tools.sqlite import (
...
def _stat_file(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    path = str(action_input["path"])
    return ToolExecutionResult(ok=True, content=stat_file_impl(task, path))
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "grep_file": ToolSpec(
            name="grep_file",
            description=(
                "Search for lines matching a keyword pattern (case-insensitive substring) "
                "in ANY file (CSV, markdown, JSON, text). Returns up to max_matches snippets, "
                "each with context_lines surrounding lines. "
                "IMPORTANT: the preamble shows schema only — use grep_file to find actual values. "
                "Example: grep_file('csv/data.csv', 'Tokyo') to find rows containing 'Tokyo'."
            ),
            input_schema={
                "path": "relative/path/to/file.csv",
                "pattern": "keyword_or_value",
                "context_lines": 3,
                "max_matches": 20,
            },
        ),
        "stat_file": ToolSpec(
            name="stat_file",
            description=(
                "Return file metadata: size in bytes, line count, and file extension. "
                "Use this to check file size before reading, or to count rows in a CSV "
                "without loading the full file. Especially useful when preamble shows only schema."
            ),
