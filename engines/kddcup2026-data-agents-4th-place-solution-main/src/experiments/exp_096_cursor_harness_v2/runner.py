...
from experiments.exp_096_cursor_harness_v2.agent import ReActAgent, ReActAgentConfig
from experiments.exp_096_cursor_harness_v2.config import AppConfig
from experiments.exp_096_cursor_harness_v2.preamble import build_preamble
from experiments.exp_096_cursor_harness_v2.tools.registry import (
    ToolRegistry,
...
_MAJORITY_K = 1  # vestigial: only 1 attempt so any value works
# Single-attempt class with Qwen3.5 official "thinking precise" sampling.
# Hypothesis = cursor harness effect (schema-only ~5K preamble + grep_file/
# stat_file discovery tools) is BIG-EFFECT (+0.05+) per PRIORITIES; therefore
# single-attempt's std~0.04 is sufficient to detect it cleanly, and we save
# 2/3 of bench time vs 3-attempt class. If the run-1 score clears 0.65 and
# n=3 mean shows Δ > +0.02 vs single-attempt pseudo-base (~0.69), promote
# to 3-attempt class for confirmation.
# Original exp_082_cursor_harness used T=0.0 single-attempt and was poisoned
# by the workers=20 era vLLM saturation (n=3 mean=0.6174 with miss=8.3); v2
# retests with clean infra (workers=10) + R1 official sampling.
_ATTEMPT_TEMPS: tuple[float, ...] = (0.6,)  # Qwen3.5 precise, SINGLE attempt
