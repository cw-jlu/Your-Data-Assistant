...
from experiments.exp_167_domain_pruner.agent import ReActAgent, ReActAgentConfig
from experiments.exp_167_domain_pruner.phased_agent import (
    PhasedReActAgent,
    PhasedAgentConfig,
)
from experiments.exp_167_domain_pruner.adaptive_vote import adaptive_vote
from experiments.exp_167_domain_pruner.config import AppConfig
from experiments.exp_167_domain_pruner import flags
from experiments.exp_167_domain_pruner.anti_aggregation import (
    no_agg_note,
    review_aggregation,
)
...
from experiments.exp_167_domain_pruner.prefix_cache import with_prefix_cache_header
from experiments.exp_167_domain_pruner.tools.registry import (
    ToolRegistry,
...
def _ensure_runtime_cache_roots(config: AppConfig) -> None:
    ...
    if flags.pdf_preprocess_on() and not _os.environ.get("EXP167_PDF_TEXT_CACHE_ROOT"):
        _os.environ["EXP167_PDF_TEXT_CACHE_ROOT"] = str(
            Path("/tmp") / "kobushi_exp149_pdf_text_cache" / run_name
        )
    if (
        flags.keyframes_on() or flags.video_keyframe_note_on()
    ) and not _os.environ.get("EXP167_KEYFRAME_ROOT"):
        _os.environ["EXP167_KEYFRAME_ROOT"] = str(
...
def _run_single_task_core(
    *,
    ...
    # unknown general DB that may need such calc). Toggle with EXP167_MATH_OTHER_ONLY.
    ...
    trace_root = _os.environ.get("EXP167_TRACE_ROOT")
    if trace_root:
