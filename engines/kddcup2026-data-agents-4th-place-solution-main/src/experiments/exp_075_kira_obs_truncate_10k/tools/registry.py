...
from experiments.exp_075_kira_obs_truncate_10k.tools.filesystem import (
    list_context_tree,
    read_csv_preview,
    read_doc_preview,
    read_json_preview,
...
from experiments.exp_075_kira_obs_truncate_10k.tools.python_exec import (
    execute_python_code,
    execute_python_for_answer,
)
from experiments.exp_075_kira_obs_truncate_10k.tools.sqlite import (
...
_OBS_MAX_BYTES = 10_000


def _truncate_obs(content: dict[str, Any], max_bytes: int = _OBS_MAX_BYTES) -> dict[str, Any]:
    """Cap large observations to keep context size bounded.

    Checks total JSON byte size. If > max_bytes, truncates 'rows' (list) first,
    then 'output' (string). Returns content unchanged if within limit.
    """
    import json
    ...
    if len(raw) <= max_bytes:
        return content
    result = dict(content)
    for field in ("rows", "output", "data", "content"):
        ...
        elif isinstance(val, str) and len(val.encode("utf-8")) > max_bytes // 4:
            result = {
                **result,
                field: val[: max_bytes // 4] + "...[truncated]",
                "_truncated": True,
                "_full_size": len(val),
            }
...
def _read_csv(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    content = read_csv_preview(task, path, max_rows=max_rows)
    return ToolExecutionResult(ok=True, content=_truncate_obs(content))
...
def _execute_python(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    return ToolExecutionResult(ok=bool(content.get("success")), content=_truncate_obs(content))
