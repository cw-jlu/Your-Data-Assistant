...
def _load_task_record(task_json_path: Path) -> TaskRecord:
    ...
    # Phase 2 drops `difficulty`. Require task_id+question; treat missing
    # difficulty as "" and allow extra unknown fields forward-compat.
    required = {"task_id", "question"}
    actual_keys = set(payload)
    if not required.issubset(actual_keys):
        raise ValueError(
            f"Required task.json keys for {task_json_path.parent.name}: "
            f"need {sorted(required)}, got {sorted(actual_keys)}"
        )
