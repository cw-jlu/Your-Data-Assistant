...
点击窗口顶部始终可见的“LLM 设置”，填写 OpenAI-compatible 的 API URL、模型名和
API Key。客户端不内置默认模型服务，也不内置任何 API Key。

- API URL、模型名和运行参数只保存在当前 Windows 用户的本机应用存储中；
- API Key 只保留在当前应用运行期间，关闭客户端后需要重新填写；
- API Key 不写入 `localStorage`、SQLite、上传目录或任务记录；
- 某些 Agent 需要临时配置文件时，客户端会在任务结束后立即删除该文件。
...
API Key 不会持久化到 SQLite 或浏览器本地存储。Mamba 的上游配置加载器无法通过
环境变量覆盖 Key，因此客户端只会生成运行期临时配置，并在子进程结束后删除。
